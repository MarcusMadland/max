# If max.cmake was compiled without tools or cross compiled without host having tools,
...
if(TARGET max::bin2c)
	...
	function(_max_bin2c_parse ARG_OUT)
		set(options "")
		...
		if(ARG_INPUT_FILE)
			...
			message(SEND_ERROR "Call to _max_bin2c_parse() must have an INPUT_FILE")
		endif()
		...
		if(ARG_OUTPUT_FILE)
			...
			message(SEND_ERROR "Call to _max_bin2c_parse() must have an OUTPUT_FILE")
		endif()
		...
		if(ARG_ARRAY_NAME)
			...
			message(SEND_ERROR "Call to _max_bin2c_parse() must have an ARRAY_NAME")
		endif()

		set(${ARG_OUT} ${CLI} PARENT_SCOPE)
	endfunction()

	# max_compile_binary_to_header(
	#	INPUT_FILE filename
	#	OUTPUT_FILE filename
	#	ARRAY_NAME name
	# )
	#
	function(max_compile_binary_to_header)
		set(options "")
		set(oneValueArgs INPUT_FILE;OUTPUT_FILE;ARRAY_NAME)
		set(multiValueArgs "")
		cmake_parse_arguments(ARG "${options}" "${oneValueArgs}" "${multiValueArgs}" "${ARGN}")
...
if(TARGET max::texturec)
	...
	#	[AS extension]
	# )
	function(_max_texturec_parse ARG_OUT)
	...
	function(max_compile_texture)
		...
		_max_texturec_parse(CLI ${ARGV})
		add_custom_command(
			OUTPUT ${ARG_OUTPUT} #
			COMMAND max::texturec ${CLI} #
...
# If max.cmake was compiled without tools or cross compiled without host having tools,
# then don't provide helper functions
if(TARGET max::geometryc)
	# _max_geometryc_parse(
	#	FILE filename
	#	OUTPUT filename
	#	[SCALE scale]
	#	[CCW]
	...
	# )
	function(_max_geometryc_parse ARG_OUT)
		cmake_parse_arguments(
			ARG #
	...
	function(max_compile_geometry)
		...
		_max_geometryc_parse(CLI ${ARGV})
		add_custom_command(
			OUTPUT ${ARG_OUTPUT} #
			COMMAND max::geometryc ${CLI} #
			MAIN_DEPENDENCY ${ARG_INPUT} #
		)
	endfunction()
endif()
...
if(TARGET max::shaderc)
	...
	#	[WERROR]
	# )
	function(_max_shaderc_parse ARG_OUT)
		cmake_parse_arguments(
			ARG
		...
		if(ARG_FILE)
			...
			message(SEND_ERROR "Call to _max_shaderc_parse() must have an input file path specified.")
		endif()
		...
		if(ARG_OUTPUT)
			...
			message(SEND_ERROR "Call to _max_shaderc_parse() must have an output file path specified.")
		endif()
		...
		foreach(P ${PLATFORMS})
			if(ARG_${P})
				if(PLATFORM)
					message(SEND_ERROR "Call to _max_shaderc_parse() cannot have both flags ${PLATFORM} and ${P}.")
					return()
				endif()
				set(PLATFORM "${P}")
			endif()
		...
		if(PLATFORM STREQUAL "")
			message(SEND_ERROR "Call to _max_shaderc_parse() must have a platform flag: ${PLATFORMS}")
			return()
		...
		foreach(T ${TYPES})
			if(ARG_${T})
				if(TYPE)
					message(SEND_ERROR "Call to _max_shaderc_parse() cannot have both flags ${TYPE} and ${T}.")
					return()
				endif()
				set(TYPE "${T}")
			endif()
		endforeach()
		if("${TYPE}" STREQUAL "")
			message(SEND_ERROR "Call to _max_shaderc_parse() must have a type flag: ${TYPES}")
			return()
		...
		if(ARG_PROFILE)
			...
			message(SEND_ERROR "Call to _max_shaderc_parse() must have a shader profile.")
		endif()
	...
	function(_max_get_profile_ext PROFILE PROFILE_EXT)
	...
	# )
	#
	function(max_compile_shader_to_header)
		set(options "")
		...
		foreach(SHADER_FILE ${ARGS_SHADERS})
			...
			foreach(PROFILE ${PROFILES})
				_max_get_profile_ext(${PROFILE} PROFILE_EXT)
				...
				_max_shaderc_parse(
					...
					INCLUDES ${MAX_SHADER_INCLUDE_PATH} ${ARGS_INCLUDE_DIRS}
					BIN2C BIN2C ${SHADER_FILE_NAME_WE}_${PROFILE_EXT}
				)
				list(APPEND OUTPUTS ${OUTPUT})
				list(APPEND ALL_OUTPUTS ${OUTPUT})
				list(APPEND COMMANDS COMMAND max::shaderc ${CLI})
			endforeach()
