...
#	endif
	...
	struct EntryContext
	{
		...
		int run(int _argc, const char* const* _argv)
		{
			...
			m_window[0] = glfwCreateWindow(BGFX_DEFAULT_WIDTH
				, BGFX_DEFAULT_HEIGHT
				, "bgfx"
				, NULL
				, NULL
			...
			m_eventQueue.postSizeEvent(handle, BGFX_DEFAULT_WIDTH, BGFX_DEFAULT_HEIGHT);

			for (uint32_t ii = 0; ii < BGFX_CONFIG_MAX_GAMEPADS; ++ii)
			{
					...
					case GLFW_WINDOW_CREATE:
						{
							...
							if (msg->m_flags & BGFX_WINDOW_FLAG_ASPECT_RATIO)
							{
		...
		GLFWwindow* m_window[BGFX_CONFIG_MAX_WINDOWS];
		bx::HandleAllocT<BGFX_CONFIG_MAX_WINDOWS> m_windowAlloc;

		GamepadGLFW m_gamepad[BGFX_CONFIG_MAX_GAMEPADS];
	...
	static void joystickCb(int _jid, int _action)
	{
		if (_jid >= BGFX_CONFIG_MAX_GAMEPADS)
		{
			return;
		}
