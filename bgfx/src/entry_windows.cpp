...
#if BGFX_CONFIG_USE_NATIVE && BX_PLATFORM_WINDOWS
...
//				XInputEnable   = (PFN_XINPUT_ENABLE   )bx::dlsym(m_xinputdll, "XInputEnable"  );
		...
		XINPUT_STATE m_state[BGFX_CONFIG_MAX_GAMEPADS];
		bool m_connected[BGFX_CONFIG_MAX_GAMEPADS];
	};
	...
	struct EntryContext
	{
		...
		int32_t run(int _argc, const char* const* _argv)
		{
			...
			m_flags[0] = 0
				| BGFX_WINDOW_FLAG_ASPECT_RATIO
				| BGFX_WINDOW_FLAG_FRAME
				;

			adjust(m_hwnd[0], BGFX_DEFAULT_WIDTH, BGFX_DEFAULT_HEIGHT, true);
			clear(m_hwnd[0]);

			m_width     = BGFX_DEFAULT_WIDTH;
			m_height    = BGFX_DEFAULT_HEIGHT;
			m_oldWidth  = BGFX_DEFAULT_WIDTH;
			m_oldHeight = BGFX_DEFAULT_HEIGHT;
				...
				case WM_SIZING:
					{
						...
						if (isValid(handle)
						&&  BGFX_WINDOW_FLAG_ASPECT_RATIO & m_flags[handle.idx])
						{
		...
		HWND m_hwnd[BGFX_CONFIG_MAX_WINDOWS];
		uint32_t m_flags[BGFX_CONFIG_MAX_WINDOWS];
		RECT m_rect;
		DWORD m_style;
		uint32_t m_width;
		uint32_t m_height;
