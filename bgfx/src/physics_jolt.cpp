...
namespace JPH
{
	...
	public:
		...
		virtual void DrawTriangle(JPH::RVec3Arg inV1, JPH::RVec3Arg inV2, JPH::RVec3Arg inV3, JPH::ColorArg inColor, ECastShadow inCastShadow) override
		{
			// @todo Batch together. 
			bgfx::dbgDrawBegin(0);
			bgfx::dbgDrawSetColor(0xff00ffff);
			//bgfx::dbgDrawSetWireframe(true);
...
namespace bgfx { namespace jolt
{
	...
	struct PhysicsContextJolt : public PhysicsContextI
	{
		...
		bool init(const Init& _init)
		{
			...
			// @todo Make bgfx::Init settings for physics.
			s_system = new JPH::PhysicsSystem();
			s_system->Init(
				BGFX_CONFIG_MAX_BODIES,
				0, // @todo
				BGFX_CONFIG_MAX_BODY_PAIRS,
				BGFX_CONFIG_MAX_CONTACT_CONSTRAINTS,
				m_bpli,
				m_ovbplf,
				m_olpf
			);
		...
		void simulate(const float _dt) override
		{
			...
			for (uint32_t ii = 0; ii < BGFX_CONFIG_MAX_BODIES; ++ii)
			{
			...
			// @todo Check for BGFX_DEBUG_PHYSICS
		...
		BodyRef m_bodies[BGFX_CONFIG_MAX_BODIES];
	};
...
} /* namespace jolt */ } // namespace bgfx

#endif // BGFX_CONFIG_PHYSICS_JOLT
