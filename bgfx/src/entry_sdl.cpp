...
//		SDL_Haptic*         m_haptic;
	...
	struct EntryContext
	{
							...
							case SDL_USER_WINDOW_TOGGLE_FRAME:
								{
									...
									if (isValid(handle) )
									{
										m_flags[handle.idx] ^= BGFX_WINDOW_FLAG_FRAME;
										SDL_SetWindowBordered(m_window[handle.idx], (SDL_bool)!!(m_flags[handle.idx] & BGFX_WINDOW_FLAG_FRAME) );
									}
								}
								break;
		...
		SDL_Window* m_window[BGFX_CONFIG_MAX_WINDOWS];
		uint32_t m_flags[BGFX_CONFIG_MAX_WINDOWS];

		bx::HandleAllocT<BGFX_CONFIG_MAX_GAMEPADS> m_gamepadAlloc;
		GamepadSDL m_gamepad[BGFX_CONFIG_MAX_GAMEPADS];
