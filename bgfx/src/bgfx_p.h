...
#if !defined(BGFX_DEFAULT_WIDTH) && !defined(BGFX_DEFAULT_HEIGHT)
#	define BGFX_DEFAULT_WIDTH  1280
#	define BGFX_DEFAULT_HEIGHT 720
#elif !defined(BGFX_DEFAULT_WIDTH) || !defined(BGFX_DEFAULT_HEIGHT)
#	error "Both BGFX_DEFAULT_WIDTH and BGFX_DEFAULT_HEIGHT must be defined."
#endif // BGFX_DEFAULT_WIDTH && BGFX_DEFAULT_HEIGHT

#define BGFX_IMPLEMENT_EVENT(_class, _type) \
...
#define BGFX_PHYSICS_NOOP_NAME		  "Noop"
#define BGFX_PHYSICS_JOLT_NAME		  "Jolt"
...
#undef IMPLEMENT_HANDLE
	...
	{
		BGFX_IMPLEMENT_EVENT(AxisEvent, Event::Axis);
	...
	{
		BGFX_IMPLEMENT_EVENT(CharEvent, Event::Char);
	...
	{
		BGFX_IMPLEMENT_EVENT(GamepadEvent, Event::Gamepad);
	...
	{
		BGFX_IMPLEMENT_EVENT(KeyEvent, Event::Key);
	...
	{
		BGFX_IMPLEMENT_EVENT(MouseEvent, Event::Mouse);
	...
	{
		BGFX_IMPLEMENT_EVENT(SizeEvent, Event::Size);
	...
	{
		BGFX_IMPLEMENT_EVENT(WindowEvent, Event::Window);
	...
	{
		BGFX_IMPLEMENT_EVENT(SuspendEvent, Event::Suspend);
	...
	{
		BGFX_IMPLEMENT_EVENT(DropFileEvent, Event::DropFile);
...
#undef SORT_KEY_RENDER_DRAW
	...
	struct MaterialRef
	{
		...
		typedef bx::HandleHashMapT<BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL> UniformHashMap;
		UniformHashMap m_uniformHashMap;
		Uniform		   m_uniforms[BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL];

		float    m_data[BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL * 16];
		uint32_t m_num;
	...
	struct EntityRef
	{
		bx::HandleHashMapT<BGFX_CONFIG_MAX_COMPONENTS_PER_ENTITY * 2> m_components;
	...
	BX_ALIGN_DECL_CACHE_LINE(struct) Frame
	{
		...
		FreeHandle<MaterialHandle,	   BGFX_CONFIG_MAX_MATERIALS>      m_freeMaterial;
		FreeHandle<MeshHandle,		   BGFX_CONFIG_MAX_MESHES>         m_freeMesh;
		FreeHandle<ComponentHandle,	   BGFX_CONFIG_MAX_COMPONENTS>     m_freeComponent;
		FreeHandle<EntityHandle,	   BGFX_CONFIG_MAX_ENTITIES>       m_freeEntity;
...
//				m_uniformSet.insert(_handle.idx);
	...
	struct DebugDrawShared
	{
		...
		bgfx::ProgramHandle m_program[DebugProgram::Count];
...
#undef WARN_RESET_CAPS_FLAGS
		...
		BGFX_API_FUNC(void processInput())
		{
		...
		BGFX_API_FUNC(void addBindings(const char* _name, const InputBinding* _bindings))
		{
			m_inputBindingsMap.insert(stl::make_pair(stl::string(_name), _bindings));
		}

		BGFX_API_FUNC(void removeBindings(const char* _name))
		{
		...
		BGFX_API_FUNC(void addMappings(uint32_t _id, const InputMapping* _mappings))
		{
			m_inputMappingsMap.insert(stl::make_pair(_id, _mappings));
		}

		BGFX_API_FUNC(void removeMappings(uint32_t _id))
		{
		...
		BGFX_API_FUNC(float getValue(uint32_t _id, uint32_t _action))
		{
			InputMappingMap::iterator it = m_inputMappingsMap.find(_id);
			if (it != m_inputMappingsMap.end())
			{
...
//			setNameForHandle(_handle, _name);
		...
		{
			UniformHandle uniforms[BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL];
			uint16_t num = getShaderUniforms(_shader, uniforms, BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL);
			...
			for (uint32_t ii = 0; ii < num; ++ii)
			{
				...
				{
					uniform.m_th = BGFX_INVALID_HANDLE;
				}
		...
		BGFX_API_FUNC(MaterialHandle createMaterial(ProgramHandle _program))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
			...
			mr.m_num = 0;
			bx::memSet(&mr.m_data[0], 0, sizeof(float) * (BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL * 16));
			...
			if (uint32_t num = mr.m_uniformHashMap.getNumElements() > BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL)
			{
				BX_TRACE("Material created with %u uniforms, maximum amount allowed per material is %u. Uniforms above this will be ignored.", num, BGFX_CONFIG_MAX_UNIFORMS_PER_MATERIAL);
			}

			return handle;
		}

		BGFX_API_FUNC(void setMaterial(MaterialHandle _material))
		{
			...
			for (uint32_t ii = 0; ii < mr.m_uniformHashMap.getNumElements(); ++ii)
			{
				...
				bgfx::getUniformInfo(uniform.m_uh, info);

				if (info.type == bgfx::UniformType::Sampler)
				{
					if (isValid(uniform.m_th))
					{
						bgfx::setTexture(uniform.m_stage, uniform.m_uh, uniform.m_th /*, flags @todo */);
					}
				}
				else
				{
					bgfx::setUniform(uniform.m_uh, &mr.m_data[uniform.m_dataIdx], uniform.m_num);
				}
			}
		}

		BGFX_API_FUNC(void addParameter(MaterialHandle _material, const char* _name, float* _value, uint32_t _num = 1))
		{
			MaterialRef& mr = m_materialRef[_material.idx];
		...
		BGFX_API_FUNC(void addParameter(MaterialHandle _material, const char* _name, uint32_t _stage, TextureHandle _texture))
		{
			MaterialRef& mr = m_materialRef[_material.idx];
			uint16_t idx = mr.m_uniformHashMap.find(bx::hash<bx::HashMurmur2A>(_name));
		...
		BGFX_API_FUNC(void destroyMaterial(MaterialHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("destroyMaterial", m_materialHandle, _handle);

			if (!isValid(_handle))
			{
				BX_WARN(false, "Passing invalid material handle to bgfx::destroyMaterial.");
				return;
			}
		...
		BGFX_API_FUNC(MeshHandle createMesh(const Memory* _mem, bool _ramcopy))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
			...
			while (4 == bx::read(&reader, chunk, &err) && err.isOk())
			{
				...
				case kChunkVertexBuffer:
				{
					...
					group.m_vbh = createVertexBuffer(mem, mr.m_layout, BGFX_BUFFER_NONE);
				}
				break;
				...
				case kChunkIndexBuffer:
				{
					...
					group.m_ibh = createIndexBuffer(mem, BGFX_BUFFER_NONE);
				}
				break;
		...
		BGFX_API_FUNC(MeshHandle createMesh(const Memory* _vertices, const Memory* _indices, const VertexLayout& _layout, bool _dynamic))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
			...
			if (_dynamic)
			{
				group.m_dvbh = bgfx::createDynamicVertexBuffer(_vertices, _layout);
				group.m_dibh = bgfx::createDynamicIndexBuffer(_indices);
			}
			else
			{
				group.m_vbh = bgfx::createVertexBuffer(_vertices, _layout);
				group.m_ibh = bgfx::createIndexBuffer(_indices);
			}
			mr.m_groups.push_back(group);
		...
		BGFX_API_FUNC(void update(MeshHandle _mesh, const Memory* _vertices, const Memory* _indices))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
		...
		BGFX_API_FUNC(MeshQuery* queryMesh(MeshHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getMeshInfo", m_meshHandle, _handle);

			MeshRef& mr = m_meshRef[_handle.idx];

			m_meshQuery.m_num = (uint32_t)mr.m_groups.size();
			BX_ASSERT(m_meshQuery.m_num <= BGFX_CONFIG_MAX_MESH_GROUPS, "Mesh has more groups than allowed.");
		...
		BGFX_API_FUNC(void destroyMesh(MeshHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("destroyMesh", m_meshHandle, _handle);

			if (!isValid(_handle))
			{
				BX_WARN(false, "Passing invalid mesh handle to bgfx::destroyMesh.");
				return;
			}
		...
		BGFX_API_FUNC(ComponentHandle createComponent(void* _data, uint32_t _size))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
		...
		BGFX_API_FUNC(void destroyComponent(ComponentHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("destroyComponent", m_componentHandle, _handle);

			if (!isValid(_handle))
			{
				BX_WARN(false, "Passing invalid component handle to bgfx::destroyComponent.");
				return;
			}
		...
		BGFX_API_FUNC(EntityHandle createEntity(bool _destroyComponents))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
		...
		BGFX_API_FUNC(void addComponent(EntityHandle _entity, ComponentHandle _component, uint32_t _hash))
		{
			EntityRef& er = m_entityRef[_entity.idx];
			er.m_components.insert(_hash, _component.idx);
		}

		BGFX_API_FUNC(void* getComponent(EntityHandle _handle, uint32_t _hash))
		{
			EntityRef& er = m_entityRef[_handle.idx];
		...
		BGFX_API_FUNC(EntityQuery* queryEntities(const HashQuery& _hashes))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			m_entityQuery.m_num = 0;

			for (uint16_t idx = 0, end = BGFX_CONFIG_MAX_ENTITIES; idx < end; ++idx)
			{
		...
		BGFX_API_FUNC(void destroyEntity(EntityHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("destroyEntity", m_entityHandle, _handle);

			if (!isValid(_handle))
			{
				BX_WARN(false, "Passing invalid entity handle to bgfx::destroyEntity.");
				return;
			}
		...
		BGFX_API_FUNC(BodyHandle createBody(CollisionShape::Enum _shape, const bx::Vec3& _pos, const bx::Quaternion& _quat, const bx::Vec3& _scale, LayerType::Enum _layer, MotionType::Enum _motion, Activation::Enum _activation, float _maxVelocity, uint8_t _flags))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
		...
		BGFX_API_FUNC(void setPosition(BodyHandle _handle, const bx::Vec3& _pos, Activation::Enum _activation))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("setPosition", m_bodyHandle, _handle);

			m_physicsCtx->setPosition(_handle, _pos, _activation);
		}

		BGFX_API_FUNC(bx::Vec3 getPosition(BodyHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getPosition", m_bodyHandle, _handle);

			return m_physicsCtx->getPosition(_handle);
		}

		BGFX_API_FUNC(void setRotation(BodyHandle _handle, const bx::Quaternion& _rot, Activation::Enum _activation))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("setRotation", m_bodyHandle, _handle);

			m_physicsCtx->setRotation(_handle, _rot, _activation);
		}

		BGFX_API_FUNC(bx::Quaternion getRotation(BodyHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getRotation", m_bodyHandle, _handle);

			return m_physicsCtx->getRotation(_handle);
		}

		BGFX_API_FUNC(void setLinearVelocity(BodyHandle _handle, const bx::Vec3& _velocity))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("setLinearVelocity", m_bodyHandle, _handle);

			m_physicsCtx->setLinearVelocity(_handle, _velocity);
		}

		BGFX_API_FUNC(bx::Vec3 getLinearVelocity(BodyHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getLinearVelocity", m_bodyHandle, _handle);

			return m_physicsCtx->getLinearVelocity(_handle);
		}

		BGFX_API_FUNC(void setAngularVelocity(BodyHandle _handle, const bx::Vec3& _angularVelocity))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("setAngularVelocity", m_bodyHandle, _handle);

			m_physicsCtx->setAngularVelocity(_handle, _angularVelocity);
		}

		BGFX_API_FUNC(bx::Vec3 getAngularVelocity(BodyHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getAngularVelocity", m_bodyHandle, _handle);

			return m_physicsCtx->getAngularVelocity(_handle);
		}

		BGFX_API_FUNC(void addLinearAndAngularVelocity(BodyHandle _handle, const bx::Vec3& _linearVelocity, const bx::Vec3& _angularVelocity))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addLinearAndAngularVelocity", m_bodyHandle, _handle);

			m_physicsCtx->addLinearAndAngularVelocity(_handle, _linearVelocity, _angularVelocity);
		}

		BGFX_API_FUNC(void addLinearImpulse(BodyHandle _handle, const bx::Vec3& _impulse))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addLinearImpulse", m_bodyHandle, _handle);

			m_physicsCtx->addLinearImpulse(_handle, _impulse);
		}

		BGFX_API_FUNC(void addAngularImpulse(BodyHandle _handle, const bx::Vec3& _impulse))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addAngularImpulse", m_bodyHandle, _handle);

			m_physicsCtx->addAngularImpulse(_handle, _impulse);
		}

		BGFX_API_FUNC(void addBuoyancyImpulse(BodyHandle _handle, const bx::Vec3& _surfacePosition, const bx::Vec3& _surfaceNormal, float _buoyancy, float _linearDrag, float _angularDrag, const bx::Vec3& _fluidVelocity, const bx::Vec3& _gravity, float _deltaTime))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addBuoyancyImpulse", m_bodyHandle, _handle);
		...
		BGFX_API_FUNC(void addForce(BodyHandle _handle, const bx::Vec3& _force, Activation::Enum _activation))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addForce", m_bodyHandle, _handle);

			m_physicsCtx->addForce(_handle, _force,  _activation);
		}

		BGFX_API_FUNC(void addTorque(BodyHandle _handle, const bx::Vec3& _torque, Activation::Enum _activation))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addTorque", m_bodyHandle, _handle);

			m_physicsCtx->addTorque(_handle, _torque, _activation);
		}

		BGFX_API_FUNC(void addMovement(BodyHandle _handle, const bx::Vec3& _position, const bx::Quaternion& _rotation, const float _deltaTime))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("addMovement", m_bodyHandle, _handle);

			m_physicsCtx->addMovement(_handle, _position, _rotation, _deltaTime);
		}

		BGFX_API_FUNC(void setFriction(BodyHandle _handle, float _friction))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("setFriction", m_bodyHandle, _handle);

			m_physicsCtx->setFriction(_handle, _friction);
		}

		BGFX_API_FUNC(float getFriction(BodyHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getFriction", m_bodyHandle, _handle);

			return m_physicsCtx->getFriction(_handle);
		}

		BGFX_API_FUNC(void getGroundInfo(BodyHandle _handle, GroundInfo& _info))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("getGroundInfo", m_bodyHandle, _handle);

			return m_physicsCtx->getGroundInfo(_handle, _info);
		}

		BGFX_API_FUNC(void destroyBody(BodyHandle _handle))
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);

			BGFX_CHECK_HANDLE("destroyBody", m_bodyHandle, _handle);
		...
		BGFX_API_FUNC(const bx::Vec3 getGravity())
		{
		...
		BGFX_API_FUNC(void cmdAdd(const char* _name, ConsoleFn _fn, void* _userData))
		{
		...
		BGFX_API_FUNC(void cmdRemove(const char* _name))
		{
			const uint32_t cmd = bx::hash<bx::HashMurmur2A>(_name, (uint32_t)bx::strLen(_name));
		...
		BGFX_API_FUNC(void cmdExec(const char* _cmd))
		{
		...
		BodyHandle				  m_freeBodyHandle[BGFX_CONFIG_MAX_BODIES];
		...
		bx::HandleAllocT<BGFX_CONFIG_MAX_MATERIALS> m_materialHandle;
		bx::HandleAllocT<BGFX_CONFIG_MAX_MESHES> m_meshHandle;
		bx::HandleAllocT<BGFX_CONFIG_MAX_COMPONENTS> m_componentHandle;
		bx::HandleAllocT<BGFX_CONFIG_MAX_ENTITIES> m_entityHandle;
		bx::HandleAllocT<BGFX_CONFIG_MAX_BODIES> m_bodyHandle;
		...
		typedef bx::HandleHashMapT<BGFX_CONFIG_MAX_MESHES * 2> MeshHashMap;
		MeshHashMap m_meshHashMap;
		MeshRef		m_meshRef[BGFX_CONFIG_MAX_MESHES];
		...
		EntityRef	    m_entityRef[BGFX_CONFIG_MAX_ENTITIES];
		ComponentRef	m_componentRef[BGFX_CONFIG_MAX_COMPONENTS];
		...
		InputMouse m_mouse;
		Gamepad m_gamepad[BGFX_CONFIG_MAX_GAMEPADS];
