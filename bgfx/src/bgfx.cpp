...
#if BGFX_CONFIG_IMPLEMENT_DEFAULT_ALLOCATOR
...
#endif // BGFX_CONFIG_IMPLEMENT_DEFAULT_ALLOCATOR
...
#	define BGFX_CONFIG_MEMORY_TRACKING (BGFX_CONFIG_DEBUG && BX_CONFIG_SUPPORTS_THREADING)
	...
	static uint32_t s_width =  BGFX_DEFAULT_WIDTH;
	static uint32_t s_height = BGFX_DEFAULT_HEIGHT;
	...
	bgfx::VertexLayout DebugPosVertex::ms_layout;
	...
	int main(int _argc, const char* const* _argv)
	{
		...
		setWindowSize({ 0 }, BGFX_DEFAULT_WIDTH, BGFX_DEFAULT_HEIGHT);
	...
	WindowState s_window[BGFX_CONFIG_MAX_WINDOWS];
...
#include "charset.h"
	...
	void DebugDrawShared::init()
	{
		...
		m_program[DebugProgram::Lines] = bgfx::createProgram(
		...
		m_program[DebugProgram::LinesStipple] = bgfx::createProgram(
		...
		m_program[DebugProgram::Fill] = bgfx::createProgram(
		...
		m_program[DebugProgram::FillMesh] = bgfx::createProgram(
			bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill_mesh")
			, bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_fill")
			, true
		);

		m_program[DebugProgram::FillLit] = bgfx::createProgram(
		...
		m_program[DebugProgram::FillLitMesh] = bgfx::createProgram(
			bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill_lit_mesh")
			, bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_fill_lit")
			, true
		);

		m_program[DebugProgram::FillTexture] = bgfx::createProgram(
	...
	void DebugDrawEncoderImpl::flush()
	{
		if (0 != m_pos)
		{
			...
			{
				bgfx::TransientVertexBuffer tvb;
				bgfx::allocTransientVertexBuffer(&tvb, m_pos, DebugPosVertex::ms_layout);
...
#undef LIMITS
	...
	bool Context::init(const Init& _init)
	{
		...
		// @todo Move elsewhere? 
		m_entityQuery.alloc(BGFX_CONFIG_MAX_ENTITIES);
		m_meshQuery.alloc(BGFX_CONFIG_MAX_MESH_GROUPS);
...
#define BGFX_PHYSICS_CONTEXT(_namespace)                           \
	namespace _namespace                                            \
	{                                                               \
	...
	BGFX_PHYSICS_CONTEXT(noop);
	BGFX_PHYSICS_CONTEXT(jolt);

#undef BGFX_PHYSICS_CONTEXT
	...
	{
		{ noop::physicsCreate, noop::physicsDestroy, BGFX_PHYSICS_NOOP_NAME, true }, // Noop
		{ jolt::physicsCreate, jolt::physicsDestroy, BGFX_PHYSICS_JOLT_NAME, true }, // Jolt
	};
...
#undef BGFX_ENCODER
	...
	void dbgDrawBegin(uint16_t _viewId, bool _depthTestLess, bgfx::Encoder* _encoder)
	{
		BGFX_CHECK_API_THREAD();
	...
	void dbgDrawQuad(bgfx::TextureHandle _handle, const bx::Vec3& _normal, const bx::Vec3& _center, float _size)
	{
		BGFX_CHECK_API_THREAD();
	...
	const bgfx::Memory* loadMemory(const char* _filePath)
	{
		bx::FileReader reader;
		if (bx::open(&reader, _filePath))
		{
