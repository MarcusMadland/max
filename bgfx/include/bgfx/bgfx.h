...
///
#if 1
#define BGFX_IMPLEMENT_MAIN(_app, ...)                 \
	int _main_(int _argc, char** _argv)                 \
	{                                                   \
			_app app(__VA_ARGS__);                      \
			return bgfx::runApp(&app, _argc, _argv);   \
	}
#else
#define BGFX_IMPLEMENT_MAIN(_app, ...) \
...
///
#define BGFX_INPUT_BINDING_END { bgfx::Key::None, bgfx::Modifier::None, 0, NULL, NULL }
#define BGFX_INPUT_MAPPING_END { UINT32_MAX, NULL }
...
namespace bgfx
{
	...
	BGFX_HANDLE(MaterialHandle)
	BGFX_HANDLE(MeshHandle)
	BGFX_HANDLE(ComponentHandle)
	BGFX_HANDLE(EntityHandle)
	BGFX_HANDLE(BodyHandle)
		//BGFX_HANDLE(TrueTypeHandle)		 
		//BGFX_HANDLE(FontHandle)			 
		//BGFX_HANDLE(SoundHandle)		 
		//BGFX_HANDLE(EmitterHandle)		 
		//BGFX_HANDLE(EmitterSpriteHandle) 

	/// App interface to implement a bgfx game or application.
	///
	...
	struct Encoder
	{
		...
		///
		void dbgDrawBegin(uint16_t _viewId, bool _depthTestLess = true, bgfx::Encoder* _encoder = nullptr);
	...
	/// Returns bgfx engine allocator.
	///
	bx::AllocatorI* getAllocator();

	/// Returns bgfx engine delta time.
	///
	...
	///
	WindowHandle createWindow(int32_t _x, int32_t _y, uint32_t _width, uint32_t _height, uint32_t _flags = BGFX_WINDOW_FLAG_NONE, const char* _title = "");
	...
	BodyHandle createBody(
		...
		uint8_t _flags = BGFX_BODY_ALLOW_ALL
	);
