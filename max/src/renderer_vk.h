...
#ifndef MAX_RENDERER_VK_H_HEADER_GUARD
#define MAX_RENDERER_VK_H_HEADER_GUARD
...
#if MAX_CONFIG_DEBUG_ANNOTATION
#	define MAX_VK_BEGIN_DEBUG_UTILS_LABEL(_name, _abgr)         \
		BX_MACRO_BLOCK_BEGIN                                     \
...
#	define MAX_VK_END_DEBUG_UTILS_LABEL()               \
		BX_MACRO_BLOCK_BEGIN                             \
			vkCmdEndDebugUtilsLabelEXT(m_commandBuffer); \
		BX_MACRO_BLOCK_END
#else
#	define MAX_VK_BEGIN_DEBUG_UTILS_LABEL(_view, _abgr) BX_UNUSED(_view, _abgr)
#	define MAX_VK_END_DEBUG_UTILS_LABEL() BX_NOOP()
#endif // MAX_CONFIG_DEBUG_ANNOTATION

#define MAX_VK_PROFILER_BEGIN(_view, _abgr)                      \
	BX_MACRO_BLOCK_BEGIN                                          \
		MAX_VK_BEGIN_DEBUG_UTILS_LABEL(s_viewName[view], _abgr); \
		MAX_PROFILER_BEGIN(s_viewName[view], _abgr);             \
	BX_MACRO_BLOCK_END

#define MAX_VK_PROFILER_BEGIN_LITERAL(_name, _abgr)   \
	BX_MACRO_BLOCK_BEGIN                               \
		MAX_VK_BEGIN_DEBUG_UTILS_LABEL(_name, _abgr); \
		MAX_PROFILER_BEGIN_LITERAL("" _name, _abgr);  \
	BX_MACRO_BLOCK_END

#define MAX_VK_PROFILER_END()           \
	BX_MACRO_BLOCK_BEGIN                 \
		MAX_PROFILER_END();             \
		MAX_VK_END_DEBUG_UTILS_LABEL(); \
	BX_MACRO_BLOCK_END
...
#undef VK_DESTROY_FUNC
	...
	struct BindInfo
	{
		UniformHandle uniformHandle = MAX_INVALID_HANDLE;
		BindType::Enum type;
		uint32_t binding;
	...
	struct ShaderVK
	{
		...
		BindInfo m_bindInfo[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];

		TextureBindInfo m_textures[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
		uint8_t m_numTextures;

		uint32_t m_uniformBinding;
		uint16_t m_numBindings;
		VkDescriptorSetLayoutBinding m_bindings[2 * MAX_CONFIG_MAX_TEXTURE_SAMPLERS + 1];
	...
	struct TimerQueryVK
	{
		...
		Result m_result[MAX_CONFIG_MAX_VIEWS+1];
		Query m_query[MAX_CONFIG_MAX_VIEWS*4];
	...
	struct OcclusionQueryVK
	{
		...
		OcclusionQueryHandle m_handle[MAX_CONFIG_MAX_OCCLUSION_QUERIES];
	...
	constexpr uint32_t kMaxBackBuffers = bx::max(MAX_CONFIG_MAX_BACK_BUFFERS, 10);
	...
	struct FrameBufferVK
	{
		...
		TextureHandle m_texture[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
		...
		uint8_t m_num;
		uint8_t m_numTh;
		Attachment m_attachment[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
		...
		VkImageView m_textureImageViews[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
	...
	struct CommandQueueVK
	{
		...
		CommandList m_commandList[MAX_CONFIG_MAX_FRAME_LATENCY];

		uint32_t             m_numWaitSemaphores;
		VkSemaphore          m_waitSemaphores[MAX_CONFIG_MAX_FRAME_BUFFERS];
		VkPipelineStageFlags m_waitSemaphoreStages[MAX_CONFIG_MAX_FRAME_BUFFERS];
		uint32_t             m_numSignalSemaphores;
		VkSemaphore          m_signalSemaphores[MAX_CONFIG_MAX_FRAME_BUFFERS];
		...
		ResourceArray m_release[MAX_CONFIG_MAX_FRAME_LATENCY];
...
#endif // MAX_RENDERER_VK_H_HEADER_GUARD
