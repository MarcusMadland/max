...
#ifndef MAX_RENDERER_H_HEADER_GUARD
#define MAX_RENDERER_H_HEADER_GUARD
...
namespace max
{
	...
	struct ViewState
	{
		...
		Matrix4  m_viewTmp[MAX_CONFIG_MAX_VIEWS];
		Matrix4  m_viewProj[MAX_CONFIG_MAX_VIEWS];
		Matrix4* m_view;
		Rect     m_rect;
		Matrix4  m_invView;
		Matrix4  m_invProj;
	...
	struct Profiler
	{
		Profiler(Frame* _frame, Ty& _gpuTimer, const char (*_viewName)[MAX_CONFIG_MAX_VIEW_NAME], bool _enabled = true)
			: m_viewName(_viewName)
			, m_frame(_frame)
			, m_gpuTimer(_gpuTimer)
			, m_queryIdx(UINT32_MAX)
			, m_numViews(0)
			, m_enabled(_enabled && 0 != (_frame->m_debug & MAX_DEBUG_PROFILER) )
		{
		}
		...
		void begin(uint16_t _view)
		{
			if (m_enabled)
			{
				...
				bx::strCopy(viewStats.name
					, MAX_CONFIG_MAX_VIEW_NAME
					, &m_viewName[_view][MAX_CONFIG_MAX_VIEW_NAME_RESERVED]
					);
			}
		}
		...
		const char (*m_viewName)[MAX_CONFIG_MAX_VIEW_NAME];
		Frame*   m_frame;
		Ty&      m_gpuTimer;
		uint32_t m_queryIdx;
		uint16_t m_numViews;
...
#endif // MAX_RENDERER_H_HEADER_GUARD
