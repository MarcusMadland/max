...
#if MAX_CONFIG_USE_NATIVE && BX_PLATFORM_OSX
...
namespace entry
{
	...
	struct Context
	{
			...
			, m_mouseLock(NULL)
		{
			...
			for(int ii=0; ii<MAX_CONFIG_MAX_WINDOWS; ++ii)
			{
				m_window[ii] = NULL;
			}
		}
		...
		int32_t run(int _argc, const char* const* _argv)
		{
			...
			const float centerX = (screenRect.size.width  - (float)MAX_DEFAULT_WIDTH )*0.5f;
			const float centerY = (screenRect.size.height - (float)MAX_DEFAULT_HEIGHT)*0.5f;
			NSString* appName = [[NSProcessInfo processInfo] processName];
			createWindow(centerX, centerY, MAX_DEFAULT_WIDTH, MAX_DEFAULT_HEIGHT, MAX_WINDOW_FLAG_NONE, [appName UTF8String]);
		...
		NSWindow* m_window[MAX_CONFIG_MAX_WINDOWS];
