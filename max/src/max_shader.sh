...
#ifndef MAX_SHADER_H_HEADER_GUARD
#define MAX_SHADER_H_HEADER_GUARD

#if !defined(MAX_CONFIG_MAX_BONES)
#	define MAX_CONFIG_MAX_BONES 32
#endif // !defined(MAX_CONFIG_MAX_BONES)

#ifndef __cplusplus

#if MAX_SHADER_LANGUAGE_HLSL > 300
...
#	define LOOP
#	define UNROLL
#endif // MAX_SHADER_LANGUAGE_HLSL > 300

#if (MAX_SHADER_LANGUAGE_HLSL > 300 || MAX_SHADER_LANGUAGE_METAL || MAX_SHADER_LANGUAGE_SPIRV) && MAX_SHADER_TYPE_FRAGMENT
#	define EARLY_DEPTH_STENCIL [earlydepthstencil]
#else
#	define EARLY_DEPTH_STENCIL
#endif // MAX_SHADER_LANGUAGE_HLSL > 300 && MAX_SHADER_TYPE_FRAGMENT
...
#if MAX_SHADER_LANGUAGE_HLSL \
 || MAX_SHADER_LANGUAGE_PSSL \
 || MAX_SHADER_LANGUAGE_SPIRV \
 || MAX_SHADER_LANGUAGE_METAL
...
#	if MAX_SHADER_LANGUAGE_HLSL > 400 || MAX_SHADER_LANGUAGE_PSSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
#		define REGISTER(_type, _reg) register( _type[_reg] )
#	else
#		define REGISTER(_type, _reg) register(_type ## _reg)
#	endif // MAX_SHADER_LANGUAGE_HLSL

#	if MAX_SHADER_LANGUAGE_HLSL > 300 || MAX_SHADER_LANGUAGE_PSSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
#		if MAX_SHADER_LANGUAGE_HLSL > 400 || MAX_SHADER_LANGUAGE_PSSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
...
#		endif // MAX_SHADER_LANGUAGE_HLSL > 400

#		if MAX_SHADER_LANGUAGE_HLSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
...
#		endif // MAX_SHADER_LANGUAGE_HLSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
...
#		endif // !MAX_SHADER_LANGUAGE_SPIRV
...
struct MaxISampler2D
{
	Texture2D<ivec4> m_texture;
};

struct MaxUSampler2D
{
	Texture2D<uvec4> m_texture;
};

struct MaxSampler2DArray
{
	SamplerState m_sampler;
	Texture2DArray m_texture;
};

struct MaxSampler2DShadow
{
	SamplerComparisonState m_sampler;
	Texture2D m_texture;
};

struct MaxSampler2DArrayShadow
{
	SamplerComparisonState m_sampler;
	Texture2DArray m_texture;
};
...
struct MaxISampler3D
{
	Texture3D<ivec4> m_texture;
};

struct MaxUSampler3D
{
	Texture3D<uvec4> m_texture;
};

struct MaxSamplerCube
{
	SamplerState m_sampler;
	TextureCube m_texture;
};

struct MaxSamplerCubeShadow
{
	SamplerComparisonState m_sampler;
	TextureCube m_texture;
};

struct MaxSampler2DMS
{
	Texture2DMS<vec4> m_texture;
};

vec4 maxTexture2D(MaxSampler2D _sampler, vec2 _coord)
{
	return _sampler.m_texture.Sample(_sampler.m_sampler, _coord);
}

vec4 maxTexture2DBias(MaxSampler2D _sampler, vec2 _coord, float _bias)
{
	return _sampler.m_texture.SampleBias(_sampler.m_sampler, _coord, _bias);
}

vec4 maxTexture2DLod(MaxSampler2D _sampler, vec2 _coord, float _level)
{
	return _sampler.m_texture.SampleLevel(_sampler.m_sampler, _coord, _level);
}

vec4 maxTexture2DLodOffset(MaxSampler2D _sampler, vec2 _coord, float _level, ivec2 _offset)
{
	return _sampler.m_texture.SampleLevel(_sampler.m_sampler, _coord, _level, _offset);
}

vec4 maxTexture2DProj(MaxSampler2D _sampler, vec3 _coord)
{
	vec2 coord = _coord.xy * rcp(_coord.z);
	return _sampler.m_texture.Sample(_sampler.m_sampler, coord);
}

vec4 maxTexture2DProj(MaxSampler2D _sampler, vec4 _coord)
{
	vec2 coord = _coord.xy * rcp(_coord.w);
	return _sampler.m_texture.Sample(_sampler.m_sampler, coord);
}

vec4 maxTexture2DGrad(MaxSampler2D _sampler, vec2 _coord, vec2 _dPdx, vec2 _dPdy)
{
	return _sampler.m_texture.SampleGrad(_sampler.m_sampler, _coord, _dPdx, _dPdy);
}

vec4 maxTexture2DArray(MaxSampler2DArray _sampler, vec3 _coord)
{
	return _sampler.m_texture.Sample(_sampler.m_sampler, _coord);
}

vec4 maxTexture2DArrayLod(MaxSampler2DArray _sampler, vec3 _coord, float _lod)
{
	return _sampler.m_texture.SampleLevel(_sampler.m_sampler, _coord, _lod);
}

vec4 maxTexture2DArrayLodOffset(MaxSampler2DArray _sampler, vec3 _coord, float _level, ivec2 _offset)
{
	return _sampler.m_texture.SampleLevel(_sampler.m_sampler, _coord, _level, _offset);
}

float maxShadow2D(MaxSampler2DShadow _sampler, vec3 _coord)
{
	return _sampler.m_texture.SampleCmpLevelZero(_sampler.m_sampler, _coord.xy, _coord.z);
}

float maxShadow2DProj(MaxSampler2DShadow _sampler, vec4 _coord)
{
	vec3 coord = _coord.xyz * rcp(_coord.w);
	return _sampler.m_texture.SampleCmpLevelZero(_sampler.m_sampler, coord.xy, coord.z);
}

vec2 maxTextureSize(MaxSampler2DShadow _sampler, int _lod)
{
	vec2 result;
...
vec4 maxShadow2DArray(MaxSampler2DArrayShadow _sampler, vec4 _coord)
{
	return _sampler.m_texture.SampleCmpLevelZero(_sampler.m_sampler, _coord.xyz, _coord.w);
}

vec2 maxTextureSize(MaxSampler2DArrayShadow _sampler, int _lod)
{
	vec2 result;
	float numberOfMipMapLevels;
...
vec4 maxTexture3D(MaxSampler3D _sampler, vec3 _coord)
{
	return _sampler.m_texture.Sample(_sampler.m_sampler, _coord);
}

vec4 maxTexture3DLod(MaxSampler3D _sampler, vec3 _coord, float _level)
{
	return _sampler.m_texture.SampleLevel(_sampler.m_sampler, _coord, _level);
}

ivec4 maxTexture3D(MaxISampler3D _sampler, vec3 _coord)
{
	uvec3 size;
...
uvec4 maxTexture3D(MaxUSampler3D _sampler, vec3 _coord)
{
	uvec3 size;
	_sampler.m_texture.GetDimensions(size.x, size.y, size.z);
	return _sampler.m_texture.Load(ivec4(_coord * size, 0) );
...
vec4 maxTextureCube(MaxSamplerCube _sampler, vec3 _coord)
{
	return _sampler.m_texture.Sample(_sampler.m_sampler, _coord);
}

vec4 maxTextureCubeBias(MaxSamplerCube _sampler, vec3 _coord, float _bias)
{
	return _sampler.m_texture.SampleBias(_sampler.m_sampler, _coord, _bias);
}

vec4 maxTextureCubeLod(MaxSamplerCube _sampler, vec3 _coord, float _level)
{
	return _sampler.m_texture.SampleLevel(_sampler.m_sampler, _coord, _level);
}

float maxShadowCube(MaxSamplerCubeShadow _sampler, vec4 _coord)
{
	return _sampler.m_texture.SampleCmpLevelZero(_sampler.m_sampler, _coord.xyz, _coord.w);
}

vec4 maxTexelFetch(MaxSampler2D _sampler, ivec2 _coord, int _lod)
{
	return _sampler.m_texture.Load(ivec3(_coord, _lod) );
}

vec4 maxTexelFetchOffset(MaxSampler2D _sampler, ivec2 _coord, int _lod, ivec2 _offset)
{
	return _sampler.m_texture.Load(ivec3(_coord, _lod), _offset );
}

vec2 maxTextureSize(MaxSampler2D _sampler, int _lod)
{
	vec2 result;
	float numberOfMipMapLevels;
	_sampler.m_texture.GetDimensions(_lod, result.x, result.y, numberOfMipMapLevels);
...
vec2 maxTextureSize(MaxISampler2D _sampler, int _lod)
{
	vec2 result;
	float numberOfMipMapLevels;
	_sampler.m_texture.GetDimensions(_lod, result.x, result.y, numberOfMipMapLevels);
...
vec2 maxTextureSize(MaxUSampler2D _sampler, int _lod)
{
	vec2 result;
	float numberOfMipMapLevels;
	_sampler.m_texture.GetDimensions(_lod, result.x, result.y, numberOfMipMapLevels);
...
vec4 maxTextureGather0(MaxSampler2D _sampler, vec2 _coord)
{
	return _sampler.m_texture.GatherRed(_sampler.m_sampler, _coord);
}

vec4 maxTextureGather1(MaxSampler2D _sampler, vec2 _coord)
{
	return _sampler.m_texture.GatherGreen(_sampler.m_sampler, _coord);
}

vec4 maxTextureGather2(MaxSampler2D _sampler, vec2 _coord)
{
	return _sampler.m_texture.GatherBlue(_sampler.m_sampler, _coord);
}

vec4 maxTextureGather3(MaxSampler2D _sampler, vec2 _coord)
{
	return _sampler.m_texture.GatherAlpha(_sampler.m_sampler, _coord);
}

vec4 maxTextureGatherOffset0(MaxSampler2D _sampler, vec2 _coord, ivec2 _offset)
{
	return _sampler.m_texture.GatherRed(_sampler.m_sampler, _coord, _offset);
}

vec4 maxTextureGatherOffset1(MaxSampler2D _sampler, vec2 _coord, ivec2 _offset)
{
	return _sampler.m_texture.GatherGreen(_sampler.m_sampler, _coord, _offset);
}

vec4 maxTextureGatherOffset2(MaxSampler2D _sampler, vec2 _coord, ivec2 _offset)
{
	return _sampler.m_texture.GatherBlue(_sampler.m_sampler, _coord, _offset);
}

vec4 maxTextureGatherOffset3(MaxSampler2D _sampler, vec2 _coord, ivec2 _offset)
{
	return _sampler.m_texture.GatherAlpha(_sampler.m_sampler, _coord, _offset);
}

vec4 maxTextureGather0(MaxSampler2DArray _sampler, vec3 _coord)
{
	return _sampler.m_texture.GatherRed(_sampler.m_sampler, _coord);
}

vec4 maxTextureGather1(MaxSampler2DArray _sampler, vec3 _coord)
{
	return _sampler.m_texture.GatherGreen(_sampler.m_sampler, _coord);
}

vec4 maxTextureGather2(MaxSampler2DArray _sampler, vec3 _coord)
{
	return _sampler.m_texture.GatherBlue(_sampler.m_sampler, _coord);
}

vec4 maxTextureGather3(MaxSampler2DArray _sampler, vec3 _coord)
{
	return _sampler.m_texture.GatherAlpha(_sampler.m_sampler, _coord);
}

ivec4 maxTexelFetch(MaxISampler2D _sampler, ivec2 _coord, int _lod)
{
	return _sampler.m_texture.Load(ivec3(_coord, _lod) );
}

uvec4 maxTexelFetch(MaxUSampler2D _sampler, ivec2 _coord, int _lod)
{
	return _sampler.m_texture.Load(ivec3(_coord, _lod) );
}

vec4 maxTexelFetch(MaxSampler2DMS _sampler, ivec2 _coord, int _sampleIdx)
{
	return _sampler.m_texture.Load(_coord, _sampleIdx);
}

vec4 maxTexelFetch(MaxSampler2DArray _sampler, ivec3 _coord, int _lod)
{
	return _sampler.m_texture.Load(ivec4(_coord, _lod) );
}

vec4 maxTexelFetch(MaxSampler3D _sampler, ivec3 _coord, int _lod)
{
	return _sampler.m_texture.Load(ivec4(_coord, _lod) );
}

vec3 maxTextureSize(MaxSampler3D _sampler, int _lod)
{
	vec3 result;
	float numberOfMipMapLevels;
...
#		define SAMPLER2D(_name, _reg) \
			...
			static MaxSampler2D _name = { _name ## Sampler, _name ## Texture }
#		define ISAMPLER2D(_name, _reg) \
			uniform Texture2D<ivec4> _name ## Texture : REGISTER(t, _reg); \
			static MaxISampler2D _name = { _name ## Texture }
#		define USAMPLER2D(_name, _reg) \
			uniform Texture2D<uvec4> _name ## Texture : REGISTER(t, _reg); \
			static MaxUSampler2D _name = { _name ## Texture }
#		define sampler2D MaxSampler2D
#		define texture2D(_sampler, _coord) maxTexture2D(_sampler, _coord)
#		define texture2DBias(_sampler, _coord, _bias) maxTexture2DBias(_sampler, _coord, _bias)
#		define texture2DLod(_sampler, _coord, _level) maxTexture2DLod(_sampler, _coord, _level)
#		define texture2DLodOffset(_sampler, _coord, _level, _offset) maxTexture2DLodOffset(_sampler, _coord, _level, _offset)
#		define texture2DProj(_sampler, _coord) maxTexture2DProj(_sampler, _coord)
#		define texture2DGrad(_sampler, _coord, _dPdx, _dPdy) maxTexture2DGrad(_sampler, _coord, _dPdx, _dPdy)

#		define SAMPLER2DARRAY(_name, _reg) \
			uniform SamplerState _name ## Sampler : REGISTER(s, _reg); \
			uniform Texture2DArray _name ## Texture : REGISTER(t, _reg); \
			static MaxSampler2DArray _name = { _name ## Sampler, _name ## Texture }
#		define sampler2DArray MaxSampler2DArray
#		define texture2DArray(_sampler, _coord) maxTexture2DArray(_sampler, _coord)
#		define texture2DArrayLod(_sampler, _coord, _lod) maxTexture2DArrayLod(_sampler, _coord, _lod)
#		define texture2DArrayLodOffset(_sampler, _coord, _level, _offset) maxTexture2DArrayLodOffset(_sampler, _coord, _level, _offset)

#		define SAMPLER2DMS(_name, _reg) \
			uniform Texture2DMS<vec4> _name ## Texture : REGISTER(t, _reg); \
			static MaxSampler2DMS _name = { _name ## Texture }
#		define sampler2DMS MaxSampler2DMS

#		define SAMPLER2DSHADOW(_name, _reg) \
			uniform SamplerComparisonState _name ## SamplerComparison : REGISTER(s, _reg); \
			uniform Texture2D _name ## Texture : REGISTER(t, _reg); \
			static MaxSampler2DShadow _name = { _name ## SamplerComparison, _name ## Texture }
#		define sampler2DShadow MaxSampler2DShadow
#		define shadow2D(_sampler, _coord) maxShadow2D(_sampler, _coord)
#		define shadow2DProj(_sampler, _coord) maxShadow2DProj(_sampler, _coord)

#		define SAMPLER2DARRAYSHADOW(_name, _reg) \
			uniform SamplerComparisonState _name ## SamplerComparison : REGISTER(s, _reg); \
			uniform Texture2DArray _name ## Texture : REGISTER(t, _reg); \
			static MaxSampler2DArrayShadow _name = { _name ## SamplerComparison, _name ## Texture }
#		define sampler2DArrayShadow MaxSampler2DArrayShadow
#		define shadow2DArray(_sampler, _coord) maxShadow2DArray(_sampler, _coord)

#		define SAMPLER3D(_name, _reg) \
			uniform SamplerState _name ## Sampler : REGISTER(s, _reg); \
			uniform Texture3D _name ## Texture : REGISTER(t, _reg); \
			static MaxSampler3D _name = { _name ## Sampler, _name ## Texture }
#		define ISAMPLER3D(_name, _reg) \
			uniform Texture3D<ivec4> _name ## Texture : REGISTER(t, _reg); \
			static MaxISampler3D _name = { _name ## Texture }
#		define USAMPLER3D(_name, _reg) \
			uniform Texture3D<uvec4> _name ## Texture : REGISTER(t, _reg); \
			static MaxUSampler3D _name = { _name ## Texture }
#		define sampler3D MaxSampler3D
#		define texture3D(_sampler, _coord) maxTexture3D(_sampler, _coord)
#		define texture3DLod(_sampler, _coord, _level) maxTexture3DLod(_sampler, _coord, _level)

#		define SAMPLERCUBE(_name, _reg) \
			uniform SamplerState _name ## Sampler : REGISTER(s, _reg); \
			uniform TextureCube _name ## Texture : REGISTER(t, _reg); \
			static MaxSamplerCube _name = { _name ## Sampler, _name ## Texture }
#		define samplerCube MaxSamplerCube
#		define textureCube(_sampler, _coord) maxTextureCube(_sampler, _coord)
#		define textureCubeBias(_sampler, _coord, _bias) maxTextureCubeBias(_sampler, _coord, _bias)
#		define textureCubeLod(_sampler, _coord, _level) maxTextureCubeLod(_sampler, _coord, _level)

#		define SAMPLERCUBESHADOW(_name, _reg) \
			uniform SamplerComparisonState _name ## SamplerComparison : REGISTER(s, _reg); \
			uniform TextureCube _name ## Texture : REGISTER(t, _reg); \
			static MaxSamplerCubeShadow _name = { _name ## SamplerComparison, _name ## Texture }
#		define samplerCubeShadow MaxSamplerCubeShadow
#		define shadowCube(_sampler, _coord) maxShadowCube(_sampler, _coord)

#		define texelFetch(_sampler, _coord, _lod) maxTexelFetch(_sampler, _coord, _lod)
#		define texelFetchOffset(_sampler, _coord, _lod, _offset) maxTexelFetchOffset(_sampler, _coord, _lod, _offset)
#		define textureSize(_sampler, _lod) maxTextureSize(_sampler, _lod)
#		define textureGather(_sampler, _coord, _comp) maxTextureGather ## _comp(_sampler, _coord)
#		define textureGatherOffset(_sampler, _coord, _offset, _comp) maxTextureGatherOffset ## _comp(_sampler, _coord, _offset)
#	else

#		define sampler2DShadow sampler2D

vec4 maxTexture2DProj(sampler2D _sampler, vec3 _coord)
{
	return tex2Dproj(_sampler, vec4(_coord.xy, 0.0, _coord.z) );
}

vec4 maxTexture2DProj(sampler2D _sampler, vec4 _coord)
{
	return tex2Dproj(_sampler, _coord);
}

float maxShadow2D(sampler2DShadow _sampler, vec3 _coord)
{
#if 0
...
float maxShadow2DProj(sampler2DShadow _sampler, vec4 _coord)
{
#if 0
	vec3 coord = _coord.xyz * rcp(_coord.w);
...
#	if MAX_SHADER_LANGUAGE_GLSL >= 130
...
#	endif // MAX_SHADER_LANGUAGE_GLSL >= 130
...
#endif // MAX_SHADER_LANGUAGE_*

vec2 vec2_splat(float _x) { return vec2(_x, _x); }
vec3 vec3_splat(float _x) { return vec3(_x, _x, _x); }
vec4 vec4_splat(float _x) { return vec4(_x, _x, _x, _x); }

#if MAX_SHADER_LANGUAGE_GLSL >= 130 || MAX_SHADER_LANGUAGE_HLSL || MAX_SHADER_LANGUAGE_PSSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
uvec2 uvec2_splat(uint _x) { return uvec2(_x, _x); }
uvec3 uvec3_splat(uint _x) { return uvec3(_x, _x, _x); }
uvec4 uvec4_splat(uint _x) { return uvec4(_x, _x, _x, _x); }
#endif // MAX_SHADER_LANGUAGE_GLSL >= 130 || MAX_SHADER_LANGUAGE_HLSL || MAX_SHADER_LANGUAGE_PSSL || MAX_SHADER_LANGUAGE_SPIRV || MAX_SHADER_LANGUAGE_METAL
...
uniform mat4  u_model[MAX_CONFIG_MAX_BONES];
...
#endif // MAX_SHADER_H_HEADER_GUARD
