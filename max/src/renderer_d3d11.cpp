...
#if MAX_CONFIG_RENDERER_DIRECT3D11
#	include "renderer_d3d11.h"
#	include <bx/pixelformat.h>

namespace max { namespace d3d11
{
	static wchar_t s_viewNameW[MAX_CONFIG_MAX_VIEWS][MAX_CONFIG_MAX_VIEW_NAME];
	...
	union Zero
	{
		...
		ID3D11RenderTargetView*    m_rtv[MAX_CONFIG_MAX_FRAME_BUFFERS];
	...
	struct TextureStage
	{
		...
		ID3D11UnorderedAccessView* m_uav[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
		ID3D11ShaderResourceView*  m_srv[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
		ID3D11SamplerState*        m_sampler[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
	};
...
//						| D3D11_CREATE_DEVICE_PREVENT_INTERNAL_THREADING_OPTIMIZATIONS
					...
					for (uint32_t ii = 0; ii < BX_COUNTOF(featureLevel) && FAILED(hr);)
					{
						...
						// Enable debug flags.
						flags |= (BX_ENABLED(MAX_CONFIG_DEBUG) ? D3D11_CREATE_DEVICE_DEBUG : 0);
						++ii;
					}
...

			{
				...
				if (MAX_PCI_ID_NVIDIA != m_dxgi.m_adapterDesc.VendorId)
				{
					m_nvapi.shutdown();
				}
				...
				if (NULL == g_platformData.backBuffer)
				{
					...
					m_swapBufferCount = bx::clamp<uint8_t>(_init.resolution.numBackBuffers, 2, MAX_CONFIG_MAX_BACK_BUFFERS);
					...
					m_scd.sampleDesc  = s_msaa[(_init.resolution.reset&MAX_RESET_MSAA_MASK)>>MAX_RESET_MSAA_SHIFT];
					...
					m_scd.alphaMode = (_init.resolution.reset & MAX_RESET_TRANSPARENT_BACKBUFFER)
					...
					m_scd.maxFrameLatency = bx::min<uint8_t>(_init.resolution.maxFrameLatency, MAX_CONFIG_MAX_FRAME_LATENCY);
					...
					if (NULL != m_scd.nwh)
					{
						...
						m_resolution.reset = _init.resolution.reset & (~MAX_RESET_INTERNAL_FORCE);
						...
						if (1 < m_scd.sampleDesc.Count)
						{
							...
							*/
							desc.Format     = (m_resolution.reset & MAX_RESET_SRGB_BACKBUFFER) ? s_textureFormat[m_resolution.format].m_fmtSrgb : s_textureFormat[m_resolution.format].m_fmt;
...

			{
				g_caps.supported |= (0
					...
					| (getIntelExtensions(m_device)
						...
						| MAX_CAPS_TEXTURE_DIRECT_ACCESS
						: 0)
					| MAX_CAPS_SWAP_CHAIN
				...
				m_directAccessSupport = 0 != (g_caps.supported & MAX_CAPS_TEXTURE_DIRECT_ACCESS);
				...
				{
					g_caps.limits.maxComputeBindings = bx::min(MAX_MAX_COMPUTE_BINDINGS
						, D3D_FEATURE_LEVEL_11_1 <= m_featureLevel
						? D3D11_1_UAV_SLOT_COUNT
						: D3D11_PS_CS_UAV_REGISTER_COUNT
						);

					g_caps.supported |= MAX_CAPS_TEXTURE_COMPARE_ALL;
				...
				||  m_featureLevel == D3D_FEATURE_LEVEL_10_1)
				{
					...
					{
						g_caps.supported |= MAX_CAPS_COMPUTE;
					}
				}
				...
				if (m_featureLevel <= D3D_FEATURE_LEVEL_9_3)
				{
					...
					{
						g_caps.supported |= MAX_CAPS_TEXTURE_COMPARE_LEQUAL;
					}
				}
				...
				// support for SV_ViewportArrayIndex and SV_RenderTargetArrayIndex in the vertex shader is optional
				{
					...
					{
						g_caps.supported |= MAX_CAPS_VIEWPORT_LAYER_ARRAY;
					}
				}
				...
				for (uint32_t ii = 0; ii < TextureFormat::Count; ++ii)
				{
					...
					if (DXGI_FORMAT_UNKNOWN != fmt)
					{
						if (BX_ENABLED(BX_PLATFORM_LINUX || BX_PLATFORM_WINDOWS || BX_PLATFORM_WINRT) )
						{
							...
							if (0 != (support & MAX_CAPS_FORMAT_TEXTURE_IMAGE_READ) )
							{
								D3D11_FEATURE_DATA_FORMAT_SUPPORT2 data2;

								// clear image flag for additional testing
								support &= ~MAX_CAPS_FORMAT_TEXTURE_IMAGE_READ;
				...
				for (uint32_t ii = 0; ii < MAX_CONFIG_MAX_VIEWS; ++ii)
				{
					...
					mbstowcs(s_viewNameW[ii], s_viewName[ii], MAX_CONFIG_MAX_VIEW_NAME_RESERVED);
				}
		...
		{
			return MAX_RENDERER_DIRECT3D11_NAME;
		}
		...
		{
			// Resource ref. counts might be messed up outside of max.
		...
		void updateViewName(ViewId _id, const char* _name) override
		{
			if (BX_ENABLED(MAX_CONFIG_DEBUG_ANNOTATION) )
			{
				mbstowcs(&s_viewNameW[_id][MAX_CONFIG_MAX_VIEW_NAME_RESERVED]
					, _name
					, BX_COUNTOF(s_viewNameW[0])-MAX_CONFIG_MAX_VIEW_NAME_RESERVED
					);
			}
		...
		void blitSetup(TextVideoMemBlitter& _blitter) override
		{
			...
			setFrameBuffer(MAX_INVALID_HANDLE, false, false);
			...
			uint64_t state = MAX_STATE_WRITE_RGB
				| MAX_STATE_WRITE_A
			...
			m_textures[_blitter.m_texture.idx].commit(0, MAX_SAMPLER_INTERNAL_DEFAULT, NULL);
			commitTextureStage();
		}
...
//			invalidateCache();
		...
		void postReset()
		{
			if (NULL != m_swapChain)
			{
				...
				desc.ViewDimension = (m_resolution.reset & MAX_RESET_MSAA_MASK)
		...
		void flip() override
		{
			if (!m_lost)
			{
				...
				uint32_t syncInterval = BX_ENABLED(!BX_PLATFORM_WINDOWS)
					...
					: !!(m_resolution.reset & MAX_RESET_VSYNC)
					;
		...
		bool updateResolution(const Resolution& _resolution)
		{
			...
			const bool wasSuspended = !!(m_resolution.reset & MAX_RESET_SUSPEND);
			...
			else if (suspended)
			{
				...
				m_dxgi.trim();
				suspend(m_device);
				m_resolution.reset |= MAX_RESET_SUSPEND;
				return true;
			}
			...
			bool depthClamp = true
				&& !!(_resolution.reset & MAX_RESET_DEPTH_CLAMP)
			...
			|| (m_resolution.reset&maskFlags) != (_resolution.reset&maskFlags) )
			{
				...
				bool resize = true
					...
					&& (m_resolution.reset&MAX_RESET_MSAA_MASK) == (flags&MAX_RESET_MSAA_MASK)
					;
				...
				// see comment in init() about why we don't worry about MAX_RESET_SRGB_BACKBUFFER here
				...
				else
				{
					...
					{
						updateMsaa(m_scd.format);
						m_scd.sampleDesc = s_msaa[(m_resolution.reset&MAX_RESET_MSAA_MASK)>>MAX_RESET_MSAA_SHIFT];
						...
						MAX_FATAL(SUCCEEDED(hr), max::Fatal::UnableToInitialize, "Failed to create swap chain.");
					}
		...
		void clear(const Clear& _clear, const float _palette[][4])
		{
			...
			else
			{
				...
				&&  MAX_CLEAR_COLOR & _clear.m_flags)
				{
				...
				&& (MAX_CLEAR_DEPTH|MAX_CLEAR_STENCIL) & _clear.m_flags)
				{
					DWORD flags = 0;
					flags |= (_clear.m_flags & MAX_CLEAR_DEPTH)   ? D3D11_CLEAR_DEPTH   : 0;
					flags |= (_clear.m_flags & MAX_CLEAR_STENCIL) ? D3D11_CLEAR_STENCIL : 0;
		...
		void setInputLayout(uint8_t _numStreams, const VertexLayout** _layouts, const ProgramD3D11& _program, uint16_t _numInstanceData)
		{
			...
			if (NULL == inputLayout)
			{
				D3D11_INPUT_ELEMENT_DESC vertexElements[Attrib::Count+1+MAX_CONFIG_MAX_INSTANCE_DATA_COUNT];
		...
		{
			_state &= MAX_D3D11_BLEND_STATE_MASK;

			bx::HashMurmur2A murmur;
			murmur.begin();
			murmur.add(_state);
			murmur.add(!!(MAX_STATE_BLEND_INDEPENDENT & _state)
				? _rgba
				: -1
				);
			...
			if (NULL == bs)
			{
				...
				desc.AlphaToCoverageEnable  = !!(MAX_STATE_BLEND_ALPHA_TO_COVERAGE & _state);
				desc.IndependentBlendEnable = !!(MAX_STATE_BLEND_INDEPENDENT       & _state);

				D3D11_RENDER_TARGET_BLEND_DESC* drt = &desc.RenderTarget[0];
				drt->BlendEnable = !!(MAX_STATE_BLEND_MASK & _state);
				...
				uint8_t writeMask = 0;
				writeMask |= (_state&MAX_STATE_WRITE_R) ? D3D11_COLOR_WRITE_ENABLE_RED   : 0;
				writeMask |= (_state&MAX_STATE_WRITE_G) ? D3D11_COLOR_WRITE_ENABLE_GREEN : 0;
				writeMask |= (_state&MAX_STATE_WRITE_B) ? D3D11_COLOR_WRITE_ENABLE_BLUE  : 0;
				writeMask |= (_state&MAX_STATE_WRITE_A) ? D3D11_COLOR_WRITE_ENABLE_ALPHA : 0;

				drt->RenderTargetWriteMask = writeMask;

				if (desc.IndependentBlendEnable)
				{
					for (uint32_t ii = 1, rgba = _rgba; ii < MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS; ++ii, rgba >>= 11)
					{
				...
				{
					for (uint32_t ii = 1; ii < MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS; ++ii)
					{
		...
		void setDepthStencilState(uint64_t _state, uint64_t _stencil = 0)
		{
			...
			_state &= 0 == func ? 0 : MAX_D3D11_DEPTH_STENCIL_MASK;

			uint32_t fstencil = unpackStencil(0, _stencil);
			uint32_t ref = (fstencil&MAX_STENCIL_FUNC_REF_MASK)>>MAX_STENCIL_FUNC_REF_SHIFT;
			...
			if (NULL == dss)
			{
				...
				desc.DepthWriteMask = !!(MAX_STATE_WRITE_Z & _state) ? D3D11_DEPTH_WRITE_MASK_ALL : D3D11_DEPTH_WRITE_MASK_ZERO;
				...
				desc.StencilReadMask  = (fstencil&MAX_STENCIL_FUNC_RMASK_MASK)>>MAX_STENCIL_FUNC_RMASK_SHIFT;
				desc.StencilWriteMask = 0xff;
				desc.FrontFace.StencilFailOp      = s_stencilOp[(fstencil&MAX_STENCIL_OP_FAIL_S_MASK)>>MAX_STENCIL_OP_FAIL_S_SHIFT];
				desc.FrontFace.StencilDepthFailOp = s_stencilOp[(fstencil&MAX_STENCIL_OP_FAIL_Z_MASK)>>MAX_STENCIL_OP_FAIL_Z_SHIFT];
				desc.FrontFace.StencilPassOp      = s_stencilOp[(fstencil&MAX_STENCIL_OP_PASS_Z_MASK)>>MAX_STENCIL_OP_PASS_Z_SHIFT];
				desc.FrontFace.StencilFunc        = s_cmpFunc[(fstencil&MAX_STENCIL_TEST_MASK)>>MAX_STENCIL_TEST_SHIFT];
				desc.BackFace.StencilFailOp       = s_stencilOp[(bstencil&MAX_STENCIL_OP_FAIL_S_MASK)>>MAX_STENCIL_OP_FAIL_S_SHIFT];
				desc.BackFace.StencilDepthFailOp  = s_stencilOp[(bstencil&MAX_STENCIL_OP_FAIL_Z_MASK)>>MAX_STENCIL_OP_FAIL_Z_SHIFT];
				desc.BackFace.StencilPassOp       = s_stencilOp[(bstencil&MAX_STENCIL_OP_PASS_Z_MASK)>>MAX_STENCIL_OP_PASS_Z_SHIFT];
				desc.BackFace.StencilFunc         = s_cmpFunc[(bstencil&MAX_STENCIL_TEST_MASK)>>MAX_STENCIL_TEST_SHIFT];
		...
		void setRasterizerState(uint64_t _state, bool _wireframe = false, bool _scissor = false)
		{
			_state &= 0
				...
				;
			_state |= _wireframe ? MAX_STATE_PT_LINES : MAX_STATE_NONE;
			_state |= _scissor   ? MAX_STATE_RESERVED_MASK : 0;
			_state &= ~(m_deviceInterfaceVersion >= 3 ? 0 : MAX_STATE_CONSERVATIVE_RASTER);

			ID3D11RasterizerState* rs = m_rasterizerStateCache.find(_state);
			if (NULL == rs)
			{
				uint32_t cull = (_state&MAX_STATE_CULL_MASK)>>MAX_STATE_CULL_SHIFT;
				...
				if (m_deviceInterfaceVersion >= 3)
				{
					...
					desc.FrontCounterClockwise = !!(_state&MAX_STATE_FRONT_CCW);
					desc.DepthBias             = 0;
					desc.DepthBiasClamp        = 0.0f;
					desc.SlopeScaledDepthBias  = 0.0f;
					desc.DepthClipEnable       = !m_depthClamp;
					desc.ScissorEnable         = _scissor;
					desc.MultisampleEnable     = !!(_state&MAX_STATE_MSAA);
					desc.AntialiasedLineEnable = !!(_state&MAX_STATE_LINEAA);
					desc.ForcedSampleCount     = 0;
					desc.ConservativeRaster    = !!(_state&MAX_STATE_CONSERVATIVE_RASTER)
		...
		ID3D11SamplerState* getSamplerState(uint32_t _flags, const float _rgba[4])
		{
			...
			{
				_flags |= MAX_SAMPLER_MIN_ANISOTROPIC|MAX_SAMPLER_MAG_ANISOTROPIC;
				_flags &= ~MAX_SAMPLER_MIP_MASK;
			}
			...
			if (NULL == sampler)
			{
				...
				const uint8_t  minFilter = s_textureFilter[0][(_flags&MAX_SAMPLER_MIN_MASK)>>MAX_SAMPLER_MIN_SHIFT];
				const uint8_t  magFilter = s_textureFilter[1][(_flags&MAX_SAMPLER_MAG_MASK)>>MAX_SAMPLER_MAG_SHIFT];
				const uint8_t  mipFilter = s_textureFilter[2][(_flags&MAX_SAMPLER_MIP_MASK)>>MAX_SAMPLER_MIP_SHIFT];
				...
				sd.AddressU       = s_textureAddress[(_flags&MAX_SAMPLER_U_MASK)>>MAX_SAMPLER_U_SHIFT];
				sd.AddressV       = s_textureAddress[(_flags&MAX_SAMPLER_V_MASK)>>MAX_SAMPLER_V_SHIFT];
				sd.AddressW       = s_textureAddress[(_flags&MAX_SAMPLER_W_MASK)>>MAX_SAMPLER_W_SHIFT];
				sd.MipLODBias     = float(MAX_CONFIG_MIP_LOD_BIAS);
		...
		ID3D11ShaderResourceView* getCachedSrv(TextureHandle _handle, uint8_t _mip, bool _compute = false, bool _stencil = false)
		{
			...
			{
				const TextureD3D11& texture = m_textures[_handle.idx];
				const uint32_t msaaQuality = bx::uint32_satsub( (texture.m_flags&MAX_TEXTURE_RT_MSAA_MASK)>>MAX_TEXTURE_RT_MSAA_SHIFT, 1);
				const DXGI_SAMPLE_DESC& msaa = s_msaa[msaaQuality];
				const bool msaaSample = 1 < msaa.Count && 0 != (texture.m_flags&MAX_TEXTURE_MSAA_SAMPLE);
		...
		void clearQuad(ClearQuad& _clearQuad, const Rect& _rect, const Clear& _clear, const float _palette[][4])
		{
			...
			else
			{
				...
				uint64_t state = 0;
				state |= _clear.m_flags & MAX_CLEAR_COLOR ? MAX_STATE_WRITE_RGB|MAX_STATE_WRITE_A         : 0;
				state |= _clear.m_flags & MAX_CLEAR_DEPTH ? MAX_STATE_DEPTH_TEST_ALWAYS|MAX_STATE_WRITE_Z : 0;

				uint64_t stencil = 0;
				stencil |= _clear.m_flags & MAX_CLEAR_STENCIL ? 0
					| MAX_STENCIL_TEST_ALWAYS
					| MAX_STENCIL_FUNC_REF(_clear.m_stencil)
					| MAX_STENCIL_FUNC_RMASK(0xff)
					| MAX_STENCIL_OP_FAIL_S_REPLACE
					| MAX_STENCIL_OP_FAIL_Z_REPLACE
					| MAX_STENCIL_OP_PASS_Z_REPLACE
					: 0
					;
		...
		IndexBufferD3D11 m_indexBuffers[MAX_CONFIG_MAX_INDEX_BUFFERS];
		VertexBufferD3D11 m_vertexBuffers[MAX_CONFIG_MAX_VERTEX_BUFFERS];
		ShaderD3D11 m_shaders[MAX_CONFIG_MAX_SHADERS];
		ProgramD3D11 m_program[MAX_CONFIG_MAX_PROGRAMS];
		TextureD3D11 m_textures[MAX_CONFIG_MAX_TEXTURES];
		VertexLayout m_vertexLayouts[MAX_CONFIG_MAX_VERTEX_LAYOUTS];
		FrameBufferD3D11 m_frameBuffers[MAX_CONFIG_MAX_FRAME_BUFFERS];
	...
	{	//  MAX_BUFFER_COMPUTE_TYPE_INT,  MAX_BUFFER_COMPUTE_TYPE_UINT,  MAX_BUFFER_COMPUTE_TYPE_FLOAT
		{ { DXGI_FORMAT_UNKNOWN,           DXGI_FORMAT_UNKNOWN,            DXGI_FORMAT_UNKNOWN            },  0 }, // ignored
		{ { DXGI_FORMAT_R8_SINT,           DXGI_FORMAT_R8_UINT,            DXGI_FORMAT_UNKNOWN            },  1 }, // MAX_BUFFER_COMPUTE_FORMAT_8X1
		{ { DXGI_FORMAT_R8G8_SINT,         DXGI_FORMAT_R8G8_UINT,          DXGI_FORMAT_UNKNOWN            },  2 }, // MAX_BUFFER_COMPUTE_FORMAT_8X2
		{ { DXGI_FORMAT_R8G8B8A8_SINT,     DXGI_FORMAT_R8G8B8A8_UINT,      DXGI_FORMAT_UNKNOWN            },  4 }, // MAX_BUFFER_COMPUTE_FORMAT_8X4
		{ { DXGI_FORMAT_R16_SINT,          DXGI_FORMAT_R16_UINT,           DXGI_FORMAT_R16_FLOAT          },  2 }, // MAX_BUFFER_COMPUTE_FORMAT_16X1
		{ { DXGI_FORMAT_R16G16_SINT,       DXGI_FORMAT_R16G16_UINT,        DXGI_FORMAT_R16G16_FLOAT       },  4 }, // MAX_BUFFER_COMPUTE_FORMAT_16X2
		{ { DXGI_FORMAT_R16G16B16A16_SINT, DXGI_FORMAT_R16G16B16A16_UINT,  DXGI_FORMAT_R16G16B16A16_FLOAT },  8 }, // MAX_BUFFER_COMPUTE_FORMAT_16X4
		{ { DXGI_FORMAT_R32_SINT,          DXGI_FORMAT_R32_UINT,           DXGI_FORMAT_R32_FLOAT          },  4 }, // MAX_BUFFER_COMPUTE_FORMAT_32X1
		{ { DXGI_FORMAT_R32G32_SINT,       DXGI_FORMAT_R32G32_UINT,        DXGI_FORMAT_R32G32_FLOAT       },  8 }, // MAX_BUFFER_COMPUTE_FORMAT_32X2
		{ { DXGI_FORMAT_R32G32B32A32_SINT, DXGI_FORMAT_R32G32B32A32_UINT,  DXGI_FORMAT_R32G32B32A32_FLOAT }, 16 }, // MAX_BUFFER_COMPUTE_FORMAT_32X4
	};
	...
	void BufferD3D11::create(uint32_t _size, void* _data, uint16_t _flags, uint16_t _stride, bool _vertex)
	{
		...
		const bool needUav = 0 != (_flags & (MAX_BUFFER_COMPUTE_WRITE|MAX_BUFFER_DRAW_INDIRECT) );
		const bool needSrv = 0 != (_flags & MAX_BUFFER_COMPUTE_READ);
		const bool drawIndirect = 0 != (_flags & MAX_BUFFER_DRAW_INDIRECT);
		...
		{
			uint32_t uavFormat = (_flags & MAX_BUFFER_COMPUTE_FORMAT_MASK) >> MAX_BUFFER_COMPUTE_FORMAT_SHIFT;
			if (0 == uavFormat)
			{
				if (_vertex)
				{
				...
				{
					if (0 == (_flags & MAX_BUFFER_INDEX32) )
					{
			...
			{
				const uint32_t uavType = bx::uint32_satsub( (_flags & MAX_BUFFER_COMPUTE_TYPE_MASK  ) >> MAX_BUFFER_COMPUTE_TYPE_SHIFT, 1);
		...
		{
#if MAX_CONFIG_RENDERER_DIRECT3D11_USE_STAGING_BUFFER
			desc.Usage          = D3D11_USAGE_DEFAULT;
			desc.CPUAccessFlags = 0;
	...
	void ShaderD3D11::create(const Memory* _mem)
	{
		...
		if (isShaderType(magic, 'F') )
		{
			...
			MAX_FATAL(NULL != m_ptr, max::Fatal::InvalidShader, "Failed to create fragment shader.");
		}
		else if (isShaderType(magic, 'V') )
		{
			...
			MAX_FATAL(NULL != m_ptr, max::Fatal::InvalidShader, "Failed to create vertex shader.");
		}
		else if (isShaderType(magic, 'C') )
		{
			DX_CHECK(s_renderD3D11->m_device->CreateComputeShader(code, shaderSize, NULL, &m_computeShader) );
			MAX_FATAL(NULL != m_ptr, max::Fatal::InvalidShader, "Failed to create compute shader.");
		}
	...
	void* TextureD3D11::create(const Memory* _mem, uint64_t _flags, uint8_t _skip)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			const bool writeOnly    = 0 != (m_flags&(MAX_TEXTURE_RT_WRITE_ONLY|MAX_TEXTURE_READ_BACK) );
			...
			const bool readBack     = 0 != (m_flags&MAX_TEXTURE_READ_BACK);
			const uint32_t msaaQuality = bx::uint32_satsub( (m_flags&MAX_TEXTURE_RT_MSAA_MASK)>>MAX_TEXTURE_RT_MSAA_SHIFT, 1);
			const DXGI_SAMPLE_DESC& msaa = s_msaa[msaaQuality];
	...
	void TextureD3D11::commit(uint8_t _stage, uint32_t _flags, const float _palette[][4])
	{
		...
		if (0 != (_flags & MAX_SAMPLER_SAMPLE_STENCIL) )
		{
		...
		const uint32_t flags = 0 == (MAX_SAMPLER_INTERNAL_DEFAULT & _flags)
			...
			: uint32_t(m_flags)
			;
		uint32_t index = (flags & MAX_SAMPLER_BORDER_COLOR_MASK) >> MAX_SAMPLER_BORDER_COLOR_SHIFT;
	...
	DXGI_FORMAT TextureD3D11::getSrvFormat() const
	{
		...
		return 0 != (m_flags&MAX_TEXTURE_SRGB)
	...
	void FrameBufferD3D11::create(uint16_t _denseIdx, void* _nwh, uint32_t _width, uint32_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat)
	{
		...
		MAX_FATAL(SUCCEEDED(hr), Fatal::UnableToInitialize, "Failed to create swap chain.");
	...
	void RendererContextD3D11::submit(Frame* _render, ClearQuad& _clearQuad, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		...
		MAX_D3D11_PROFILER_BEGIN_LITERAL("rendererSubmit", kColorView);
		...
		const uint64_t primType = _render->m_debug&MAX_DEBUG_WIREFRAME ? MAX_STATE_PT_LINES : 0;
		uint8_t primIndex = uint8_t(primType >> MAX_STATE_PT_SHIFT);
		...
		if (0 == (_render->m_debug&MAX_DEBUG_IFH) )
		{
			...
			setFrameBuffer(MAX_INVALID_HANDLE, true, false);
			...
			for (int32_t item = 0; item < numItems;)
			{
				...
				if (viewChanged)
				{
					...
					MAX_D3D11_PROFILER_END();
					setViewType(view, "  ");
					MAX_D3D11_PROFILER_BEGIN(view, kColorView);
					...
					if (MAX_CLEAR_NONE != (clr.m_flags & MAX_CLEAR_MASK) )
					{
				...
				if (isCompute)
				{
					if (!wasCompute)
					{
						...
						setViewType(view, "C");
						MAX_D3D11_PROFILER_END();
						MAX_D3D11_PROFILER_BEGIN(view, kColorCompute);
					...
					ID3D11UnorderedAccessView* uav[MAX_MAX_COMPUTE_BINDINGS] = {};
				...
				if (wasCompute)
				{
					setViewType(view, " ");
					MAX_D3D11_PROFILER_END();
					MAX_D3D11_PROFILER_BEGIN(view, kColorDraw);
				...
				changedFlags |= currentState.m_rgba != draw.m_rgba ? MAX_D3D11_BLEND_STATE_MASK : 0;
				currentState.m_stateFlags = newFlags;

				const uint64_t newStencil = draw.m_stencil;
				uint64_t changedStencil = currentState.m_stencil ^ draw.m_stencil;
				changedFlags |= 0 != changedStencil ? MAX_D3D11_DEPTH_STENCIL_MASK : 0;
				...
				if (resetState)
				{
					...
					setDepthStencilState(newFlags, packStencil(MAX_STENCIL_DEFAULT, MAX_STENCIL_DEFAULT) );
				...
				if (MAX_D3D11_DEPTH_STENCIL_MASK & changedFlags)
				{
					setDepthStencilState(newFlags, newStencil);
				}

				if (MAX_D3D11_BLEND_STATE_MASK & changedFlags)
				{
				...
				||  vertexStreamChanged)
				{
					...
					ID3D11Buffer* buffers[MAX_CONFIG_MAX_VERTEX_STREAMS];
					uint32_t strides[MAX_CONFIG_MAX_VERTEX_STREAMS];
					uint32_t offsets[MAX_CONFIG_MAX_VERTEX_STREAMS];
					...
					if (isValid(draw.m_indirectBuffer) )
					{
						...
						if (isValid(draw.m_indexBuffer) )
						{
							...
							multiDrawIndexedInstancedIndirect(
								  numDrawIndirect
								, ptr
								, draw.m_startIndirect * MAX_CONFIG_DRAW_INDIRECT_STRIDE
					...
					else
					{
						if (isValid(draw.m_indexBuffer) )
						{
							if (UINT32_MAX == draw.m_numIndices)
							{
								...
								const uint32_t indexSize = 0 == (ib.m_flags & MAX_BUFFER_INDEX32) ? 2 : 4;
		...
		if (UINT32_MAX != frameQueryIdx)
		{
			...
			const TimerQueryD3D11::Result& result = m_gpuTimer.m_result[MAX_CONFIG_MAX_VIEWS];
		...
		{
			MAX_D3D11_PROFILER_BEGIN_LITERAL("debugstats", kColorFrame);
...
//				tvm.printf(10, pos++, 0x8b, " Uniform size: %7d, Max: %7d ", _render->m_uniformEnd, _render->m_uniformMax);
		...
		{
			MAX_D3D11_PROFILER_BEGIN_LITERAL("debugtext", kColorFrame);
