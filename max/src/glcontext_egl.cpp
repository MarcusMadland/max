...
#undef EGL_IMPORT_FUNC
	...
	void* eglOpen()
	{
		...
		MAX_FATAL(NULL != handle, Fatal::UnableToInitialize, "Failed to load libEGL dynamic library.");

#define EGL_IMPORT_FUNC(_proto, _func)         \
	_func = (_proto)bx::dlsym(handle, #_func); \
	BX_TRACE("%p " #_func, _func);             \
	MAX_FATAL(NULL != _func, Fatal::UnableToInitialize, "Failed get " #_func ".")
EGL_IMPORT
#undef EGL_IMPORT_FUNC
...
#	include "glimports.h"
	...
	struct SwapChainGL
	{
			...
			, m_display(_display)
		{
			...
			MAX_FATAL(m_surface != EGL_NO_SURFACE, Fatal::UnableToInitialize, "Failed to create surface.");
...
#	endif // BX_PLATFORM_WINDOWS
			...
			MAX_FATAL(m_display != EGL_NO_DISPLAY, Fatal::UnableToInitialize, "Failed to create display %p", m_display);

			EGLint major = 0;
			EGLint minor = 0;
			EGLBoolean success = eglInitialize(m_display, &major, &minor);
			MAX_FATAL(success && major >= 1 && minor >= 3, Fatal::UnableToInitialize, "Failed to initialize %d.%d", major, minor);
			...
			if (BX_ENABLED(MAX_CONFIG_RENDERER_OPENGL) )
			{
				...
				MAX_FATAL(ok, Fatal::UnableToInitialize, "Could not set API! error: %d", eglGetError());
			}

			const bool hasEglAndroidRecordable = !bx::findIdentifierMatch(extensions, "EGL_ANDROID_recordable").isEmpty();

			const uint32_t glVersion = !!MAX_CONFIG_RENDERER_OPENGL
				? MAX_CONFIG_RENDERER_OPENGL
				: MAX_CONFIG_RENDERER_OPENGLES
				;

#if BX_PLATFORM_ANDROID
			const uint32_t msaa = (_flags&MAX_RESET_MSAA_MASK)>>MAX_RESET_MSAA_SHIFT;
			...
			EGLint attrs[] =
			{
				EGL_RENDERABLE_TYPE, !!MAX_CONFIG_RENDERER_OPENGL
					? EGL_OPENGL_BIT
...
#	endif // BX_PLATFORM_
			...
			MAX_FATAL(success, Fatal::UnableToInitialize, "eglChooseConfig");
...
#	else
				if (hasEglKhrCreateContext)
				{
					...
					flags |= MAX_CONFIG_DEBUG && hasEglKhrNoError ? 0
...
#	endif // BX_PLATFORM_RPI
			...
			MAX_FATAL(m_context != EGL_NO_CONTEXT, Fatal::UnableToInitialize, "Failed to create context.");

			success = eglMakeCurrent(m_display, m_surface, m_surface, m_context);
			MAX_FATAL(success, Fatal::UnableToInitialize, "Failed to set context.");
			m_current = NULL;
...
#		define GL_EXTENSION(_optional, _proto, _func, _import)                           \
			{                                                                            \
				...
				{                                                                        \
					...
					MAX_FATAL(_optional || NULL != _func                                \
