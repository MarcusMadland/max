...
namespace max
{
	...
	void* loadRenderDoc()
	{
		...
		if (NULL != renderDocDll)
		{
			...
			{
				s_renderDoc->SetCaptureFilePathTemplate(MAX_CONFIG_RENDERDOC_LOG_FILEPATH);

				s_renderDoc->SetFocusToggleKeys(NULL, 0);

				RENDERDOC_InputButton captureKeys[] = MAX_CONFIG_RENDERDOC_CAPTURE_KEYS;
	...
	void unloadRenderDoc(void* _renderdocdll)
	{
		if (NULL != _renderdocdll)
		{
			...
			// https://github.com/bkaradzic/max/issues/1192
			//
