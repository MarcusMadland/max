...
namespace max { namespace vk
{
	static char s_viewName[MAX_CONFIG_MAX_VIEWS][MAX_CONFIG_MAX_VIEW_NAME];

	inline void setViewType(ViewId _view, const bx::StringView _str)
	{
		if (BX_ENABLED(MAX_CONFIG_DEBUG_ANNOTATION || MAX_CONFIG_PROFILER) )
		{
...
#undef $A
	...
	{
		{ VK_IMAGE_TYPE_2D, VK_IMAGE_USAGE_SAMPLED_BIT,                  0,                                   { MAX_CAPS_FORMAT_TEXTURE_2D,          MAX_CAPS_FORMAT_TEXTURE_2D_SRGB   } },
		{ VK_IMAGE_TYPE_3D, VK_IMAGE_USAGE_SAMPLED_BIT,                  0,                                   { MAX_CAPS_FORMAT_TEXTURE_3D,          MAX_CAPS_FORMAT_TEXTURE_3D_SRGB   } },
		{ VK_IMAGE_TYPE_2D, VK_IMAGE_USAGE_SAMPLED_BIT,                  VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, { MAX_CAPS_FORMAT_TEXTURE_CUBE,        MAX_CAPS_FORMAT_TEXTURE_CUBE_SRGB } },
		{ VK_IMAGE_TYPE_2D, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,         0,                                   { MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER, 0                                  } },
		{ VK_IMAGE_TYPE_2D, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 0,                                   { MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER, 0                                  } },
		{ VK_IMAGE_TYPE_2D, VK_IMAGE_USAGE_STORAGE_BIT,                  0,                                   { MAX_CAPS_FORMAT_TEXTURE_IMAGE_READ,  0                                  } },
		{ VK_IMAGE_TYPE_2D, VK_IMAGE_USAGE_STORAGE_BIT,                  0,                                   { MAX_CAPS_FORMAT_TEXTURE_IMAGE_WRITE, 0                                  } },
	};
	...
	static Extension s_extension[] =
	{
		...
		{ "VK_EXT_debug_utils",                     1, false, false, MAX_CONFIG_DEBUG_OBJECT_NAME || MAX_CONFIG_DEBUG_ANNOTATION, Layer::Count },
	...
	bool updateExtension(const char* _name, uint32_t _version, bool _instanceExt, Extension _extensions[Extension::Count])
	{
		...
		if (BX_ENABLED(MAX_CONFIG_RENDERER_USE_EXTENSIONS) )
		{
...
#undef VKENUM
	...
	{
		if (BX_ENABLED(MAX_CONFIG_DEBUG_OBJECT_NAME)
...
#define MAX_DESCRIPTOR_SETS (1024 * MAX_CONFIG_MAX_FRAME_LATENCY)
...
#undef VK_IMPORT_FUNC
...

			{
				...
				s_extension[Extension::EXT_shader_viewport_index_layer].m_initialize = !!(_init.capabilities & MAX_CAPS_VIEWPORT_LAYER_ARRAY);
				s_extension[Extension::EXT_conservative_rasterization ].m_initialize = !!(_init.capabilities & MAX_CAPS_CONSERVATIVE_RASTER );
				s_extension[Extension::KHR_draw_indirect_count        ].m_initialize = !!(_init.capabilities & MAX_CAPS_DRAW_INDIRECT_COUNT );
				...
				appInfo.pNext = NULL;
				appInfo.pApplicationName   = "max";
				appInfo.applicationVersion = MAX_API_VERSION;
				appInfo.pEngineName        = "max";
				appInfo.engineVersion      = MAX_API_VERSION;
...
#undef VK_IMPORT_INSTANCE_FUNC
...

			{
				...
				for (uint32_t ii = 0; ii < numPhysicalDevices; ++ii)
				{
					...
					{
						pdp.vendorID = MAX_PCI_ID_SOFTWARE_RASTERIZER;
					}
					...
					if ( (MAX_PCI_ID_NONE != g_caps.vendorId ||            0 != g_caps.deviceId)
					&&   (MAX_PCI_ID_NONE == g_caps.vendorId || pdp.vendorID == g_caps.vendorId)
				...
				m_deviceFeatures.imageCubeArray                    = supportedFeatures.imageCubeArray            && (_init.capabilities & MAX_CAPS_TEXTURE_CUBE_ARRAY);
				m_deviceFeatures.independentBlend                  = supportedFeatures.independentBlend          && (_init.capabilities & MAX_CAPS_BLEND_INDEPENDENT);
				m_deviceFeatures.multiDrawIndirect                 = supportedFeatures.multiDrawIndirect         && (_init.capabilities & MAX_CAPS_DRAW_INDIRECT);
				m_deviceFeatures.drawIndirectFirstInstance         = supportedFeatures.drawIndirectFirstInstance && (_init.capabilities & MAX_CAPS_DRAW_INDIRECT);
				...
				g_caps.supported |= ( 0
					...
					| (m_deviceFeatures.independentBlend ? MAX_CAPS_BLEND_INDEPENDENT : 0)
					| MAX_CAPS_COMPUTE
					| (indirectDrawSupport ? MAX_CAPS_DRAW_INDIRECT : 0)
					| MAX_CAPS_FRAGMENT_DEPTH
					| MAX_CAPS_IMAGE_RW
					| (m_deviceFeatures.fullDrawIndexUint32 ? MAX_CAPS_INDEX32 : 0)
					| MAX_CAPS_INSTANCING
					| MAX_CAPS_OCCLUSION_QUERY
					| (!headless ? MAX_CAPS_SWAP_CHAIN : 0)
					| MAX_CAPS_TEXTURE_2D_ARRAY
					| MAX_CAPS_TEXTURE_3D
					| MAX_CAPS_TEXTURE_BLIT
					| MAX_CAPS_TEXTURE_COMPARE_ALL
					| (m_deviceFeatures.imageCubeArray ? MAX_CAPS_TEXTURE_CUBE_ARRAY : 0)
					| MAX_CAPS_TEXTURE_READ_BACK
					| MAX_CAPS_VERTEX_ATTRIB_HALF
					| MAX_CAPS_VERTEX_ATTRIB_UINT10
					| MAX_CAPS_VERTEX_ID
					| (m_deviceFeatures.geometryShader ? MAX_CAPS_PRIMITIVE_ID : 0)
					);

				g_caps.supported |= 0
					| (s_extension[Extension::EXT_conservative_rasterization ].m_supported ? MAX_CAPS_CONSERVATIVE_RASTER  : 0)
					| (s_extension[Extension::EXT_shader_viewport_index_layer].m_supported ? MAX_CAPS_VIEWPORT_LAYER_ARRAY : 0)
					| (s_extension[Extension::KHR_draw_indirect_count        ].m_supported && indirectDrawSupport ? MAX_CAPS_DRAW_INDIRECT_COUNT : 0)
					;
				...
				g_caps.limits.maxFBAttachments   = bx::min<uint32_t>(maxAttachments, MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS);
				g_caps.limits.maxTextureSamplers = bx::min<uint32_t>(m_deviceProperties.limits.maxPerStageResources, MAX_CONFIG_MAX_TEXTURE_SAMPLERS);
				g_caps.limits.maxComputeBindings = bx::min<uint32_t>(m_deviceProperties.limits.maxPerStageResources, MAX_MAX_COMPUTE_BINDINGS);
				g_caps.limits.maxVertexStreams   = bx::min<uint32_t>(m_deviceProperties.limits.maxVertexInputBindings, MAX_CONFIG_MAX_VERTEX_STREAMS);
				...
				{
					uint16_t support = MAX_CAPS_FORMAT_TEXTURE_NONE;
								...
								if (VK_SUCCESS == result)
								{
									...
									if (it.usage & VK_IMAGE_USAGE_SAMPLED_BIT)
									{
										support |= 0
											...
											| (multisample ? MAX_CAPS_FORMAT_TEXTURE_MSAA : 0)
											;
									}
									...
									if (it.usage & (VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) )
									{
										support |= 0
											...
											| (multisample ? MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA : 0)
											;
									}
								}
							}
...
#undef VK_IMPORT_DEVICE_FUNC
...

			{
				...
				m_resolution.reset &= ~MAX_RESET_INTERNAL_FORCE;
				...
				if (!headless)
				{
					...
					m_windows[0] = MAX_INVALID_HANDLE;
					m_numWindows++;
...

			{
				...
				{
					{ VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,          MAX_DESCRIPTOR_SETS * MAX_CONFIG_MAX_TEXTURE_SAMPLERS },
					{ VK_DESCRIPTOR_TYPE_SAMPLER,                MAX_DESCRIPTOR_SETS * MAX_CONFIG_MAX_TEXTURE_SAMPLERS },
					{ VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, MAX_DESCRIPTOR_SETS * 2                                },
					{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,         MAX_DESCRIPTOR_SETS * MAX_CONFIG_MAX_TEXTURE_SAMPLERS },
					{ VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,          MAX_DESCRIPTOR_SETS * MAX_CONFIG_MAX_TEXTURE_SAMPLERS },
				};
...

			{
				...
				const uint32_t count = MAX_CONFIG_MAX_DRAW_CALLS;
				...
				for (uint32_t ii = 0; ii < m_numFramesInFlight; ++ii)
				{
					...
					m_scratchStagingBuffer[ii].createStaging(MAX_CONFIG_PER_FRAME_SCRATCH_STAGING_BUFFER_SIZE);
				}
			}
			...
			{
				bx::snprintf(s_viewName[ii], MAX_CONFIG_MAX_VIEW_NAME_RESERVED+1, "%3d   ", ii);
			}
		...
		{
			return MAX_RENDERER_VULKAN_NAME;
		}
		...
		void destroyFrameBuffer(FrameBufferHandle _handle) override
		{
			...
			{
				setFrameBuffer(MAX_INVALID_HANDLE, false);
			}
		...
		{
			bx::strCopy(&s_viewName[_id][MAX_CONFIG_MAX_VIEW_NAME_RESERVED]
				, BX_COUNTOF(s_viewName[0]) - MAX_CONFIG_MAX_VIEW_NAME_RESERVED
				, _name
				);
		}
		...
		{
			if (BX_ENABLED(MAX_CONFIG_DEBUG_ANNOTATION) )
			{
				BX_UNUSED(_len);
		...
		void blitSetup(TextVideoMemBlitter& _blitter) override
		{
			...
			bind.m_bind[0].m_samplerFlags = (uint32_t)(texture.m_flags & MAX_SAMPLER_BITS_MASK);
		...
		void postReset()
		{
			...
			if (m_resolution.reset & MAX_RESET_CAPTURE)
			{
		...
		{
			const bool suspended = !!(_resolution.reset & MAX_RESET_SUSPEND);

			float maxAnisotropy = 1.0f;
			if (!!(_resolution.reset & MAX_RESET_MAXANISOTROPY) )
			{
			...
			bool depthClamp = m_deviceFeatures.depthClamp && !!(_resolution.reset & MAX_RESET_DEPTH_CLAMP);
			...
			uint32_t flags = _resolution.reset & ~(0
				| MAX_RESET_SUSPEND
				| MAX_RESET_MAXANISOTROPY
				| MAX_RESET_DEPTH_CLAMP
				);
			...
			{
				flags &= ~MAX_RESET_INTERNAL_FORCE;
		...
		{
			MAX_PROFILER_SCOPE("Vk::setFrameBuffer()", kColorFrame);
			BX_ASSERT(false
				  ||  isValid(_fbh)
			...
			&&  m_fbh.idx != _fbh.idx)
			{
				...
				if (isValid(oldFrameBuffer.m_depth) )
				{
					...
					const bool writeOnly  = 0 != (texture.m_flags&MAX_TEXTURE_RT_WRITE_ONLY);
		...
		void setBlendState(VkPipelineColorBlendStateCreateInfo& _desc, uint64_t _state, uint32_t _rgba = 0)
		{
			...
			uint8_t writeMask = 0;
			writeMask |= (_state & MAX_STATE_WRITE_R) ? VK_COLOR_COMPONENT_R_BIT : 0;
			writeMask |= (_state & MAX_STATE_WRITE_G) ? VK_COLOR_COMPONENT_G_BIT : 0;
			writeMask |= (_state & MAX_STATE_WRITE_B) ? VK_COLOR_COMPONENT_B_BIT : 0;
			writeMask |= (_state & MAX_STATE_WRITE_A) ? VK_COLOR_COMPONENT_A_BIT : 0;

			bas->blendEnable = !!(MAX_STATE_BLEND_MASK & _state);

			{
				const uint32_t blend    = uint32_t( (_state & MAX_STATE_BLEND_MASK         ) >> MAX_STATE_BLEND_SHIFT);
				const uint32_t equation = uint32_t( (_state & MAX_STATE_BLEND_EQUATION_MASK) >> MAX_STATE_BLEND_EQUATION_SHIFT);
			...
			if (!!(MAX_STATE_BLEND_INDEPENDENT & _state)
		...
		{
			const uint32_t cull = (_state&MAX_STATE_CULL_MASK) >> MAX_STATE_CULL_SHIFT;
			...
			_desc.frontFace = (_state&MAX_STATE_FRONT_CCW) ? VK_FRONT_FACE_COUNTER_CLOCKWISE : VK_FRONT_FACE_CLOCKWISE;
		...
		void setConservativeRasterizerState(VkPipelineRasterizationConservativeStateCreateInfoEXT& _desc, uint64_t _state)
		{
			...
			_desc.pNext = NULL;
			_desc.flags = 0;
			_desc.conservativeRasterizationMode = (_state&MAX_STATE_CONSERVATIVE_RASTER)
		...
		void setLineRasterizerState(VkPipelineRasterizationLineStateCreateInfoEXT& _desc, uint64_t _state)
		{
			...
			_desc.pNext = NULL;
			_desc.lineRasterizationMode = (_state & MAX_STATE_LINEAA)
		...
		void setDepthStencilState(VkPipelineDepthStencilStateCreateInfo& _desc, uint64_t _state, uint64_t _stencil = 0)
		{
			...
			uint32_t func = (_state&MAX_STATE_DEPTH_TEST_MASK)>>MAX_STATE_DEPTH_TEST_SHIFT;

			_desc.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
			_desc.pNext = NULL;
			_desc.flags = 0;
			_desc.depthTestEnable  = 0 != func;
			_desc.depthWriteEnable = !!(MAX_STATE_WRITE_Z & _state);
			...
			uint32_t frontAndBack = bstencil != MAX_STENCIL_NONE && bstencil != fstencil;
			bstencil = frontAndBack ? bstencil : fstencil;

			_desc.front.failOp      = s_stencilOp[(fstencil & MAX_STENCIL_OP_FAIL_S_MASK) >> MAX_STENCIL_OP_FAIL_S_SHIFT];
			_desc.front.passOp      = s_stencilOp[(fstencil & MAX_STENCIL_OP_PASS_Z_MASK) >> MAX_STENCIL_OP_PASS_Z_SHIFT];
			_desc.front.depthFailOp = s_stencilOp[(fstencil & MAX_STENCIL_OP_FAIL_Z_MASK) >> MAX_STENCIL_OP_FAIL_Z_SHIFT];
			_desc.front.compareOp   = s_cmpFunc[(fstencil & MAX_STENCIL_TEST_MASK) >> MAX_STENCIL_TEST_SHIFT];
			_desc.front.compareMask = UINT32_MAX;
			_desc.front.writeMask   = UINT32_MAX;
			_desc.front.reference   = 0;

			_desc.back.failOp       = s_stencilOp[(bstencil & MAX_STENCIL_OP_FAIL_S_MASK) >> MAX_STENCIL_OP_FAIL_S_SHIFT];
			_desc.back.passOp       = s_stencilOp[(bstencil & MAX_STENCIL_OP_PASS_Z_MASK) >> MAX_STENCIL_OP_PASS_Z_SHIFT];
			_desc.back.depthFailOp  = s_stencilOp[(bstencil & MAX_STENCIL_OP_FAIL_Z_MASK) >> MAX_STENCIL_OP_FAIL_Z_SHIFT];
			_desc.back.compareOp    = s_cmpFunc[(bstencil&MAX_STENCIL_TEST_MASK) >> MAX_STENCIL_TEST_SHIFT];
		...
		VkResult getRenderPass(uint8_t _num, const VkFormat* _formats, const VkImageAspectFlags* _aspects, const bool* _resolve, VkSampleCountFlagBits _samples, ::VkRenderPass* _renderPass)
		{
			...
			VkAttachmentDescription ad[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS * 2];
			...
			VkAttachmentReference colorAr[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
			VkAttachmentReference resolveAr[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
		...
		{
			VkFormat formats[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
			VkImageAspectFlags aspects[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
		...
		{
			uint32_t index = ((_flags & MAX_SAMPLER_BORDER_COLOR_MASK) >> MAX_SAMPLER_BORDER_COLOR_SHIFT);
			index = bx::min<uint32_t>(MAX_CONFIG_MAX_COLOR_PALETTE - 1, index);

			_flags &= MAX_SAMPLER_BITS_MASK;
			_flags &= ~(m_deviceFeatures.samplerAnisotropy ? 0 : (MAX_SAMPLER_MIN_ANISOTROPIC | MAX_SAMPLER_MAG_ANISOTROPIC) );

			// Force both min+max anisotropic, can't be set individually.
			_flags |= 0 != (_flags & (MAX_SAMPLER_MIN_ANISOTROPIC|MAX_SAMPLER_MAG_ANISOTROPIC) )
					? MAX_SAMPLER_MIN_ANISOTROPIC|MAX_SAMPLER_MAG_ANISOTROPIC
					: 0
					;
			...
			const uint32_t cmpFunc = (_flags&MAX_SAMPLER_COMPARE_MASK)>>MAX_SAMPLER_COMPARE_SHIFT;

			const float maxLodBias = m_deviceProperties.limits.maxSamplerLodBias;
			const float lodBias = bx::clamp(float(MAX_CONFIG_MIP_LOD_BIAS), -maxLodBias, maxLodBias);

			VkSamplerCreateInfo sci;
			sci.sType            = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
			sci.pNext            = NULL;
			sci.flags            = 0;
			sci.magFilter        = _flags & MAX_SAMPLER_MAG_POINT ? VK_FILTER_NEAREST              : VK_FILTER_LINEAR;
			sci.minFilter        = _flags & MAX_SAMPLER_MIN_POINT ? VK_FILTER_NEAREST              : VK_FILTER_LINEAR;
			sci.mipmapMode       = _flags & MAX_SAMPLER_MIP_POINT ? VK_SAMPLER_MIPMAP_MODE_NEAREST : VK_SAMPLER_MIPMAP_MODE_LINEAR;
			sci.addressModeU     = s_textureAddress[(_flags&MAX_SAMPLER_U_MASK)>>MAX_SAMPLER_U_SHIFT];
			sci.addressModeV     = s_textureAddress[(_flags&MAX_SAMPLER_V_MASK)>>MAX_SAMPLER_V_SHIFT];
			sci.addressModeW     = s_textureAddress[(_flags&MAX_SAMPLER_W_MASK)>>MAX_SAMPLER_W_SHIFT];
			sci.mipLodBias       = lodBias;
			sci.anisotropyEnable = !!(_flags & (MAX_SAMPLER_MIN_ANISOTROPIC | MAX_SAMPLER_MAG_ANISOTROPIC) );
		...
		VkPipeline getPipeline(uint64_t _state, uint64_t _stencil, uint8_t _numStreams, const VertexLayout** _layouts, ProgramHandle _program, uint8_t _numInstanceData)
		{
			...
			_state &= 0
				| MAX_STATE_WRITE_MASK
				| MAX_STATE_DEPTH_TEST_MASK
				| MAX_STATE_BLEND_MASK
				| MAX_STATE_BLEND_EQUATION_MASK
				| (g_caps.supported & MAX_CAPS_BLEND_INDEPENDENT ? MAX_STATE_BLEND_INDEPENDENT : 0)
				| MAX_STATE_BLEND_ALPHA_TO_COVERAGE
				| MAX_STATE_CULL_MASK
				| MAX_STATE_FRONT_CCW
				| MAX_STATE_MSAA
				| (m_lineAASupport ? MAX_STATE_LINEAA : 0)
				| (g_caps.supported & MAX_CAPS_CONSERVATIVE_RASTER ? MAX_STATE_CONSERVATIVE_RASTER : 0)
				| MAX_STATE_PT_MASK
				;

			_stencil &= packStencil(~MAX_STENCIL_FUNC_REF_MASK, ~MAX_STENCIL_FUNC_REF_MASK);
			...
			VkPipelineColorBlendAttachmentState blendAttachmentState[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
			...
			inputAssemblyState.topology = s_primInfo[(_state&MAX_STATE_PT_MASK) >> MAX_STATE_PT_SHIFT].m_topology;
			...
			VkVertexInputBindingDescription  inputBinding[MAX_CONFIG_MAX_VERTEX_STREAMS + 1];
			VkVertexInputAttributeDescription inputAttrib[Attrib::Count + MAX_CONFIG_MAX_INSTANCE_DATA_COUNT];
			...
			multisampleState.alphaToCoverageEnable = !!(MAX_STATE_BLEND_ALPHA_TO_COVERAGE & _state);
		...
		VkDescriptorSet getDescriptorSet(const ProgramVK& program, const RenderBind& renderBind, const ScratchBufferVK& scratchBuffer, const float _palette[][4])
		{
			...
			VkDescriptorImageInfo  imageInfo[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
			VkDescriptorBufferInfo bufferInfo[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];

			constexpr uint32_t kMaxDescriptorSets = 2 * MAX_CONFIG_MAX_TEXTURE_SAMPLERS + 2;
			...
			for (uint32_t stage = 0; stage < MAX_CONFIG_MAX_TEXTURE_SAMPLERS; ++stage)
			{
					...
					case Binding::Texture:
						{
							...
							const uint32_t samplerFlags = 0 == (MAX_SAMPLER_INTERNAL_DEFAULT & bind.m_samplerFlags)
								? bind.m_samplerFlags
								: (uint32_t)texture.m_flags
								;
							const bool sampleStencil = !!(samplerFlags & MAX_SAMPLER_SAMPLE_STENCIL);
		...
		void clearQuad(const Rect& _rect, const Clear& _clear, const float _palette[][4])
		{
			...
			uint32_t numMrt;
			max::TextureFormat::Enum mrtFormat[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS];
			...
			if (NULL == fb.m_nwh)
			{
				...
				{
					mrtFormat[ii] = max::TextureFormat::Enum(m_textures[fb.m_texture[ii].idx].m_requestedFormat);
				}
			...
			VkClearAttachment attachments[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS + 1];
			uint32_t mrt = 0;

			if (MAX_CLEAR_COLOR & _clear.m_flags)
			{
				for (uint32_t ii = 0; ii < numMrt; ++ii)
				{
					...
					if (MAX_CLEAR_COLOR_USE_PALETTE & _clear.m_flags)
					{
						const uint8_t index = bx::min<uint8_t>(MAX_CONFIG_MAX_COLOR_PALETTE-1, _clear.m_index[ii]);
			...
			depthAspectMask &= 0
				| (_clear.m_flags & MAX_CLEAR_DEPTH   ? VK_IMAGE_ASPECT_DEPTH_BIT   : 0)
				| (_clear.m_flags & MAX_CLEAR_STENCIL ? VK_IMAGE_ASPECT_STENCIL_BIT : 0)
				;
		...
		{
			MAX_PROFILER_SCOPE("RendererContextVK::allocateMemory", kColorResource);
		...
		{
			MAX_PROFILER_SCOPE("createHostBuffer", kColorResource);
			VkResult result = VK_SUCCESS;
			...
			if (_data != NULL)
			{
				MAX_PROFILER_SCOPE("map and copy data", kColorResource);
				void* dst;
		...
		{
			MAX_PROFILER_SCOPE("allocFromScratchStagingBuffer", kColorResource);

			StagingBufferVK result;
			ScratchBufferVK &scratch = m_scratchStagingBuffer[m_cmd.m_currentFrameInFlight];

			if (_size <= MAX_CONFIG_MAX_STAGING_SIZE_FOR_SCRACH_BUFFER)
			{
				...
				if (scratchOffset != UINT32_MAX)
				{
					...
					if (_data != NULL)
					{
						MAX_PROFILER_SCOPE("copy to scratch", kColorResource);
		...
		FrameBufferHandle m_windows[MAX_CONFIG_MAX_FRAME_BUFFERS];
		int64_t m_presentElapsed;

		ScratchBufferVK m_scratchBuffer[MAX_CONFIG_MAX_FRAME_LATENCY];
		ScratchBufferVK m_scratchStagingBuffer[MAX_CONFIG_MAX_FRAME_LATENCY];
		...
		IndexBufferVK  m_indexBuffers[MAX_CONFIG_MAX_INDEX_BUFFERS];
		VertexBufferVK m_vertexBuffers[MAX_CONFIG_MAX_VERTEX_BUFFERS];
		ShaderVK       m_shaders[MAX_CONFIG_MAX_SHADERS];
		ProgramVK      m_program[MAX_CONFIG_MAX_PROGRAMS];
		TextureVK      m_textures[MAX_CONFIG_MAX_TEXTURES];
		VertexLayout   m_vertexLayouts[MAX_CONFIG_MAX_VERTEX_LAYOUTS];
		FrameBufferVK  m_frameBuffers[MAX_CONFIG_MAX_FRAME_BUFFERS];

		void* m_uniforms[MAX_CONFIG_MAX_UNIFORMS];
...
#define VK_DESTROY_FUNC(_name)                                                          \
	...
	{                                                                                   \
		...
		{                                                                               \
			MAX_PROFILER_SCOPE("vkDestroy" #_name, kColorResource);                    \
...
#undef VK_DESTROY_FUNC
	...
	void vkDestroy(VkDeviceMemory& _obj)
	{
		if (VK_NULL_HANDLE != _obj)
		{
			MAX_PROFILER_SCOPE("vkFreeMemory", kColorResource);
	...
	void vkDestroy(VkSurfaceKHR& _obj)
	{
		if (VK_NULL_HANDLE != _obj)
		{
			MAX_PROFILER_SCOPE("vkDestroySurfaceKHR", kColorResource);
	...
	void vkDestroy(VkDescriptorSet& _obj)
	{
		if (VK_NULL_HANDLE != _obj)
		{
			MAX_PROFILER_SCOPE("vkFreeDescriptorSets", kColorResource);
	...
	void BufferVK::create(VkCommandBuffer _commandBuffer, uint32_t _size, void* _data, uint16_t _flags, bool _vertex, uint32_t _stride)
	{
		...
		const bool storage  = m_flags & MAX_BUFFER_COMPUTE_READ_WRITE;
		const bool indirect = m_flags & MAX_BUFFER_DRAW_INDIRECT;
	...
	{
		MAX_PROFILER_SCOPE("BufferVK::update", kColorFrame);
		BX_UNUSED(_discard);
	...
	void ShaderVK::create(const Memory* _mem)
	{
		...
		{
			m_bindInfo[ii].uniformHandle  = MAX_INVALID_HANDLE;
...
//		disassemble(bx::getDebugOut(), m_code->data, m_code->size);
	...
	{
		MAX_PROFILER_SCOPE("TimerQueryVK::init", kColorFrame);
		VkResult result = VK_SUCCESS;
	...
	{
		MAX_PROFILER_SCOPE("TimerQueryVK::begin", kColorFrame);
	...
	{
		MAX_PROFILER_SCOPE("TimerQueryVK::end", kColorFrame);
	...
	{
		MAX_PROFILER_SCOPE("OcclusionQueryVK::init", kColorFrame);
		VkResult result = VK_SUCCESS;
	...
	{
		MAX_PROFILER_SCOPE("OcclusionQueryVK::shutdown", kColorFrame);
		m_control.reserve(1);
	...
	{
		MAX_PROFILER_SCOPE("OcclusionQueryVK::end", kColorFrame);
		const VkCommandBuffer commandBuffer = s_renderVK->m_commandBuffer;
	...
	{
		MAX_PROFILER_SCOPE("OcclusionQueryVK::flush", kColorFrame);
		if (0 < m_control.available() )
		{
	...
	{
		MAX_PROFILER_SCOPE("ReadbackVK::copyImageToBuffer", kColorFrame);
		uint32_t mipWidth  = bx::uint32_max(1, m_width  >> _mip);
	...
	{
		MAX_PROFILER_SCOPE("ReadbackVK::readback", kColorResource);
	...
	{
		MAX_PROFILER_SCOPE("TextureVK::create", kColorResource);
		BX_ASSERT(0 != (_flags & MAX_TEXTURE_RT_MASK), "");
		_flags |= MAX_TEXTURE_RT_WRITE_ONLY;
		...
		m_sampler = s_msaa[bx::uint32_satsub( (m_flags & MAX_TEXTURE_RT_MSAA_MASK) >> MAX_TEXTURE_RT_MSAA_SHIFT, 1)];
	...
	{
		MAX_PROFILER_SCOPE("TextureVK::createImages", kColorResource);
		VkResult result = VK_SUCCESS;
		...
		ici.usage                 = 0
			...
			| (m_flags & MAX_TEXTURE_RT_MASK
				? (m_aspectMask & (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT)
					? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT
					: VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT)
				: 0
				)
			| (m_flags & MAX_TEXTURE_COMPUTE_WRITE ? VK_IMAGE_USAGE_STORAGE_BIT : 0)
			;
		ici.format        = m_format;
		...
		m_sampledLayout = m_flags & MAX_TEXTURE_COMPUTE_WRITE
		...
		const bool needResolve = true
			...
			&& 0 == (m_flags & MAX_TEXTURE_MSAA_SAMPLE)
			&& 0 == (m_flags & MAX_TEXTURE_RT_WRITE_ONLY)
			;
	...
	void* TextureVK::create(VkCommandBuffer _commandBuffer, const Memory* _mem, uint64_t _flags, uint8_t _skip)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			m_format = bimg::isDepth(bimg::TextureFormat::Enum(m_textureFormat) )
				...
				: (m_flags & MAX_TEXTURE_SRGB) ? s_textureFormat[m_textureFormat].m_fmtSrgb : s_textureFormat[m_textureFormat].m_fmt
				;
			...
			const bool swizzle = TextureFormat::BGRA8 == m_textureFormat && 0 != (m_flags & MAX_TEXTURE_COMPUTE_WRITE);

			const bool writeOnly    = 0 != (m_flags & MAX_TEXTURE_RT_WRITE_ONLY);
			const bool computeWrite = 0 != (m_flags & MAX_TEXTURE_COMPUTE_WRITE);
			const bool renderTarget = 0 != (m_flags & MAX_TEXTURE_RT_MASK);
			const bool blit         = 0 != (m_flags & MAX_TEXTURE_BLIT_DST);
			...
			if (totalMemSize > 0)
			{
				...
				for (uint32_t ii = 0; ii < numSrd; ++ii)
				{
					...
					BX_ASSERT(
						  bx::uint32_mod((uint32_t)bufferCopyInfo[ii].bufferOffset, dstBlockInfo.blockSize) == 0
	...
	{
		MAX_PROFILER_SCOPE("TextureVK::destroy", kColorResource);
		m_readback.destroy();
	...
	{
		MAX_PROFILER_SCOPE("TextureVK::update", kColorResource);
	...
	{
		MAX_PROFILER_SCOPE("TextureVK::resolve", kColorResource);
		const bool needResolve = VK_NULL_HANDLE != m_singleMsaaImage;

		const bool needMipGen = true
			&& !needResolve
			&& 0 != (m_flags & MAX_TEXTURE_RT_MASK)
			&& 0 == (m_flags & MAX_TEXTURE_RT_WRITE_ONLY)
			&& (_mip + 1) < m_numMips
			&& 0 != (_resolve & MAX_RESOLVE_AUTO_GEN_MIPS)
			;
		...
		if (needMipGen)
		{
			MAX_PROFILER_SCOPE("TextureVK::resolve genMipmaps", kColorResource);
			...
			{
				MAX_PROFILER_SCOPE("mipmap", kColorResource);
				blit.srcOffsets[1] = { mipWidth, mipHeight, 1 };
	...
	{
		MAX_PROFILER_SCOPE("TextureVK::copyBufferToTexture", kColorResource);
		...
		for (uint32_t i = 0; i < _bufferImageCopyCount; ++i) {
			BX_ASSERT(
				  bx::uint32_mod((uint32_t)_bufferImageCopy[i].bufferOffset, blockInfo.blockSize) == 0
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::update", kColorFrame);
		const VkPhysicalDevice physicalDevice = s_renderVK->m_physicalDevice;

		m_lastImageRenderedSemaphore = VK_NULL_HANDLE;
		m_lastImageAcquiredSemaphore = VK_NULL_HANDLE;

		const uint64_t recreateSurfaceMask     = MAX_RESET_HIDPI;
		const uint64_t recreateSwapchainMask   = MAX_RESET_VSYNC | MAX_RESET_SRGB_BACKBUFFER;
		const uint64_t recreateAttachmentsMask = MAX_RESET_MSAA_MASK;
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::createSurface", kColorFrame);
		...
		{
#if     defined(WL_EGL_PLATFORM)
			if (g_platformData.type == max::NativeWindowHandleType::Wayland)
			{
...
#elif BX_PLATFORM_OSX
		{
			if (NULL != vkCreateMacOSSurfaceMVK)
			{
				...
				if (m_resolution.reset & MAX_RESET_HIDPI)
				{
...
#	error "Figure out KHR surface..."
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::createSwapchain", kColorFrame);
		VkResult result = VK_SUCCESS;
		...
		const bool srgb = !!(m_resolution.reset & MAX_RESET_SRGB_BACKBUFFER);
		...
		const bool vsync = !!(m_resolution.reset & MAX_RESET_VSYNC);
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::releaseSwapChain", kColorFrame);
		for (uint32_t ii = 0; ii < BX_COUNTOF(m_backBufferColorImageView); ++ii)
		{
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::createAttachments", kColorFrame);
		VkResult result = VK_SUCCESS;

		const uint32_t samplerIndex = (m_resolution.reset & MAX_RESET_MSAA_MASK) >> MAX_RESET_MSAA_SHIFT;
		const uint64_t textureFlags = (uint64_t(samplerIndex + 1) << MAX_TEXTURE_RT_MSAA_SHIFT) | MAX_TEXTURE_RT | MAX_TEXTURE_RT_WRITE_ONLY;
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::releaseAttachments", kColorFrame);
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::createFrameBuffer", kColorFrame);
		VkResult result = VK_SUCCESS;
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::findPresentMode", kColorFrame);
		VkResult result = VK_SUCCESS;
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::findSurfaceFormat", kColorFrame);
		VkResult result = VK_SUCCESS;
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVK::acquire", kColorFrame);
		if (VK_NULL_HANDLE == m_swapChain
		...
		if (!m_needPresent)
		{
			...
			VkResult result;
			{
				MAX_PROFILER_SCOPE("vkAcquireNextImageKHR", kColorFrame);
			...
			{
				MAX_PROFILER_SCOPE("vkWaitForFences", kColorFrame);
	...
	{
		MAX_PROFILER_SCOPE("SwapChainVk::present", kColorFrame);
		if (VK_NULL_HANDLE != m_swapChain
		&&  m_needPresent)
		{
			VkPresentInfoKHR pi;
			...
			VkResult result;
			{
				MAX_PROFILER_SCOPE("vkQueuePresentHKR", kColorFrame);
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::create", kColorFrame);
		m_numTh = _num;
	...
	VkResult FrameBufferVK::create(uint16_t _denseIdx, void* _nwh, uint32_t _width, uint32_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat)
	{
		...
		{
			resolution.reset &= ~MAX_RESET_MSAA_MASK;
		}
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::preReset", kColorFrame);
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::postReset", kColorFrame);
		if (m_numTh > 0)
		{
			const VkDevice device = s_renderVK->m_device;
			const VkAllocationCallbacks* allocatorCb = s_renderVK->m_allocatorCb;
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::update", kColorResource);
	...
	void FrameBufferVK::resolve()
	{
		...
		MAX_PROFILER_SCOPE("FrameBufferVK::resolve", kColorFrame);
		if (NULL == m_nwh)
		{
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::destroy", kColorFrame);
		preReset();
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::acquire", kColorFrame);
		bool acquired = true;
	...
	{
		MAX_PROFILER_SCOPE("FrameBufferVK::present", kColorFrame);
		m_swapChain.present();
		m_needPresent = false;
	}
	...
	VkResult CommandQueueVK::init(uint32_t _queueFamily, VkQueue _queue, uint32_t _numFramesInFlight)
	{
		...
		m_queue = _queue;
		m_numFramesInFlight = bx::clamp<uint32_t>(_numFramesInFlight, 1, MAX_CONFIG_MAX_FRAME_LATENCY);
	...
	{
		MAX_PROFILER_SCOPE("CommandQueueVK::alloc", kColorResource);
		VkResult result = VK_SUCCESS;
	...
	{
		MAX_PROFILER_SCOPE("CommandQueueVK::kick", kColorDraw);
		if (VK_NULL_HANDLE != m_activeCommandBuffer)
		{
			...
			{
				MAX_PROFILER_SCOPE("CommandQueueVK::kick vkQueueSubmit", kColorDraw);
				VK_CHECK(vkQueueSubmit(m_queue, 1, &si, m_completedFence) );
			}

			if (_wait)
			{
				MAX_PROFILER_SCOPE("CommandQueue::kick vkWaitForFences", kColorDraw);
	...
	{
		MAX_PROFILER_SCOPE("CommandQueueVK::finish", kColorDraw);
		if (_finishAll)
		{
			for (uint32_t ii = 0; ii < m_numFramesInFlight; ++ii)
			{
	...
	{
		MAX_PROFILER_SCOPE("CommandQueueVK::consume", kColorResource);
	...
	{
		MAX_PROFILER_SCOPE("RendererContextVK::submitBlit", kColorFrame);
		VkImageLayout srcLayouts[MAX_CONFIG_MAX_BLIT_ITEMS];
		VkImageLayout dstLayouts[MAX_CONFIG_MAX_BLIT_ITEMS];
	...
	void RendererContextVK::submit(Frame* _render, ClearQuad& _clearQuad, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		...
		MAX_VK_PROFILER_BEGIN_LITERAL("rendererSubmit", kColorView);
		...
		if (m_timerQuerySupport)
		{
			frameQueryIdx = m_gpuTimer.begin(MAX_CONFIG_MAX_VIEWS, _render->m_frameNum);
		}

		if (0 < _render->m_iboffset)
		{
			MAX_PROFILER_SCOPE("max/Update transient index buffer", kColorResource);
		...
		{
			MAX_PROFILER_SCOPE("max/Update transient vertex buffer", kColorResource);
		...
		currentState.clear();
		currentState.m_stateFlags = MAX_STATE_NONE;
		currentState.m_stencil    = packStencil(MAX_STENCIL_NONE, MAX_STENCIL_NONE);

		static ViewState viewState;
		viewState.reset(_render);

		bool wireframe = !!(_render->m_debug&MAX_DEBUG_WIREFRAME);
		setDebugWireframe(wireframe);

		ProgramHandle currentProgram = MAX_INVALID_HANDLE;
		...
		FrameBufferHandle fbh = { MAX_CONFIG_MAX_FRAME_BUFFERS };
		...
		const uint64_t f0 = MAX_STATE_BLEND_FACTOR;
		const uint64_t f1 = MAX_STATE_BLEND_INV_FACTOR;
		const uint64_t f2 = MAX_STATE_BLEND_FACTOR<<4;
		const uint64_t f3 = MAX_STATE_BLEND_INV_FACTOR<<4;
		...
		if (0 == (_render->m_debug&MAX_DEBUG_IFH) )
		{
			...
			for (int32_t item = 0; item < numItems;)
			{
				...
				if (viewChanged)
				{
					...
					view = key.m_view;
					currentProgram = MAX_INVALID_HANDLE;
					hasPredefined  = false;
					...
					MAX_VK_PROFILER_END();
					setViewType(view, " ");
					MAX_VK_PROFILER_BEGIN(view, kColorView);
				...
				if (isCompute)
				{
					if (!wasCompute)
					{
						...
						MAX_VK_PROFILER_END();
						setViewType(view, "C");
						MAX_VK_PROFILER_BEGIN(view, kColorCompute);
					}
					...
					if (isValid(compute.m_indirectBuffer) )
					{
						...
						uint32_t numDrawIndirect = UINT32_MAX == compute.m_numIndirect
							? vb.m_size/MAX_CONFIG_DRAW_INDIRECT_STRIDE
							: compute.m_numIndirect
							;

						uint32_t args = compute.m_startIndirect * MAX_CONFIG_DRAW_INDIRECT_STRIDE;
						for (uint32_t ii = 0; ii < numDrawIndirect; ++ii)
						{
							vkCmdDispatchIndirect(m_commandBuffer, vb.m_buffer, args);
							args += MAX_CONFIG_DRAW_INDIRECT_STRIDE;
						}
					}
					else
					{
				...
				const bool hasOcclusionQuery = 0 != (draw.m_stateFlags & MAX_STATE_INTERNAL_OCCLUSION_QUERY);
				{
					const bool occluded = true
						&& isValid(draw.m_occlusionQuery)
						&& !hasOcclusionQuery
						&& !isVisible(_render, draw.m_occlusionQuery, 0 != (draw.m_submitFlags & MAX_SUBMIT_INTERNAL_OCCLUSION_VISIBLE) )
						;
				...
				if (!beginRenderPass)
				{
					...
					MAX_VK_PROFILER_END();
					setViewType(view, " ");
					MAX_VK_PROFILER_BEGIN(view, kColorDraw);
				...
				if (0 != draw.m_streamMask)
				{
					...
					const VertexLayout* layouts[MAX_CONFIG_MAX_VERTEX_STREAMS];
					VkBuffer streamBuffers[MAX_CONFIG_MAX_VERTEX_STREAMS + 1];
					VkDeviceSize streamOffsets[MAX_CONFIG_MAX_VERTEX_STREAMS + 1];
					...
					&&  currentState.m_stencil != draw.m_stencil)
					{
						...
						const uint32_t ref = (fstencil&MAX_STENCIL_FUNC_REF_MASK)>>MAX_STENCIL_FUNC_REF_SHIFT;
					...
					||  currentProgram.idx != key.m_program.idx
					||  MAX_STATE_ALPHA_REF_MASK & changedFlags)
					{
						currentProgram = key.m_program;
						ProgramVK& program = m_program[currentProgram.idx];
					...
					if (hasPredefined)
					{
						uint32_t ref = (draw.m_stateFlags & MAX_STATE_ALPHA_REF_MASK) >> MAX_STATE_ALPHA_REF_SHIFT;
					...
					if (isValid(draw.m_indirectBuffer) )
					{
						...
						numDrawIndirect = UINT32_MAX == draw.m_numIndirect
							...
							: draw.m_numIndirect
							;
						bufferOffsetIndirect = draw.m_startIndirect * MAX_CONFIG_DRAW_INDIRECT_STRIDE;
					...
					const uint8_t primIndex = uint8_t((draw.m_stateFlags & MAX_STATE_PT_MASK) >> MAX_STATE_PT_SHIFT);
						...
						if (isValid(draw.m_indirectBuffer) )
						{
							if (isValid(draw.m_numIndirectBuffer) )
							{
								vkCmdDrawIndirectCountKHR(
									...
									, numDrawIndirect
									, MAX_CONFIG_DRAW_INDIRECT_STRIDE
									);
							}
							else
							{
			...
			submitBlit(bs, MAX_CONFIG_MAX_VIEWS);
		...
		if (UINT32_MAX != frameQueryIdx)
		{
			...
			const TimerQueryVK::Result& result = m_gpuTimer.m_result[MAX_CONFIG_MAX_VIEWS];
		...
		if (_render->m_debug & (MAX_DEBUG_IFH|MAX_DEBUG_STATS) )
		{
			MAX_VK_PROFILER_BEGIN_LITERAL("debugstats", kColorFrame);
			...
			if (timeEnd >= next)
			{
				...
				tvm.clear();
				uint16_t pos = 0;
				tvm.printf(0, pos++, MAX_CONFIG_DEBUG ? 0x8c : 0x8f
					, " %s / " BX_COMPILER_NAME
					  " / " BX_CPU_NAME
					  " / " BX_ARCH_NAME
					  " / " BX_PLATFORM_NAME
					  " / Version 1.%d.%d (commit: " MAX_REV_SHA1 ")"
					, getRendererName()
					, MAX_API_VERSION
					, MAX_REV_NUMBER
					);
				...
				const uint32_t msaa = (m_resolution.reset&MAX_RESET_MSAA_MASK)>>MAX_RESET_MSAA_SHIFT;
				tvm.printf(10, pos++, 0x8b, " Reset flags: [%c] vsync, [%c] MSAAx%d, [%c] MaxAnisotropy "
					, !!(m_resolution.reset&MAX_RESET_VSYNC) ? '\xfe' : ' '
					, 0 != msaa ? '\xfe' : ' '
					, 1<<msaa
					, !!(m_resolution.reset&MAX_RESET_MAXANISOTROPY) ? '\xfe' : ' '
					);
...
//				tvm.printf(10, pos++, 0x8b, " Uniform size: %7d, Max: %7d ", _render->m_uniformEnd, _render->m_uniformMax);
		...
		else if (_render->m_debug & MAX_DEBUG_TEXT)
		{
			MAX_VK_PROFILER_BEGIN_LITERAL("debugtext", kColorFrame);
		...
		{
			MAX_PROFILER_SCOPE("scratchBuffer::flush", kColorResource);
			scratchBuffer.flush();
		}

		{
			MAX_PROFILER_SCOPE("scratchStagingBuffer::flush", kColorResource);
