...
#if MAX_CONFIG_RENDERER_DIRECT3D12
...
#if MAX_CONFIG_DEBUG_ANNOTATION && (BX_PLATFORM_WINDOWS || BX_PLATFORM_WINRT)
PFN_PIX_GET_THREAD_INFO      max_PIXGetThreadInfo;
PFN_PIX_EVENTS_REPLACE_BLOCK max_PIXEventsReplaceBlock;
#endif // MAX_CONFIG_DEBUG_ANNOTATION && (BX_PLATFORM_WINDOWS || BX_PLATFORM_WINRT)
...
//			LUID luid;
			...
			{
				max_PIXGetThreadInfo      = (PFN_PIX_GET_THREAD_INFO     )bx::dlsym(m_winPixEvent, "PIXGetThreadInfo");
				max_PIXEventsReplaceBlock = (PFN_PIX_EVENTS_REPLACE_BLOCK)bx::dlsym(m_winPixEvent, "PIXEventsReplaceBlock");
			}

			if (NULL == max_PIXGetThreadInfo
			||  NULL == max_PIXEventsReplaceBlock)
			{
				max_PIXGetThreadInfo      = stubPIXGetThreadInfo;
				max_PIXEventsReplaceBlock = stubPIXEventsReplaceBlock;
			}
#endif // MAX_CONFIG_DEBUG_ANNOTATION && (BX_PLATFORM_WINDOWS || BX_PLATFORM_WINRT)
...
#	endif // !BX_PLATFORM_LINUX
			...
			if (NULL == g_platformData.backBuffer)
			{
				...
				if (NULL != m_scd.nwh)
				{
					...
					if (1 < m_scd.sampleDesc.Count)
					{
						...
						resourceDesc.MipLevels  = 1;
						resourceDesc.Format     = (m_resolution.reset & MAX_RESET_SRGB_BACKBUFFER)
...

			{
				...
				rtvDescHeap.NumDescriptors = 0
					...
					+ MAX_CONFIG_MAX_FRAME_BUFFERS*MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS
					;
				...
				{
					m_scratchBuffer[ii].create(MAX_CONFIG_MAX_DRAW_CALLS*1024
						, MAX_CONFIG_MAX_TEXTURES + MAX_CONFIG_MAX_SHADERS + MAX_CONFIG_MAX_DRAW_CALLS
						);
				}
				...
				{
					{ D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, MAX_CONFIG_MAX_TEXTURE_SAMPLERS, 0, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND },
					{ D3D12_DESCRIPTOR_RANGE_TYPE_SRV,     MAX_CONFIG_MAX_TEXTURE_SAMPLERS, 0, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND },
					{ D3D12_DESCRIPTOR_RANGE_TYPE_CBV,     1,                                0, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND },
					{ D3D12_DESCRIPTOR_RANGE_TYPE_UAV,     MAX_CONFIG_MAX_TEXTURE_SAMPLERS, 0, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND },
				};
				...
				g_caps.supported |= ( 0
					...
					| MAX_CAPS_COMPUTE
					| (m_options.ROVsSupported ? MAX_CAPS_FRAGMENT_ORDERING     : 0)
					| (m_directAccessSupport   ? MAX_CAPS_TEXTURE_DIRECT_ACCESS : 0)
					| (BX_ENABLED(BX_PLATFORM_WINDOWS) ? MAX_CAPS_SWAP_CHAIN : 0)
				...
				g_caps.limits.maxFBAttachments   = bx::min<uint8_t>(16, MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS);
				g_caps.limits.maxComputeBindings = bx::min(D3D12_UAV_SLOT_COUNT, MAX_MAX_COMPUTE_BINDINGS);
				...
				m_batch.create(4<<10);
#if !BX_PLATFORM_LINUX
				m_batch.setIndirectMode(MAX_PCI_ID_NVIDIA != m_dxgi.m_adapterDesc.VendorId && MAX_PCI_ID_MICROSOFT != m_dxgi.m_adapterDesc.VendorId);
#endif // !BX_PLATFORM_LINUX
...

				{
					...
					for (uint32_t ii = 0; ii < BX_COUNTOF(m_commandSignature); ++ii)
					{
						...
						D3D12_COMMAND_SIGNATURE_DESC commandSignatureDesc = { MAX_CONFIG_DRAW_INDIRECT_STRIDE, 1, &argDesc, 1 };
		...
		{
			return MAX_RENDERER_DIRECT3D12_NAME;
		}
		...
		void flip() override
		{
			if (!m_lost)
			{
				...
				HRESULT hr = S_OK;
				uint32_t syncInterval = !!(m_resolution.reset & MAX_RESET_VSYNC);
...
//				setDebugObjectName(m_shaders[_handle.idx].m_ptr, "%.*s", _len, _name);
		...
		void blitSetup(TextVideoMemBlitter& _blitter) override
		{
			...
			uint32_t samplerFlags[] = { uint32_t(texture.m_flags & MAX_SAMPLER_BITS_MASK) };
		...
		void postReset()
		{
			...
			if (NULL != m_swapChain)
			{
				for (uint32_t ii = 0, num = m_scd.bufferCount; ii < num; ++ii)
				{
					...
					rtvDesc.Format = (m_resolution.reset & MAX_RESET_SRGB_BACKBUFFER)
						? s_textureFormat[m_resolution.format].m_fmtSrgb
						: s_textureFormat[m_resolution.format].m_fmt;
...
#	endif // BX_PLATFORM_WINDOWS || BX_PLATFORM_WINRT
		...
		bool updateResolution(const Resolution& _resolution)
		{
			...
			bool depthClamp = !!(_resolution.reset & MAX_RESET_DEPTH_CLAMP);
			...
			|| (m_resolution.reset&maskFlags) != (_resolution.reset&maskFlags) )
			{
				...
				bool resize = true
					...
					&& (m_resolution.reset&MAX_RESET_MSAA_MASK) == (_resolution.reset&MAX_RESET_MSAA_MASK)
					;
		...
		D3D12_CPU_DESCRIPTOR_HANDLE getRtv(FrameBufferHandle _fbh, uint8_t _attachment)
		{
			...
			{
				rtvDescriptor.ptr + (BX_COUNTOF(m_backBufferColor) + _fbh.idx * MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS + _attachment) * rtvDescriptorSize
			};
			return result;
		}
		...
		{
			_desc.AlphaToCoverageEnable  = !!(MAX_STATE_BLEND_ALPHA_TO_COVERAGE & _state);
			_desc.IndependentBlendEnable = !!(MAX_STATE_BLEND_INDEPENDENT & _state);
			...
			uint8_t writeMask = 0;
			writeMask |= (_state & MAX_STATE_WRITE_R) ? D3D12_COLOR_WRITE_ENABLE_RED   : 0;
			writeMask |= (_state & MAX_STATE_WRITE_G) ? D3D12_COLOR_WRITE_ENABLE_GREEN : 0;
			writeMask |= (_state & MAX_STATE_WRITE_B) ? D3D12_COLOR_WRITE_ENABLE_BLUE  : 0;
			writeMask |= (_state & MAX_STATE_WRITE_A) ? D3D12_COLOR_WRITE_ENABLE_ALPHA : 0;
		...
		void setRasterizerState(D3D12_RASTERIZER_DESC& _desc, uint64_t _state, bool _wireframe = false)
		{
			...
			_desc.FrontCounterClockwise = !!(_state&MAX_STATE_FRONT_CCW);;
			_desc.DepthBias             = 0;
			_desc.DepthBiasClamp        = 0.0f;
			_desc.SlopeScaledDepthBias  = 0.0f;
			_desc.DepthClipEnable       = !m_depthClamp;
			_desc.MultisampleEnable     = !!(_state&MAX_STATE_MSAA);
			_desc.AntialiasedLineEnable = !!(_state&MAX_STATE_LINEAA);
			_desc.ForcedSampleCount     = 0;
			_desc.ConservativeRaster    = !!(_state&MAX_STATE_CONSERVATIVE_RASTER)
		...
		void setDepthStencilState(D3D12_DEPTH_STENCIL_DESC& _desc, uint64_t _state, uint64_t _stencil = 0)
		{
			...
			_desc.DepthWriteMask = !!(MAX_STATE_WRITE_Z & _state)
			...
			_desc.StencilReadMask  = (fstencil & MAX_STENCIL_FUNC_RMASK_MASK) >> MAX_STENCIL_FUNC_RMASK_SHIFT;
			_desc.StencilWriteMask = 0xff;

			_desc.FrontFace.StencilFailOp      = s_stencilOp[(fstencil & MAX_STENCIL_OP_FAIL_S_MASK) >> MAX_STENCIL_OP_FAIL_S_SHIFT];
			_desc.FrontFace.StencilDepthFailOp = s_stencilOp[(fstencil & MAX_STENCIL_OP_FAIL_Z_MASK) >> MAX_STENCIL_OP_FAIL_Z_SHIFT];
			_desc.FrontFace.StencilPassOp      = s_stencilOp[(fstencil & MAX_STENCIL_OP_PASS_Z_MASK) >> MAX_STENCIL_OP_PASS_Z_SHIFT];
			_desc.FrontFace.StencilFunc        = s_cmpFunc[(fstencil & MAX_STENCIL_TEST_MASK) >> MAX_STENCIL_TEST_SHIFT];

			_desc.BackFace.StencilFailOp       = s_stencilOp[(bstencil & MAX_STENCIL_OP_FAIL_S_MASK) >> MAX_STENCIL_OP_FAIL_S_SHIFT];
			_desc.BackFace.StencilDepthFailOp  = s_stencilOp[(bstencil & MAX_STENCIL_OP_FAIL_Z_MASK) >> MAX_STENCIL_OP_FAIL_Z_SHIFT];
			_desc.BackFace.StencilPassOp       = s_stencilOp[(bstencil & MAX_STENCIL_OP_PASS_Z_MASK) >> MAX_STENCIL_OP_PASS_Z_SHIFT];
			_desc.BackFace.StencilFunc         = s_cmpFunc[(bstencil&MAX_STENCIL_TEST_MASK) >> MAX_STENCIL_TEST_SHIFT];
		}
	...
	// 							bx::debugPrintfData(&temp->data[ii], temp->size-ii, "");
			...
			D3D12_INPUT_ELEMENT_DESC vertexElements[Attrib::Count + 1 + MAX_CONFIG_MAX_INSTANCE_DATA_COUNT];
			...
			uint8_t primIndex = uint8_t( (_state&MAX_STATE_PT_MASK) >> MAX_STATE_PT_SHIFT);
			...
			MAX_FATAL(NULL != pso, Fatal::InvalidShader, "Failed to create PSO!");
		...
		void clear(const Clear& _clear, const float _palette[][4], const D3D12_RECT* _rect = NULL, uint32_t _num = 0)
		{
			...
			else
			{
				...
				{
					uint32_t flags = 0;
					flags |= (_clear.m_flags & MAX_CLEAR_DEPTH  ) ? D3D12_CLEAR_FLAG_DEPTH   : 0;
					flags |= (_clear.m_flags & MAX_CLEAR_STENCIL) ? D3D12_CLEAR_FLAG_STENCIL : 0;
		...
		ID3D12Resource* m_backBufferColor[MAX_CONFIG_MAX_BACK_BUFFERS];
		uint64_t m_backBufferColorFence[MAX_CONFIG_MAX_BACK_BUFFERS];
		ID3D12Resource* m_backBufferDepthStencil;

		ScratchBufferD3D12 m_scratchBuffer[MAX_CONFIG_MAX_BACK_BUFFERS];
		...
		BufferD3D12 m_indexBuffers[MAX_CONFIG_MAX_INDEX_BUFFERS];
		VertexBufferD3D12 m_vertexBuffers[MAX_CONFIG_MAX_VERTEX_BUFFERS];
		ShaderD3D12 m_shaders[MAX_CONFIG_MAX_SHADERS];
		ProgramD3D12 m_program[MAX_CONFIG_MAX_PROGRAMS];
		TextureD3D12 m_textures[MAX_CONFIG_MAX_TEXTURES];
		VertexLayout m_vertexLayouts[MAX_CONFIG_MAX_VERTEX_LAYOUTS];
		FrameBufferD3D12 m_frameBuffers[MAX_CONFIG_MAX_FRAME_BUFFERS];
...
//		m_gpuHandle.ptr += m_incrementSize;
	...
	uint16_t DescriptorAllocatorD3D12::alloc(const uint32_t* _flags, uint32_t _num, const float _palette[][4])
	{
		...
		for (uint32_t ii = 0; ii < _num; ++ii)
		{
			...
			const uint8_t  minFilter = s_textureFilter[0][(flags&MAX_SAMPLER_MIN_MASK)>>MAX_SAMPLER_MIN_SHIFT];
			const uint8_t  magFilter = s_textureFilter[1][(flags&MAX_SAMPLER_MAG_MASK)>>MAX_SAMPLER_MAG_SHIFT];
			const uint8_t  mipFilter = s_textureFilter[2][(flags&MAX_SAMPLER_MIP_MASK)>>MAX_SAMPLER_MIP_SHIFT];
			...
			sd.AddressU = s_textureAddress[(flags&MAX_SAMPLER_U_MASK)>>MAX_SAMPLER_U_SHIFT];
			sd.AddressV = s_textureAddress[(flags&MAX_SAMPLER_V_MASK)>>MAX_SAMPLER_V_SHIFT];
			sd.AddressW = s_textureAddress[(flags&MAX_SAMPLER_W_MASK)>>MAX_SAMPLER_W_SHIFT];
	...
	uint32_t BatchD3D12::draw(ID3D12GraphicsCommandList* _commandList, D3D12_GPU_VIRTUAL_ADDRESS _cbv, const RenderDraw& _draw)
	{
		if (isValid(_draw.m_indirectBuffer) )
		{
			...
			D3D12_VERTEX_BUFFER_VIEW vbvs[MAX_CONFIG_MAX_VERTEX_STREAMS+1];
			...
			const uint32_t numDrawIndirect = UINT32_MAX == _draw.m_numIndirect
				? indirect.m_size/MAX_CONFIG_DRAW_INDIRECT_STRIDE
				: _draw.m_numIndirect
				;
			...
			if (isValid(_draw.m_indexBuffer) )
			{
				...
				_commandList->ExecuteIndirect(
					...
					, numDrawIndirect
					, indirect.m_ptr
					, _draw.m_startIndirect * MAX_CONFIG_DRAW_INDIRECT_STRIDE
					, numIndirect
					, numOffsetIndirect
					);
			}
				...
				if (Draw == _type)
				{
					...
					for (uint32_t ii = 0; ii < num; ++ii)
					{
						...
						if (0 != bx::memCmp(m_current.vbv, cmd.vbv, sizeof(cmd.vbv) ) )
						{
							...
							_commandList->IASetVertexBuffers(0
								, MAX_CONFIG_MAX_VERTEX_STREAMS+1
								, cmd.vbv
								);
						}
	...
	void* TextureD3D12::create(const Memory* _mem, uint64_t _flags, uint8_t _skip)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			m_srvd.Format = (m_flags & MAX_TEXTURE_SRGB) ? tfi.m_fmtSrgb : tfi.m_fmtSrv;
...
//				&& !readBack
	...
	void FrameBufferD3D12::postReset()
	{
		if (m_numTh != 0)
		{
			...
			rtvDescriptor.ptr += (BX_COUNTOF(s_renderD3D12->m_backBufferColor) + fbhIdx * MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS) * rtvDescriptorSize;
...
// 							| (blockInfo.stencilBits > 0 ? D3D12_DSV_FLAG_READ_ONLY_STENCIL : D3D12_DSV_FLAG_NONE)
	...
	void RendererContextD3D12::submit(Frame* _render, ClearQuad& /*_clearQuad*/, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		...
		MAX_D3D12_PROFILER_BEGIN_LITERAL("rendererSubmit", kColorFrame);

		int64_t timeBegin = bx::getHPCounter();
		int64_t captureElapsed = 0;

		uint32_t frameQueryIdx = m_gpuTimer.begin(MAX_CONFIG_MAX_VIEWS, _render->m_frameNum);
...
// 		setDebugWireframe(wireframe);
		...
		{
			setFrameBuffer(MAX_INVALID_HANDLE, true);
			...
			for (int32_t item = 0; item < numItems;)
			{
				...
				if (viewChanged)
				{
					...
					MAX_D3D12_PROFILER_END();
					setViewType(view, "  ");
					MAX_D3D12_PROFILER_BEGIN(view, kColorView);
				...
				if (isCompute)
				{
					if (!wasCompute)
					{
						...
						setViewType(view, "C");
						MAX_D3D12_PROFILER_END();
						MAX_D3D12_PROFILER_BEGIN(view, kColorCompute);
					...
					if (currentBindHash != bindHash)
					{
						...
						{
							uint32_t numSet = 0;
							D3D12_GPU_DESCRIPTOR_HANDLE srvHandle[MAX_MAX_COMPUTE_BINDINGS] = {};
							uint32_t samplerFlags[MAX_MAX_COMPUTE_BINDINGS] = {};
							{
										...
										case Binding::Texture:
											{
												...
												samplerFlags[stage] = (0 == (MAX_SAMPLER_INTERNAL_DEFAULT & bind.m_samplerFlags)
													? bind.m_samplerFlags
													: texture.m_flags
													) & (MAX_SAMPLER_BITS_MASK | MAX_SAMPLER_BORDER_COLOR_MASK | MAX_SAMPLER_COMPARE_MASK)
													;
					...
					if (isValid(compute.m_indirectBuffer) )
					{
						...
						m_commandList->ExecuteIndirect(
							...
							, numDrawIndirect
							, indirect.m_ptr
							, compute.m_startIndirect * MAX_CONFIG_DRAW_INDIRECT_STRIDE
							, NULL
							, 0
							);
					}
				...
				if (wasCompute)
				{
					...
					setViewType(view, " ");
					MAX_D3D12_PROFILER_END();
					MAX_D3D12_PROFILER_BEGIN(view, kColorDraw);
				...
				if (0 != draw.m_streamMask)
				{
					...
					if (0 != (MAX_STATE_PT_MASK & changedFlags) )
					{
						primIndex = uint8_t( (newFlags&MAX_STATE_PT_MASK)>>MAX_STATE_PT_SHIFT);
					}

					const uint64_t newStencil = draw.m_stencil;
					uint64_t changedStencil = (currentState.m_stencil ^ draw.m_stencil) & MAX_STENCIL_FUNC_REF_MASK;
					...
					|| (0 != (MAX_STATE_PT_MASK & changedFlags)
					...
					if (currentBindHash != bindHash)
					{
						...
						{
							uint32_t numSet = 0;
							D3D12_GPU_DESCRIPTOR_HANDLE srvHandle[MAX_CONFIG_MAX_TEXTURE_SAMPLERS] = {};
							uint32_t samplerFlags[MAX_CONFIG_MAX_TEXTURE_SAMPLERS] = {};
							{
							...
							if (0 != numSet)
							{
								Bind bind;
								bind.m_srvHandle       = srvHandle[0];
								bind.m_samplerStateIdx = getSamplerState(samplerFlags, MAX_CONFIG_MAX_TEXTURE_SAMPLERS, _render->m_colorPalette);
								bindCached = bindLru.add(bindHash, bind, 0);
							}
						}
...
//				captureElapsed += bx::getHPCounter();
		...
		if (UINT32_MAX != frameQueryIdx)
		{
			...
			const TimerQueryD3D12::Result& result = m_gpuTimer.m_result[MAX_CONFIG_MAX_VIEWS];
		...
		{
			MAX_D3D12_PROFILER_BEGIN_LITERAL("debugstats", kColorFrame);
...
//				tvm.printf(10, pos++, 0x8b, " Uniform size: %7d, Max: %7d ", _render->m_uniformEnd, _render->m_uniformMax);
		...
		{
			MAX_D3D12_PROFILER_BEGIN_LITERAL("debugtext", kColorFrame);
