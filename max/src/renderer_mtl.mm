...
#define CHECK_FEATURE_AVAILABLE(feature, ...) if (@available(__VA_ARGS__)) { feature = true; } else { feature = false; }
			...
			m_cmd.init(m_device);
			MAX_FATAL(NULL != m_cmd.m_commandQueue, Fatal::UnableToInitialize, "Unable to create Metal device.");
			...
			for (uint8_t ii = 0; ii < MAX_CONFIG_MAX_FRAME_LATENCY; ++ii)
			{
...

			{
				if ([m_device respondsToSelector: @selector(supportsFamily:)])
				{
					...
					{
						g_caps.vendorId = MAX_PCI_ID_APPLE;
			...
			g_caps.limits.maxComputeBindings = bx::uint32_min(30, MAX_MAX_COMPUTE_BINDINGS);
			...
			if (g_caps.vendorId == MAX_PCI_ID_AMD)
			{
				hasPacked16Formats = false;
			}
			if (!hasPacked16Formats)
			{
				s_textureFormat[max::TextureFormat::R5G6B5].m_fmt = MTLPixelFormatInvalid;
				s_textureFormat[max::TextureFormat::B5G6R5].m_fmt = MTLPixelFormatInvalid;
				s_textureFormat[max::TextureFormat::BGRA4].m_fmt = MTLPixelFormatInvalid;
				s_textureFormat[max::TextureFormat::RGBA4].m_fmt = MTLPixelFormatInvalid;
			}
			...
			g_caps.formats[TextureFormat::A8     ] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER | MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			g_caps.formats[TextureFormat::RG32I  ] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			g_caps.formats[TextureFormat::RG32U  ] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			g_caps.formats[TextureFormat::RGBA32I] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			g_caps.formats[TextureFormat::RGBA32U] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			...
			if (BX_ENABLED(BX_PLATFORM_IOS) || BX_ENABLED(BX_PLATFORM_VISIONOS))
			{
				...
				g_caps.formats[TextureFormat::BC7 ] = MAX_CAPS_FORMAT_TEXTURE_NONE;

				g_caps.formats[TextureFormat::RG32F  ] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
				g_caps.formats[TextureFormat::RGBA32F] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			}
			...
			if (BX_ENABLED(BX_PLATFORM_OSX) )
			{
				...
				g_caps.formats[TextureFormat::PTC14A] = MAX_CAPS_FORMAT_TEXTURE_NONE;

				g_caps.formats[TextureFormat::RGB9E5F] &= ~(MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER | MAX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER_MSAA);
			}

			for (uint32_t ii = 0; ii < TextureFormat::Count; ++ii)
			{
				if (MAX_CAPS_FORMAT_TEXTURE_NONE == g_caps.formats[ii])
				{
		...
		void shutdown()
		{
			...
			for (uint8_t i=0; i < MAX_CONFIG_MAX_FRAME_LATENCY; ++i)
			{
		...
		{
			return MAX_RENDERER_METAL_NAME;
		}
		...
		{
			bx::strCopy(
				  &s_viewName[_id][MAX_CONFIG_MAX_VIEW_NAME_RESERVED]
		...
		void blitRender(TextVideoMemBlitter& _blitter, uint32_t _numIndices) override
		{
			...
			if (0 < numVertices)
			{
				...
				FrameBufferHandle fbh = MAX_INVALID_HANDLE;
		...
		{
			m_mainFrameBuffer.m_swapChain->m_maxAnisotropy = !!(_resolution.reset & MAX_RESET_MAXANISOTROPY)
				? 16
				: 1
				;
			...
			|| (m_resolution.reset&maskFlags) != (_resolution.reset&maskFlags) )
			{
				...
				if (m_resolution.reset & MAX_RESET_INTERNAL_FORCE
		...
		{
			_state &= MAX_STATE_WRITE_Z|MAX_STATE_DEPTH_TEST_MASK;
			...
			if (NULL == dss)
			{
				...
				desc.depthWriteEnabled = !!(MAX_STATE_WRITE_Z & _state);
				...
				if (0 != _stencil)
				{
					...
					uint32_t readMask  = (fstencil&MAX_STENCIL_FUNC_RMASK_MASK)>>MAX_STENCIL_FUNC_RMASK_SHIFT;
					uint32_t writeMask = 0xff;

					frontFaceDesc.stencilFailureOperation   = s_stencilOp[(fstencil&MAX_STENCIL_OP_FAIL_S_MASK)>>MAX_STENCIL_OP_FAIL_S_SHIFT];
					frontFaceDesc.depthFailureOperation     = s_stencilOp[(fstencil&MAX_STENCIL_OP_FAIL_Z_MASK)>>MAX_STENCIL_OP_FAIL_Z_SHIFT];
					frontFaceDesc.depthStencilPassOperation = s_stencilOp[(fstencil&MAX_STENCIL_OP_PASS_Z_MASK)>>MAX_STENCIL_OP_PASS_Z_SHIFT];
					frontFaceDesc.stencilCompareFunction    = s_cmpFunc[(fstencil&MAX_STENCIL_TEST_MASK)>>MAX_STENCIL_TEST_SHIFT];
					frontFaceDesc.readMask  = readMask;
					frontFaceDesc.writeMask = writeMask;

					backfaceDesc.stencilFailureOperation    = s_stencilOp[(bstencil&MAX_STENCIL_OP_FAIL_S_MASK)>>MAX_STENCIL_OP_FAIL_S_SHIFT];
					backfaceDesc.depthFailureOperation      = s_stencilOp[(bstencil&MAX_STENCIL_OP_FAIL_Z_MASK)>>MAX_STENCIL_OP_FAIL_Z_SHIFT];
					backfaceDesc.depthStencilPassOperation  = s_stencilOp[(bstencil&MAX_STENCIL_OP_PASS_Z_MASK)>>MAX_STENCIL_OP_PASS_Z_SHIFT];
					backfaceDesc.stencilCompareFunction     = s_cmpFunc[(bstencil&MAX_STENCIL_TEST_MASK)>>MAX_STENCIL_TEST_SHIFT];
					...
					if ((!m_usesMTLBindings && [(MTLArgument*)arg isActive]) || (m_usesMTLBindings && arg.used))
					{
						if (arg.type == MTLBindingTypeBuffer)
						{
								 ...
								 &&  NULL != binding.bufferStructType)
							{
								...
								if (arg.index >= MAX_CONFIG_MAX_TEXTURE_SAMPLERS)
								{
									BX_TRACE(
										"Binding index is too large %d max is %d. "
										"User defined uniform '%s' won't be set."
										, int32_t(arg.index - 1)
										, MAX_CONFIG_MAX_TEXTURE_SAMPLERS - 1
										, name
										);
								}
								else
						...
						else if (arg.type == MTLBindingTypeTexture)
						{
							...
							if (arg.index >= MAX_CONFIG_MAX_TEXTURE_SAMPLERS)
							{
								BX_WARN(false, "Binding index is too large %d max is %d. User defined uniform '%s' won't be set.", int(arg.index), MAX_CONFIG_MAX_TEXTURE_SAMPLERS - 1, name);
							}
							else
							{
			...
			)
		{
			...
			const bool independentBlendEnable = !!(MAX_STATE_BLEND_INDEPENDENT & _state);
			...
			if (NULL == pso)
			{
				...
				pd.alphaToCoverageEnabled = !!(MAX_STATE_BLEND_ALPHA_TO_COVERAGE & _state);
				...
				uint8_t writeMask = 0;
				writeMask |= (_state&MAX_STATE_WRITE_R) ? MTLColorWriteMaskRed   : 0;
				writeMask |= (_state&MAX_STATE_WRITE_G) ? MTLColorWriteMaskGreen : 0;
				writeMask |= (_state&MAX_STATE_WRITE_B) ? MTLColorWriteMaskBlue  : 0;
				writeMask |= (_state&MAX_STATE_WRITE_A) ? MTLColorWriteMaskAlpha : 0;
				...
				for (uint32_t ii = 0; ii < (independentBlendEnable ? 1 : frameBufferAttachment); ++ii)
				{
					...
					drt.blendingEnabled = !!(MAX_STATE_BLEND_MASK & _state);
		...
		SamplerState getSamplerState(uint32_t _flags)
		{
			...
			if (NULL == sampler)
			{
				m_samplerDescriptor.sAddressMode = s_textureAddress[(_flags&MAX_SAMPLER_U_MASK)>>MAX_SAMPLER_U_SHIFT];
				m_samplerDescriptor.tAddressMode = s_textureAddress[(_flags&MAX_SAMPLER_V_MASK)>>MAX_SAMPLER_V_SHIFT];
				m_samplerDescriptor.rAddressMode = s_textureAddress[(_flags&MAX_SAMPLER_W_MASK)>>MAX_SAMPLER_W_SHIFT];
				m_samplerDescriptor.minFilter    = s_textureFilterMinMag[(_flags&MAX_SAMPLER_MIN_MASK)>>MAX_SAMPLER_MIN_SHIFT];
				m_samplerDescriptor.magFilter    = s_textureFilterMinMag[(_flags&MAX_SAMPLER_MAG_MASK)>>MAX_SAMPLER_MAG_SHIFT];
				m_samplerDescriptor.mipFilter    = s_textureFilterMip[(_flags&MAX_SAMPLER_MIP_MASK)>>MAX_SAMPLER_MIP_SHIFT];
				m_samplerDescriptor.lodMinClamp  = 0;
				m_samplerDescriptor.lodMaxClamp  = FLT_MAX;
				m_samplerDescriptor.normalizedCoordinates = TRUE;
				m_samplerDescriptor.maxAnisotropy =  (0 != (_flags & (MAX_SAMPLER_MIN_ANISOTROPIC|MAX_SAMPLER_MAG_ANISOTROPIC) ) )
					? m_mainFrameBuffer.m_swapChain->m_maxAnisotropy
					: 1
					;

				if (0 != (g_caps.supported & MAX_CAPS_TEXTURE_COMPARE_ALL) )
				{
		...
		Buffer   m_uniformBuffers[MAX_CONFIG_MAX_FRAME_LATENCY];
		...
		IndexBufferMtl  m_indexBuffers[MAX_CONFIG_MAX_INDEX_BUFFERS];
		VertexBufferMtl m_vertexBuffers[MAX_CONFIG_MAX_VERTEX_BUFFERS];
		ShaderMtl       m_shaders[MAX_CONFIG_MAX_SHADERS];
		ProgramMtl      m_program[MAX_CONFIG_MAX_PROGRAMS];
		TextureMtl      m_textures[MAX_CONFIG_MAX_TEXTURES];
		FrameBufferMtl  m_mainFrameBuffer;
		FrameBufferMtl  m_frameBuffers[MAX_CONFIG_MAX_FRAME_BUFFERS];
	...
	void ShaderMtl::create(const Memory* _mem)
	{
		...
		MAX_FATAL(NULL != m_function
			, max::Fatal::InvalidShader
...
//		murmur.add(m_attrMask, numAttrs);
	...
	void TextureMtl::create(const Memory* _mem, uint64_t _flags, uint8_t _skip)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			const bool writeOnly    = 0 != (_flags&MAX_TEXTURE_RT_WRITE_ONLY);
			const bool computeWrite = 0 != (_flags&MAX_TEXTURE_COMPUTE_WRITE);
			const bool renderTarget = 0 != (_flags&MAX_TEXTURE_RT_MASK);
			...
			const uint32_t msaaQuality = bx::uint32_satsub( (_flags&MAX_TEXTURE_RT_MSAA_MASK)>>MAX_TEXTURE_RT_MSAA_SHIFT, 1);
	...
	void TextureMtl::commit(uint8_t _stage, bool _vertex, bool _fragment, uint32_t _flags, uint8_t _mip)
	{
		if (_vertex)
		{
			...
			s_renderMtl->m_renderCommandEncoder.setVertexSamplerState(
				  0 == (MAX_SAMPLER_INTERNAL_DEFAULT & _flags)
...
#	endif // BX_PLATFORM_*
	...
	{
		const int32_t sampleCount = s_msaa[(_flags&MAX_RESET_MSAA_MASK)>>MAX_RESET_MSAA_SHIFT];

#if BX_PLATFORM_OSX
#	if __MAC_OS_X_VERSION_MAX_ALLOWED >= 101300
		if (s_renderMtl->m_hasVSync)
		{
			m_metalLayer.displaySyncEnabled = 0 != (_flags&MAX_RESET_VSYNC);
		}

		if (s_renderMtl->m_hasMaximumDrawableCount)
		{
			m_metalLayer.maximumDrawableCount = bx::clamp<uint32_t>(
				  _maximumDrawableCount != 0 ? _maximumDrawableCount : MAX_CONFIG_MAX_FRAME_LATENCY
				, 2
				, 3
				);
		}
#	endif // __MAC_OS_X_VERSION_MAX_ALLOWED >= 101300
		...
		m_metalLayer.pixelFormat  = (_flags & MAX_RESET_SRGB_BACKBUFFER)
	...
	void FrameBufferMtl::create(uint8_t _num, const Attachment* _attachment)
	{
		...
		const uint32_t msaaQuality = bx::uint32_satsub( (firstTexture.m_flags&MAX_TEXTURE_RT_MSAA_MASK)>>MAX_TEXTURE_RT_MSAA_SHIFT, 1);
		const int32_t sampleCount = s_msaa[msaaQuality];
	...
	void FrameBufferMtl::resolve()
	{
		for (uint32_t ii = 0; ii < m_num; ++ii)
		{
			if (0 != (m_colorAttachment[ii].resolve & MAX_RESOLVE_AUTO_GEN_MIPS))
			{
				const TextureMtl& texture = s_renderMtl->m_textures[m_colorHandle[ii].idx];
				const bool isRenderTarget = !!(texture.m_flags & MAX_TEXTURE_RT_MASK);
				const bool hasMips        = 1 < texture.m_numMips;
				const bool fmtSupport     = 0 != (g_caps.formats[texture.m_textureFormat] & MAX_CAPS_FORMAT_TEXTURE_MIP_AUTOGEN);
	...
	void CommandQueueMtl::init(Device _device)
	{
		...
		m_framesSemaphore.post(MAX_CONFIG_MAX_FRAME_LATENCY);
	}
	...
	void CommandQueueMtl::kick(bool _endFrame, bool _waitForFinish)
	{
		if (m_activeCommandBuffer)
		{
			if (_endFrame)
			{
				m_releaseWriteIndex = (m_releaseWriteIndex + 1) % MAX_CONFIG_MAX_FRAME_LATENCY;
	...
	void CommandQueueMtl::consume()
	{
		...
		m_releaseReadIndex = (m_releaseReadIndex + 1) % MAX_CONFIG_MAX_FRAME_LATENCY;
...
#if BX_PLATFORM_OSX
			bool     readBack  = !!(dst.m_flags & MAX_TEXTURE_READ_BACK);
#endif  // BX_PLATFORM_OSX
	...
	void RendererContextMtl::submit(Frame* _render, ClearQuad& _clearQuad, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		...
		MAX_MTL_PROFILER_BEGIN_LITERAL("rendererSubmit", kColorFrame);
		...
		m_bufferIndex = (m_bufferIndex + 1) % MAX_CONFIG_MAX_FRAME_LATENCY;
		m_uniformBufferVertexOffset = 0;
		m_uniformBufferFragmentOffset = 0;
		...
		if (0 == (_render->m_debug&MAX_DEBUG_IFH) )
		{
			...
			for (int32_t item = 0; item < numItems;)
			{
				...
				|| (!isCompute && wasCompute) )
				{
					...
					MAX_MTL_PROFILER_END();
					setViewType(view, "  ");
					MAX_MTL_PROFILER_BEGIN(view, kColorView);
					...
					if (!isCompute)
					{
						...
						||  fbh.idx != _render->m_view[view].m_fbh.idx)
						{
							...
							if (clearWithRenderPass)
							{
								for (uint32_t ii = 0; ii < g_caps.limits.maxFBAttachments; ++ii)
								{
									...
									{
										if (0 != (MAX_CLEAR_COLOR & clr.m_flags) )
										{
											if (0 != (MAX_CLEAR_COLOR_USE_PALETTE & clr.m_flags) )
											{
												uint8_t index = (uint8_t)bx::uint32_min(MAX_CONFIG_MAX_COLOR_PALETTE-1, clr.m_index[ii]);
								...
								if (NULL != depthAttachment.texture)
								{
									...
									depthAttachment.loadAction = 0 != (MAX_CLEAR_DEPTH & clr.m_flags)
										? MTLLoadActionClear
										: MTLLoadActionLoad
										;
								...
								if (NULL != stencilAttachment.texture)
								{
									...
									stencilAttachment.loadAction   = 0 != (MAX_CLEAR_STENCIL & clr.m_flags)
										? MTLLoadActionClear
										: MTLLoadActionLoad
										;
						...
						else if (BX_ENABLED(MAX_CONFIG_DEBUG_ANNOTATION) )
						{
							rce.popDebugGroup();
						}
				...
				if (isCompute)
				{
					if (!wasCompute)
					{
						...
						setViewType(view, "C");
						MAX_MTL_PROFILER_END();
						MAX_MTL_PROFILER_BEGIN(view, kColorCompute);

						m_computeCommandEncoder = m_commandBuffer.computeCommandEncoder();
					}
					else if (viewChanged && BX_ENABLED(MAX_CONFIG_DEBUG_ANNOTATION) )
					{
						m_computeCommandEncoder.popDebugGroup();
					}

					if (viewChanged
					&&  BX_ENABLED(MAX_CONFIG_DEBUG_ANNOTATION) )
					{
							...
							switch (bind.m_type)
							{
								...
								case Binding::Texture:
								{
									...
									m_computeCommandEncoder.setSamplerState(
										0 == (MAX_SAMPLER_INTERNAL_DEFAULT & flags)
				...
				if (wasCompute)
				{
					...
					setViewType(view, " ");
					MAX_MTL_PROFILER_END();
					MAX_MTL_PROFILER_BEGIN(view, kColorDraw);
				}
					 ...
					 ) & changedFlags)
				{
					...
					{
						rce.setFrontFacingWinding( (newFlags&MAX_STATE_FRONT_CCW)
					...
					{
						const uint64_t pt = newFlags&MAX_STATE_CULL_MASK;
						const uint8_t cullIndex = uint8_t(pt>>MAX_STATE_CULL_SHIFT);
				...
				&& !(newFlags & MAX_STATE_BLEND_INDEPENDENT) )
				{
				...
				|| ( (blendFactor != draw.m_rgba) && !!(newFlags & MAX_STATE_BLEND_INDEPENDENT) ) )
				{
					currentProgram = key.m_program;
					...
					for (uint8_t stage = 0; stage < maxTextureSamplers; ++stage)
					{
						...
						||  programChanged)
						{
							if (kInvalidHandle != bind.m_idx)
							{
								...
								case Binding::Image:
								{
									if (bind.m_access == Access::ReadWrite && 0 == (g_caps.supported & MAX_CAPS_IMAGE_RW))
									{
										MAX_FATAL(
											  false
											, Fatal::DebugCheck
									...
									if ( (bind.m_access == Access::Read      && (0 == (g_caps.formats[bind.m_format] &  MAX_CAPS_FORMAT_TEXTURE_IMAGE_READ) ) )
									||   (bind.m_access == Access::Write     && (0 == (g_caps.formats[bind.m_format] &  MAX_CAPS_FORMAT_TEXTURE_IMAGE_WRITE) ) )
									||   (bind.m_access == Access::ReadWrite && (0 == (g_caps.formats[bind.m_format] & (MAX_CAPS_FORMAT_TEXTURE_IMAGE_READ|MAX_CAPS_FORMAT_TEXTURE_IMAGE_WRITE) ) ) )
									   )
									{
										MAX_FATAL(
											  false
				...
				if (0 != currentState.m_streamMask)
				{
					...
					if (isValid(draw.m_indirectBuffer) )
					{
						...
						if (isValid(draw.m_indexBuffer) )
						{
							...
							for (uint32_t ii = 0; ii < numDrawIndirect; ++ii)
							{
								rce.drawIndexedPrimitives(prim.m_type, indexFormat, ib.m_ptr, 0, vb.m_ptr, (draw.m_startIndirect + ii )* MAX_CONFIG_DRAW_INDIRECT_STRIDE);
							}
						}
						else
						{
							...
							for (uint32_t ii = 0; ii < numDrawIndirect; ++ii)
							{
								rce.drawPrimitives(prim.m_type, vb.m_ptr, (draw.m_startIndirect + ii) * MAX_CONFIG_DRAW_INDIRECT_STRIDE);
							}
						}
					}
					else
