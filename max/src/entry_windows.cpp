...
#if MAX_CONFIG_USE_NATIVE && BX_PLATFORM_WINDOWS
...
//				XInputEnable   = (PFN_XINPUT_ENABLE   )bx::dlsym(m_xinputdll, "XInputEnable"  );
		...
		XINPUT_STATE m_state[MAX_CONFIG_MAX_GAMEPADS];
		bool m_connected[MAX_CONFIG_MAX_GAMEPADS];
	};
	...
	struct EntryContext
	{
		...
		int32_t run(int _argc, const char* const* _argv)
		{
			...
			m_flags[0] = 0
				| MAX_WINDOW_FLAG_ASPECT_RATIO
				| MAX_WINDOW_FLAG_FRAME
				;

			adjust(m_hwnd[0], MAX_DEFAULT_WIDTH, MAX_DEFAULT_HEIGHT, true);
			clear(m_hwnd[0]);

			m_width     = MAX_DEFAULT_WIDTH;
			m_height    = MAX_DEFAULT_HEIGHT;
			m_oldWidth  = MAX_DEFAULT_WIDTH;
			m_oldHeight = MAX_DEFAULT_HEIGHT;
			...
			while (max::RenderFrame::NoContext != max::renderFrame() ) {};
				...
				case WM_SIZING:
					{
						...
						if (isValid(handle)
						&&  MAX_WINDOW_FLAG_ASPECT_RATIO & m_flags[handle.idx])
						{
							...
							case WMSZ_RIGHT:
								{
									...
									width  = bx::uint32_max(MAX_DEFAULT_WIDTH/4, width);
							...
							default:
								{
									...
									height = bx::uint32_max(MAX_DEFAULT_HEIGHT/4, height);
		...
		HWND m_hwnd[MAX_CONFIG_MAX_WINDOWS];
		uint32_t m_flags[MAX_CONFIG_MAX_WINDOWS];
		RECT m_rect;
		DWORD m_style;
		uint32_t m_width;
		uint32_t m_height;
	...
	static EntryContext s_ctx;

	LRESULT CALLBACK EntryContext::wndProc(HWND _hwnd, UINT _id, WPARAM _wparam, LPARAM _lparam)
	{
