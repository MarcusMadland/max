...
#ifndef MAX_RENDERER_GL_H_HEADER_GUARD
#define MAX_RENDERER_GL_H_HEADER_GUARD

#define MAX_USE_EGL ( (MAX_CONFIG_RENDERER_OPENGL || MAX_CONFIG_RENDERER_OPENGLES) && (0 \
	|| BX_PLATFORM_ANDROID                                                                  \
	|| BX_PLATFORM_LINUX                                                                    \
	|| BX_PLATFORM_NX                                                                       \
	|| BX_PLATFORM_RPI                                                                      \
...
#define MAX_USE_HTML5 (MAX_CONFIG_RENDERER_OPENGLES && (0 \
	|| BX_PLATFORM_EMSCRIPTEN                               \
	) )

#define MAX_USE_WGL (MAX_CONFIG_RENDERER_OPENGL && (0 \
	|| BX_PLATFORM_WINDOWS                              \
	) )

#define MAX_USE_GL_DYNAMIC_LIB (0 \
	|| BX_PLATFORM_LINUX           \
	|| BX_PLATFORM_WINDOWS         \
	)
...
#define MAX_GL_CONFIG_UNIFORM_CACHE BX_PLATFORM_EMSCRIPTEN

#ifndef MAX_GL_CONFIG_BLIT_EMULATION
#	define MAX_GL_CONFIG_BLIT_EMULATION 0
#endif // MAX_GL_CONFIG_BLIT_EMULATION

#ifndef MAX_GL_CONFIG_TEXTURE_READ_BACK_EMULATION
#	define MAX_GL_CONFIG_TEXTURE_READ_BACK_EMULATION 0
#endif // MAX_GL_CONFIG_TEXTURE_READ_BACK_EMULATION

#define MAX_GL_PROFILER_BEGIN(_view, _abgr)                                               \
	BX_MACRO_BLOCK_BEGIN                                                                   \
		GL_CHECK(glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION, 0, -1, s_viewName[view]) ); \
		MAX_PROFILER_BEGIN(s_viewName[view], _abgr);                                      \
	BX_MACRO_BLOCK_END

#define MAX_GL_PROFILER_BEGIN_LITERAL(_name, _abgr)                                       \
	BX_MACRO_BLOCK_BEGIN                                                                   \
		GL_CHECK(glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION, 0, -1, "" _name) );         \
		MAX_PROFILER_BEGIN_LITERAL("" _name, _abgr);                                      \
	BX_MACRO_BLOCK_END

#define MAX_GL_PROFILER_END()        \
	BX_MACRO_BLOCK_BEGIN              \
		MAX_PROFILER_END();          \
...
#	elif MAX_CONFIG_RENDERER_OPENGLES >= 30
#		include <GLES3/gl3platform.h>
#		if MAX_CONFIG_RENDERER_OPENGLES >= 32
...
#include "glimports.h"
	...
	public:
		...
		{
			if (BX_ENABLED(MAX_GL_CONFIG_UNIFORM_CACHE) )
			{
	...
	struct ProgramGL
	{
		...
		GLint    m_instanceData[MAX_CONFIG_MAX_INSTANCE_DATA_COUNT+1];
		uint16_t m_instanceOffset[MAX_CONFIG_MAX_INSTANCE_DATA_COUNT];

		GLint m_sampler[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
	...
	struct OcclusionQueryGL
	{
		...
		Query m_query[MAX_CONFIG_MAX_OCCLUSION_QUERIES];
		bx::RingBufferControl m_control;
	};

} /* namespace gl */ } // namespace max

#endif // MAX_RENDERER_GL_H_HEADER_GUARD
