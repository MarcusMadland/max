...
#if MAX_CONFIG_RENDERER_DIRECT3D11 || (MAX_CONFIG_RENDERER_DIRECT3D12 && !BX_PLATFORM_LINUX)
...
#	endif // BX_PLATFORM_WINDOWS
		...
		m_driverType = MAX_PCI_ID_SOFTWARE_RASTERIZER == _caps.vendorId
		...
		if (NULL != m_factory)
		{
			...
			{
				{
					...
					if (SUCCEEDED(hr) )
					{
						...
						if (NULL == m_adapter)
						{
							if ( (MAX_PCI_ID_NONE != _caps.vendorId ||             0 != _caps.deviceId)
							&&   (MAX_PCI_ID_NONE == _caps.vendorId || desc.VendorId == _caps.vendorId)
							...
							if (BX_ENABLED(MAX_CONFIG_DEBUG_PERFHUD)
				...
				_caps.supported |= hdr10 ? MAX_CAPS_HDR10 : 0;
				_caps.supported |= BX_ENABLED(BX_PLATFORM_WINRT) ? MAX_CAPS_TRANSPARENT_BACKBUFFER : 0;
			...
			_caps.vendorId = 0 == m_adapterDesc.VendorId
				? MAX_PCI_ID_SOFTWARE_RASTERIZER
...
#endif // MAX_CONFIG_RENDERER_DIRECT3D11 || MAX_CONFIG_RENDERER_DIRECT3D12
