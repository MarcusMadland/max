...
//		SDL_Haptic*         m_haptic;
	...
	struct EntryContext
	{
		...
		int run(int _argc, char** _argv)
		{
			...
			m_windowAlloc.alloc();
			m_window[0] = SDL_CreateWindow("max"
							...
							case SDL_USER_WINDOW_TOGGLE_FRAME:
								{
									...
									if (isValid(handle) )
									{
										m_flags[handle.idx] ^= MAX_WINDOW_FLAG_FRAME;
										SDL_SetWindowBordered(m_window[handle.idx], (SDL_bool)!!(m_flags[handle.idx] & MAX_WINDOW_FLAG_FRAME) );
									}
								}
								break;
		...
		SDL_Window* m_window[MAX_CONFIG_MAX_WINDOWS];
		uint32_t m_flags[MAX_CONFIG_MAX_WINDOWS];

		bx::HandleAllocT<MAX_CONFIG_MAX_GAMEPADS> m_gamepadAlloc;
		GamepadSDL m_gamepad[MAX_CONFIG_MAX_GAMEPADS];
