...
#	endif
	...
	struct EntryContext
	{
		...
		int run(int _argc, const char* const* _argv)
		{
			...
			m_window[0] = glfwCreateWindow(MAX_DEFAULT_WIDTH
				, MAX_DEFAULT_HEIGHT
				, "max"
				, NULL
				, NULL
			...
			m_eventQueue.postSizeEvent(handle, MAX_DEFAULT_WIDTH, MAX_DEFAULT_HEIGHT);

			for (uint32_t ii = 0; ii < MAX_CONFIG_MAX_GAMEPADS; ++ii)
			{
					...
					case GLFW_WINDOW_CREATE:
						{
							...
							if (msg->m_flags & MAX_WINDOW_FLAG_ASPECT_RATIO)
							{
		...
		GLFWwindow* m_window[MAX_CONFIG_MAX_WINDOWS];
		bx::HandleAllocT<MAX_CONFIG_MAX_WINDOWS> m_windowAlloc;

		GamepadGLFW m_gamepad[MAX_CONFIG_MAX_GAMEPADS];
	...
	void EntryContext::keyCb(GLFWwindow* _window, int32_t _key, int32_t _scancode, int32_t _action, int32_t _mods)
	{
		BX_UNUSED(_scancode);
	...
	void EntryContext::charCb(GLFWwindow* _window, uint32_t _scancode)
	{
		WindowHandle handle = s_ctx.findHandle(_window);
		uint8_t chars[4];
	...
	void EntryContext::scrollCb(GLFWwindow* _window, double _dx, double _dy)
	{
		BX_UNUSED(_dx);
		WindowHandle handle = s_ctx.findHandle(_window);
		double mx, my;
	...
	void EntryContext::cursorPosCb(GLFWwindow* _window, double _mx, double _my)
	{
		WindowHandle handle = s_ctx.findHandle(_window);
		s_ctx.m_eventQueue.postMouseEvent(handle
			, (int32_t) _mx
	...
	void EntryContext::mouseButtonCb(GLFWwindow* _window, int32_t _button, int32_t _action, int32_t _mods)
	{
		BX_UNUSED(_mods);
		WindowHandle handle = s_ctx.findHandle(_window);
	...
	void EntryContext::windowSizeCb(GLFWwindow* _window, int32_t _width, int32_t _height)
	{
		WindowHandle handle = s_ctx.findHandle(_window);
		s_ctx.m_eventQueue.postSizeEvent(handle, _width, _height);
	}

	void EntryContext::dropFileCb(GLFWwindow* _window, int32_t _count, const char** _filePaths)
	{
		WindowHandle handle = s_ctx.findHandle(_window);
	...
	static void joystickCb(int _jid, int _action)
	{
		if (_jid >= MAX_CONFIG_MAX_GAMEPADS)
		{
			return;
		}
...
#	if BX_PLATFORM_LINUX
#		if MAX_CONFIG_USE_WAYLAND
		return max::NativeWindowHandleType::Wayland;
#		else
