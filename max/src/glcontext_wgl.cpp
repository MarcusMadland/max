...
#	undef GL_IMPORT
	...
	static HGLRC createContext(HDC _hdc)
	{
		...
		MAX_FATAL(0 != pixelFormat, Fatal::UnableToInitialize, "ChoosePixelFormat failed!");
		...
		MAX_FATAL(0 != result, Fatal::UnableToInitialize, "SetPixelFormat failed!");

		HGLRC context = wglCreateContext(_hdc);
		MAX_FATAL(NULL != context, Fatal::UnableToInitialize, "wglCreateContext failed!");

		result = wglMakeCurrent(_hdc, context);
		MAX_FATAL(0 != result, Fatal::UnableToInitialize, "wglMakeCurrent failed!");
	...
	void GlContext::create(uint32_t /*_width*/, uint32_t /*_height*/, uint32_t /*_flags*/)
	{
		...
		MAX_FATAL(NULL != m_opengl32dll, Fatal::UnableToInitialize, "Failed to load opengl32.dll.");

		wglGetProcAddress = bx::dlsym<PFNWGLGETPROCADDRESSPROC>(m_opengl32dll, "wglGetProcAddress");
		MAX_FATAL(NULL != wglGetProcAddress, Fatal::UnableToInitialize, "Failed get wglGetProcAddress.");
		...
		BX_WARN(NULL != g_platformData.nwh
			, "max::setPlatform with valid window is not called. This might "
		...
		if (NULL != g_platformData.nwh && NULL != g_platformData.context )
		{
			...
			MAX_FATAL(NULL != wglMakeCurrent, Fatal::UnableToInitialize, "Failed get wglMakeCurrent.");

			m_hdc = GetDC( (HWND)g_platformData.nwh);
			MAX_FATAL(NULL != m_hdc, Fatal::UnableToInitialize, "GetDC failed!");
		...
		if (NULL != g_platformData.nwh && NULL == g_platformData.context )
		{
			...
			MAX_FATAL(NULL != wglCreateContext, Fatal::UnableToInitialize, "Failed get wglCreateContext.");

			wglDeleteContext = bx::dlsym<PFNWGLDELETECONTEXTPROC>(m_opengl32dll, "wglDeleteContext");
			MAX_FATAL(NULL != wglDeleteContext, Fatal::UnableToInitialize, "Failed get wglDeleteContext.");
			...
			HDC hdc = GetDC(hwnd);
			MAX_FATAL(NULL != hdc, Fatal::UnableToInitialize, "GetDC failed!");
			...
			&&  NULL != wglCreateContextAttribsARB)
			{
				...
				int32_t flags = MAX_CONFIG_DEBUG ? WGL_CONTEXT_DEBUG_BIT_ARB : 0;
				BX_UNUSED(flags);
				int32_t contextAttrs[9] =
				{
#if MAX_CONFIG_RENDERER_OPENGL >= 31
					WGL_CONTEXT_MAJOR_VERSION_ARB, MAX_CONFIG_RENDERER_OPENGL / 10,
					WGL_CONTEXT_MINOR_VERSION_ARB, MAX_CONFIG_RENDERER_OPENGL % 10,
				...
				MAX_FATAL(NULL != m_context, Fatal::UnableToInitialize, "Failed to create context 0x%08x.", GetLastError() );
	...
	void GlContext::resize(uint32_t /*_width*/, uint32_t /*_height*/, uint32_t _flags)
	{
		if (NULL != wglSwapIntervalEXT)
		{
			bool vsync = !!(_flags&MAX_RESET_VSYNC);
...
#	define GL_EXTENSION(_optional, _proto, _func, _import)                                     \
		{                                                                                      \
			...
			{                                                                                  \
				...
				MAX_FATAL(BX_IGNORE_C4127(_optional) || NULL != _func                         \
