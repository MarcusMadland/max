...
#ifndef MAX_RENDERER_METAL_H_HEADER_GUARD
#define MAX_RENDERER_METAL_H_HEADER_GUARD
...
#define MAX_MTL_PROFILER_BEGIN(_view, _abgr)         \
	BX_MACRO_BLOCK_BEGIN                              \
		MAX_PROFILER_BEGIN(s_viewName[view], _abgr); \
	BX_MACRO_BLOCK_END

#define MAX_MTL_PROFILER_BEGIN_LITERAL(_name, _abgr) \
	BX_MACRO_BLOCK_BEGIN                              \
		MAX_PROFILER_BEGIN_LITERAL("" _name, _abgr); \
	BX_MACRO_BLOCK_END

#define MAX_MTL_PROFILER_END() \
	BX_MACRO_BLOCK_BEGIN        \
		MAX_PROFILER_END();    \
	BX_MACRO_BLOCK_END
...
#define MTL_RELEASE(_obj) \
	...
	struct ProgramMtl
	{
		...
		uint32_t m_instanceData[MAX_CONFIG_MAX_INSTANCE_DATA_COUNT+1];
	...
	struct PipelineStateMtl
	{
		PipelineStateMtl()
			...
			, m_cps(NULL)
			{
				...
				m_numThreads[1] = 1;
				m_numThreads[2] = 1;
				for(uint32_t i=0; i<MAX_CONFIG_MAX_TEXTURE_SAMPLERS; ++i)
					m_bindingTypes[i] = 0;
			}
		...
		uint8_t m_bindingTypes[MAX_CONFIG_MAX_TEXTURE_SAMPLERS];
	...
	struct TextureMtl
	{
		...
		void commit(
			...
			, bool _vertex
			, bool _fragment
			, uint32_t _flags = MAX_SAMPLER_INTERNAL_DEFAULT
	...
	struct FrameBufferMtl
	{
		...
		TextureHandle m_colorHandle[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS-1];
		TextureHandle m_depthHandle;
		Attachment m_colorAttachment[MAX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS-1];
...
} /* namespace metal */ } // namespace max

#endif // MAX_CONFIG_RENDERER_METAL

#endif // MAX_RENDERER_METAL_H_HEADER_GUARD
