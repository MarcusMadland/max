...
#define MAX_SHADER_BIN_VERSION 11
#define MAX_CHUNK_MAGIC_CSH BX_MAKEFOURCC('C', 'S', 'H', MAX_SHADER_BIN_VERSION)
#define MAX_CHUNK_MAGIC_FSH BX_MAKEFOURCC('F', 'S', 'H', MAX_SHADER_BIN_VERSION)
#define MAX_CHUNK_MAGIC_VSH BX_MAKEFOURCC('V', 'S', 'H', MAX_SHADER_BIN_VERSION)

#define MAX_SHADERC_VERSION_MAJOR 1
#define MAX_SHADERC_VERSION_MINOR 18
...
#undef HEX_DUMP_FORMAT
	...
	void addFragData(Preprocessor& _preprocessor, char* _data, uint32_t _idx, bool _comma)
	{
		...
		char replace[32];
		bx::snprintf(replace, sizeof(replace), "max_FragData%d", _idx);

		strReplace(_data, find, replace);

		_preprocessor.writef(
			" \\\n\t%sout vec4 max_FragData%d : SV_TARGET%d"
			, _comma ? ", " : "  "
			, _idx
			, _idx
			);
	...
	void voidFragData(char* _data, uint32_t _idx)
	{
		...
		strReplace(_data, find, "max_VoidFrag");
	}
	...
	bool compileShader(const char* _varying, const char* _comment, char* _shader, uint32_t _shaderLen, const Options& _options, bx::WriterI* _shaderWriter, bx::WriterI* _messageWriter)
	{
		...
		preprocessor.setDefaultDefine("MAX_SHADER_LANGUAGE_GLSL");
		preprocessor.setDefaultDefine("MAX_SHADER_LANGUAGE_HLSL");
		preprocessor.setDefaultDefine("MAX_SHADER_LANGUAGE_METAL");
		preprocessor.setDefaultDefine("MAX_SHADER_LANGUAGE_PSSL");
		preprocessor.setDefaultDefine("MAX_SHADER_LANGUAGE_SPIRV");

		preprocessor.setDefaultDefine("MAX_SHADER_TYPE_COMPUTE");
		preprocessor.setDefaultDefine("MAX_SHADER_TYPE_FRAGMENT");
		preprocessor.setDefaultDefine("MAX_SHADER_TYPE_VERTEX");
		...
		if (0 == bx::strCmpI(platform, "android") )
		{
			...
			{
				preprocessor.setDefine("MAX_SHADER_LANGUAGE_SPIRV=1");
			}
			else
			{
		...
		else if (0 == bx::strCmpI(platform, "orbis") )
		{
			...
			preprocessor.setDefine("MAX_SHADER_LANGUAGE_PSSL=1");
		...
		{
		case 'c':
			preprocessor.setDefine("MAX_SHADER_TYPE_COMPUTE=1");
			break;

		case 'f':
			preprocessor.setDefine("MAX_SHADER_TYPE_FRAGMENT=1");
			break;

		case 'v':
			preprocessor.setDefine("MAX_SHADER_TYPE_VERTEX=1");
			break;
		...
		else if (raw)
		{
			if ('f' == _options.shaderType)
			{
				bx::write(_shaderWriter, MAX_CHUNK_MAGIC_FSH, &err);
			}
			else if ('v' == _options.shaderType)
			{
				bx::write(_shaderWriter, MAX_CHUNK_MAGIC_VSH, &err);
			}
			else
			{
				bx::write(_shaderWriter, MAX_CHUNK_MAGIC_CSH, &err);
			}
		...
		else // Vertex/Fragment
		{
			...
			else
			{
				...
				||  profile->lang == ShadingLang::ESSL)
				{
					if (profile->lang != ShadingLang::ESSL)
					{
						// max shadow2D/Proj behave like EXT_shadow_samplers
						// not as GLSL language 1.2 specs shadow2D/Proj.
						preprocessor.writef(
							"#define shadow2D(_sampler, _coord) maxShadow2D(_sampler, _coord).x\n"
							"#define shadow2DProj(_sampler, _coord) maxShadow2DProj(_sampler, _coord).x\n"
							);
					}
					...
					&&  profile->id >= 300)
					{
						...
						if (hasFragColor)
						{
							preprocessor.writef("#define gl_FragColor max_FragColor\n");
							preprocessor.writef("out mediump vec4 max_FragColor;\n");
						}
						else if (numFragData)
						{
							preprocessor.writef("#define gl_FragData max_FragData\n");
							preprocessor.writef("out mediump vec4 max_FragData[gl_MaxDrawBuffers];\n");
						}
					}
				...
				else
				{
					...
					*const_cast<char*>(entry.getPtr() + 4) = '_';
					...
					if ('f' == _options.shaderType)
					{
						...
						const bool hasPrimitiveId = !bx::strFind(input, "gl_PrimitiveID").isEmpty() && MAX_CAPS_PRIMITIVE_ID;
						...
						if (0 == numFragData)
						{
							...
							preprocessor.writef("#define gl_FragColor max_FragData0\n");
						...
						||  profile->lang == ShadingLang::ESSL)
						{
							...
							if (glsl_profile < 400)
							{
								...
								if (profile->lang != ShadingLang::ESSL)
								{
									...
									{
										bx::stringPrintf(code
											, "#define maxShadow2D(_sampler, _coord)     vec4_splat(texture(_sampler, _coord) )\n"
											  "#define maxShadow2DProj(_sampler, _coord) vec4_splat(textureProj(_sampler, _coord) )\n"
											);
									}
									else
									{
										bx::stringPrintf(code
											, "#define maxShadow2D     shadow2D\n"
											  "#define maxShadow2DProj shader2DProj\n"
											);
									}
								}
								else
...
{
	return max::compileShader(_argc, _argv);
}
