...
namespace max { namespace hlsl
{
	...
	{
		max::Attrib::Enum m_attr;
		const char* m_name;
		uint8_t m_index;
	};

	static const RemapInputSemantic s_remapInputSemantic[max::Attrib::Count + 1] =
	{
		{ max::Attrib::Position,  "POSITION",     0 },
		{ max::Attrib::Normal,    "NORMAL",       0 },
		{ max::Attrib::Tangent,   "TANGENT",      0 },
		{ max::Attrib::Bitangent, "BITANGENT",    0 },
		{ max::Attrib::Color0,    "COLOR",        0 },
		{ max::Attrib::Color1,    "COLOR",        1 },
		{ max::Attrib::Color2,    "COLOR",        2 },
		{ max::Attrib::Color3,    "COLOR",        3 },
		{ max::Attrib::Indices,   "BLENDINDICES", 0 },
		{ max::Attrib::Weight,    "BLENDWEIGHT",  0 },
		{ max::Attrib::TexCoord0, "TEXCOORD",     0 },
		{ max::Attrib::TexCoord1, "TEXCOORD",     1 },
		{ max::Attrib::TexCoord2, "TEXCOORD",     2 },
		{ max::Attrib::TexCoord3, "TEXCOORD",     3 },
		{ max::Attrib::TexCoord4, "TEXCOORD",     4 },
		{ max::Attrib::TexCoord5, "TEXCOORD",     5 },
		{ max::Attrib::TexCoord6, "TEXCOORD",     6 },
		{ max::Attrib::TexCoord7, "TEXCOORD",     7 },
		{ max::Attrib::Count,     "",             0 },
	};

	const RemapInputSemantic& findInputSemantic(const char* _name, uint8_t _index)
	{
		for (uint32_t ii = 0; ii < max::Attrib::Count; ++ii)
		{
		...
		return s_remapInputSemantic[max::Attrib::Count];
	}
	...
	bool getReflectionDataD3D11(ID3DBlob* _code, bool _vshader, UniformArray& _uniforms, uint8_t& _numAttrs, uint16_t* _attrs, uint16_t& _size, UniformNameList& unusedUniforms, bx::WriterI* _messageWriter)
	{
		...
		if (_vshader) // Only care about input semantic on vertex shaders
		{
			for (uint32_t ii = 0; ii < desc.InputParameters; ++ii)
			{
				...
				if (ris.m_attr != max::Attrib::Count)
				{
					_attrs[_numAttrs] = max::attribToId(ris.m_attr);
					++_numAttrs;
				}
			}
		}
	...
	static bool compile(const Options& _options, uint32_t _version, const std::string& _code, bx::WriterI* _shaderWriter, bx::WriterI* _messageWriter, bool _firstPass)
	{
		...
		uint8_t numAttrs = 0;
		uint16_t attrs[max::Attrib::Count];
		uint16_t size = 0;
