...
#include <bx/string.h>
#include <max/max.h>
#include "../../max/src/vertexlayout.h"
...
#ifdef MAX_GEOMETRYC_FBX_COMPILER
#include <fbxsdk.h>
#endif // MAX_GEOMETRYC_FBX_COMPILER

#define CGLTF_VALIDATE_ENABLE_ASSERTS BX_CONFIG_DEBUG
#define CGLTF_IMPLEMENTATION
#include <cgltf/cgltf.h>

#define MAX_GEOMETRYC_VERSION_MAJOR 1
#define MAX_GEOMETRYC_VERSION_MINOR 0

#if 0
#	define BX_TRACE(_format, ...) \
		do { \
			bx::printf(BX_FILE_LINE_LITERAL "MAX " _format "\n", ##__VA_ARGS__); \
		} while(0)
...
void calcTangents(void* _vertices, uint16_t _numVertices, max::VertexLayout _layout, const uint16_t* _indices, uint32_t _numIndices)
{
	struct PosTexcoord
	{
		float m_x;
...
int main(int _argc, const char* _argv[])
{
	...
	*/
	...
	layout.begin();
	layout.add(max::Attrib::Position, 3, max::AttribType::Float);

	if (hasColor)
	{
		layout.add(max::Attrib::Color0, 4, max::AttribType::Uint8, true);
	}

	if (hasBc)
	{
		layout.add(max::Attrib::Color1, 4, max::AttribType::Uint8, true);
	}
	...
	if (hasTexcoord)
	{
		...
		default:
		case 0:
			layout.add(max::Attrib::TexCoord0, 2, max::AttribType::Float);
			break;

		case 1:
			layout.add(max::Attrib::TexCoord0, 2, max::AttribType::Half);
			break;
		}
	}
	...
	if (hasNormal)
	{
		...
		default:
		case 0:
			layout.add(max::Attrib::Normal, 3, max::AttribType::Float);
			if (hasTangent)
			{
				layout.add(max::Attrib::Tangent, 4, max::AttribType::Float);
			}
			break;

		case 1:
			layout.add(max::Attrib::Normal, 4, max::AttribType::Uint8, true, true);
			if (hasTangent)
			{
				layout.add(max::Attrib::Tangent, 4, max::AttribType::Uint8, true, true);
			}
			break;
		}
	}
	...
	if (hasIndices)
	{
		layout.add(max::Attrib::Indices, 4, max::AttribType::Float);
	}

	if (hasWeights)
	{
		layout.add(max::Attrib::Weight, 4, max::AttribType::Float);
	}
		...
		for (uint32_t tri = groupIt->m_startTriangle, end = tri + groupIt->m_numTriangles; tri < end; ++tri)
		{
			...
			for (uint32_t edge = 0; edge < 3; ++edge)
			{
				...
				if (hasBc)
				{
					...
					max::vertexPack(bc, true, max::Attrib::Color1, layout, vertices);
				}
				...
				if (hasTexcoord)
				{
					...
					max::vertexPack(uv, true, max::Attrib::TexCoord0, layout, vertices);
				}
				...
				if (hasIndices)
				{
					...
					max::vertexPack(indices, false, max::Attrib::Indices, layout, vertices);
				}
				...
				if (hasWeights)
				{
					...
					max::vertexPack(weights, false, max::Attrib::Weight, layout, vertices);
				}
